...
status_t InputDispatcher::pilferPointers(const sp<IBinder>& token) {
    ...
    for (const TouchedWindow& window : state.windows) {
        if (!window.windowHandle->getInfo()->canPilferPointer()) {
            continue;
        }
    ...
    // Prevent the gesture from being sent to any other can pilfer pointer stream windows.
    state.filterCanPilferPointerWindowsExcept(token);
