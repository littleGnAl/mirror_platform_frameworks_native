...
void InputReader::vibrate(int32_t deviceId, const VibrationElement* pattern, size_t patternSize,
...
void InputDevice::vibrate(const VibrationElement* pattern, size_t patternSize, ssize_t repeat,
        int32_t token) {
    size_t numMappers = mMappers.size();
    for (size_t i = 0; i < numMappers; i++) {
        InputMapper* mapper = mMappers[i];
...
void InputMapper::vibrate(const VibrationElement* pattern, size_t patternSize, ssize_t repeat,
        int32_t token) {
}
...
void VibratorInputMapper::vibrate(const VibrationElement* pattern, size_t patternSize,
        ssize_t repeat, int32_t token) {
#if DEBUG_VIBRATOR
    ...
    for (size_t i = 0; i < patternSize; i++) {
        ...
        VibrationElement element = pattern[i];
        std::string channelStr;
        bool first = true;
        for (uint16_t channel : element.channels) {
            if (!first) {
                channelStr += ", ";
            } else {
                first = false;
            }
            channelStr += std::to_string(channel);
        }

        patternStr += StringPrintf("%" PRId64 " : [%s]", element.duration, channelStr.c_str());
    }
    ...
    mVibrating = true;
    std::copy(pattern, pattern + patternSize, std::begin(mPattern));
...
void VibratorInputMapper::nextStep() {
    ...
    VibrationElement effect = mPattern[mIndex];

    // vibrator is on if any of its channels are non-zero
    bool vibratorOn = false;
    for (uint16_t value : effect.channels) {
        if (value) {
            vibratorOn = true;
            break;
        }
    ...
    if (vibratorOn) {
        ...
        std::string channelStr;
        bool first = true;
        for (uint16_t channel : effect.channels) {
            if (!first) {
                channelStr += ", ";
            } else {
                first = false;
        ...
        ALOGD("nextStep: sending vibrate deviceId=%d, duration=%" PRId64 ", channels=[%s]",
                getDeviceId(), effect.duration, channelStr.c_str());
#endif
