...
void VibratorInputMapper::vibrate(const std::vector<VibrationElement>& pattern, ssize_t repeat,
    ...
    for (auto elementIt = pattern.begin(); elementIt != pattern.end(); elementIt++) {
        if (elementIt != pattern.begin()) {
            patternStr += ", ";
        }

        std::string channelStr;
        for (auto channelIt = elementIt->channels.begin(); channelIt != elementIt->channels.end();
                channelIt++) {
            if (channelIt != elementIt->channels.begin()) {
                channelStr += ", ";
            }
            channelStr += std::to_string(*channelIt);
        }

        patternStr += StringPrintf("%" PRId64 " : [%s]", elementIt->duration, channelStr.c_str());
    }
