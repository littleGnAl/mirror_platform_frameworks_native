...
std::optional<int32_t> EventHub::getBatteryCapacity(int32_t deviceId, int32_t batteryId) const {
...

    {
        ...
        path = it->second.path.c_str();
    }

    std::string buffer;

    // Some devices report battery capacity as an integer through the "capacity" file
    if (base::ReadFileToString(path + "/" + BATTERY_NODES.at(InputBatteryClass::CAPACITY),
                               &buffer)) {
    ...
    if (base::ReadFileToString(path + "/" + BATTERY_NODES.at(InputBatteryClass::CAPACITY_LEVEL),
                               &buffer)) {
...
std::optional<int32_t> EventHub::getBatteryStatus(int32_t deviceId, int32_t batteryId) const {
    ...
    if (!base::ReadFileToString(path + "/" + BATTERY_NODES.at(InputBatteryClass::STATUS),
                                &buffer)) {
