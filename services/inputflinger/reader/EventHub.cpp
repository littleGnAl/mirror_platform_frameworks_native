...
EventHub::EventHub(void)
    ...
    if (isV4lScanningEnabled()) {
        mDevWd = inotify_add_watch(mINotifyFd, DEVICE_PATH, IN_DELETE | IN_CREATE);
        LOG_ALWAYS_FATAL_IF(mDevWd < 0, "Could not register INotify for %s: %s",
 ...
 * directory will be absent. However, the user may still plug in an input device at a later time. Add
 * watch for contents of /dev/input only when /dev/input appears.
 */
void EventHub::addDeviceInotify() {
    mInputWd = inotify_add_watch(mINotifyFd, DEVICE_INPUT_PATH, IN_DELETE | IN_CREATE);
    LOG_ALWAYS_FATAL_IF(mInputWd < 0, "Could not register INotify for %s: %s", DEVICE_INPUT_PATH,
                        strerror(errno));
}
    ...
    while (res >= (int)sizeof(*event)) {
        ...
        if (event->len) {
            if (event->wd == mInputWd) {
                std::string filename = StringPrintf("%s/%s", DEVICE_INPUT_PATH, event->name);
