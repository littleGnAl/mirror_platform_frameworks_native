    ...
    if (!changes.any() || !isIgnored()) {
        ...
        if (!changes.any() || changes.test(Change::DISPLAY_INFO)) {
            ...
            if (!inputDeviceDescriptor.empty()) {
                const std::unordered_map<std::string, uint8_t>& ports = readerConfig.portAssociations;
                const auto& displayPort = ports.find(inputDeviceDescriptor);
                if (displayPort != ports.end()) {
                    mAssociatedDisplayPort = std::make_optional(displayPort->second);
                } else {
                    const std::unordered_map<std::string, std::string>&
                            displayUniqueIdsByDescriptor = readerConfig.uniqueIdAssociationsByDescriptor;
            ...
            if (mAssociatedDisplayPort) {
                ...
                mAssociatedViewport =
                        readerConfig.getDisplayViewportByUniqueId(
                                *mAssociatedDisplayUniqueIdByDescriptor);
                if (!mAssociatedViewport) {
                ...
                mAssociatedViewport =
                        readerConfig.getDisplayViewportByUniqueId(*mAssociatedDisplayUniqueIdByPort);
                if (!mAssociatedViewport) {
                    ALOGW("Input device %s should be associated with display %s but the "
                          "corresponding viewport cannot be found",
