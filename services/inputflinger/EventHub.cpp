...
bool EventHub::searchLedsPathLocked(String8& searchPath) const {
    ...
    ALOGD("Searching for leds directory in %s", searchPath.string());
    while((dirEntry = readdir(dir)) != NULL) {
        String8 dirEntryString(dirEntry->d_name);
        ALOGD("Got entry in search path: %s", dirEntryString.string());

        if (dirEntryString.find("leds",0) >= 0) {
            // searchPath = searchPath/leds
...
bool EventHub::getLedSysfsPathLocked(const String8& eventName, String8& ledSysfsPath) const {
    ...
    // Xpad for example, does not expose its LED on /sys/class/input/eventX/, therefore
    // all searches need to be on the realpath.
    //
    // Search steps:
    // - Search for leds directory in realpath(/sys/class/input/eventX) + "/device/device"
    // - Search for leds directory in realpath(/sys/class/input/eventX) + "/../../../.."
    // - Search for leds directory in realpath(/sys/class/input/eventX)
    // - Search for leds directory in realpath(/sys/class/input/eventX) + "/.."
    // - Search for leds directory in realpath(/sys/class/input/eventX) + "/../.."
    // - Search for leds directory in realpath(/sys/class/input/eventX) + "/../../.."
    ...
    // search for leds directory
    // e.g. dualshock4 searchPath = /sys/devices/soc/6a00000.ssusb/6a00000.dwc3/xhci-hcd.0.auto/usb1/1-1/1-1.2/1-1.2:1.3/0003:054C:09CC.0001/input/input10/event12/device/device
    String8 searchPath = eventRealPath + "/device/device";
    bool found = searchLedsPathLocked(searchPath);
    // e.g. dualshock4 searchPath = /sys/devices/soc/6a00000.ssusb/6a00000.dwc3/xhci-hcd.0.auto/usb1/1-1/1-1.2/1-1.2:1.3/0003:054C:09CC.0001/input/input10/event12/device/device/leds
    if (found) {
        ledSysfsPath = searchPath + String8("/");
        ALOGI("Found LED sysfs path for %s: %s",
                eventName.string(), ledSysfsPath.string());
        return true;
    }

    // search for leds directory in other most likely paths
    // e.g. xpad searchPath = /sys/devices/soc/6a00000.ssusb/6a00000.dwc3/xhci-hcd.0.auto/usb1/1-1/1-1.4/1-1.4:1.0/input/input10/event9/../../../..
    searchPath = eventRealPath.getPathDir().getPathDir().getPathDir().getPathDir();
    found = searchLedsPathLocked(searchPath);
    // e.g. xpad searchPath = /sys/devices/soc/6a00000.ssusb/6a00000.dwc3/xhci-hcd.0.auto/usb1/1-1/1-1.4/leds
    if (found) {
        ledSysfsPath = searchPath + String8("/");
        ALOGI("Found LED sysfs path for %s: %s",
                eventName.string(), ledSysfsPath.string());
    ...
    // exhaustive search for leds directory, starting from eventX path,
    // but not searching recursively because sysfs can contain looping directories
    searchPath = eventRealPath;
    while (searchPath != eventRealPath.getPathDir().getPathDir().getPathDir().getPathDir()) {
        found = searchLedsPathLocked(searchPath);
        if (found) {
            ledSysfsPath = searchPath + String8("/");
            ALOGI("Found LED sysfs path for %s: %s",
            ...
            searchPath = searchPath.getPathDir();
        }
    }
