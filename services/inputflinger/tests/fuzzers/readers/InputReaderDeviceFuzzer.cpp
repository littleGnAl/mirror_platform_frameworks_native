...
extern "C" int LLVMFuzzerTestOneInput(uint8_t* data, size_t size) {
    ...
    int32_t deviceId = tester.ConsumeIntegralInRange(0, 10);
    constexpr uint32_t deviceClass = INPUT_DEVICE_CLASS_KEYBOARD;
    InputDevice* device =
            mReader->newDevice(deviceId, tester.ConsumeIntegralInRange(0, 10) /*controllerNumber*/,
                               tester.ConsumeRandomLengthString(
                                       tester.ConsumeIntegralInRange<int32_t>(0, kMaxSize)),
                               deviceClass);
    ...
    mReader->setNextDevice(device);
    addDevice(deviceId,
              tester.ConsumeRandomLengthString(tester.ConsumeIntegralInRange<int32_t>(0, kMaxSize)),
    ...
    mapper->setKeyCodeState(tester.ConsumeIntegralInRange<int32_t>(-5, 300), AKEY_STATE_DOWN);
    // GetScanCodeState_ForwardsRequestsToMappers

    mapper->setScanCodeState(tester.ConsumeIntegralInRange<int32_t>(-5, 300), AKEY_STATE_DOWN);

    // GetSwitchState_ForwardsRequestsToMappers
    mapper->setSwitchState(SW_LID, AKEY_STATE_DOWN);

    // MarkSupportedKeyCodes_ForwardsRequestsToMappers
    mapper->addSupportedKeyCode(tester.ConsumeIntegralInRange<int32_t>(-5, 300));
    mapper->addSupportedKeyCode(tester.ConsumeIntegralInRange<int32_t>(-5, 300));

    // LoopOnce_WhenDeviceScanFinished_SendsConfigurationChanged
    addDevice(tester.ConsumeIntegralInRange(0, 10), tester.ConsumeRandomLengthString(kMaxSize),
              INPUT_DEVICE_CLASS_KEYBOARD, nullptr, mFakeEventHub, mReader);
    NotifyConfigurationChangedArgs args;

    // LoopOnce_ForwardsRawEventsToMappers
    mFakeEventHub->enqueueEvent(tester.ConsumeIntegralInRange(0, 10),
                                tester.ConsumeIntegralInRange(0, 10), EV_KEY, KEY_A,
                                tester.ConsumeIntegralInRange(0, 10));
    mReader->loopOnce();
    ...
    addDevice(deviceId, tester.ConsumeRandomLengthString(12), deviceClass, nullptr, mFakeEventHub,
              mReader);
    ...
    // constexpr int32_t deviceId = 1;
    // constexpr uint32_t deviceClass = INPUT_DEVICE_CLASS_KEYBOARD;
    std::string DEVICE_LOCATION = tester.ConsumeRandomLengthString(kMaxSize);
    // InputDevice* device = mReader->newDevice(deviceId, 0 /*controllerNumber*/, "fake",
    ...
    addDevice(deviceId, tester.ConsumeRandomLengthString(kMaxSize), deviceClass, nullptr,
              mFakeEventHub, mReader);
    uint8_t hdmi1 = tester.ConsumeIntegralInRange(0, 10);
    // Associated touch screen with second display.
    mFakePolicy->addInputPortAssociation(DEVICE_LOCATION, hdmi1);
    // Add default and second display.
    std::string displayViewPortParam = "local" + tester.ConsumeRandomLengthString(8) + ":" +
            tester.ConsumeIntegralInRange<char>(0, kMaxSize);

    mFakePolicy->addDisplayViewport(DISPLAY_ID, tester.ConsumeIntegralInRange<int32_t>(0, kMaxSize),
                                    tester.ConsumeIntegralInRange<int32_t>(0, kMaxSize),
    ...
    mFakePolicy->addDisplayViewport(SECONDARY_DISPLAY_ID,
                                    ...
                                    tester.ConsumeRandomLengthString(8) + ":" +
                                            tester.ConsumeIntegralInRange<char>(0, kMaxSize),
