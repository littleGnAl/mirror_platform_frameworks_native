...
extern "C" int LLVMFuzzerTestOneInput(uint8_t* data, size_t size) {
    ...
    InputDevice* device =
            mReader->newDevice(deviceId, tester.ConsumeIntegralInRange(0, 10) /*controllerNumber*/,
                               tester.ConsumeRandomLengthString(
                                       tester.ConsumeIntegralInRange(0, MAX_SIZE)),
                               deviceClass);
    ...
    mReader->setNextDevice(device);
    addDevice(deviceId,
              tester.ConsumeRandomLengthString(tester.ConsumeIntegralInRange(0, MAX_SIZE)),
    ...
    addDevice(tester.ConsumeIntegralInRange(0, 10), tester.ConsumeRandomLengthString(MAX_SIZE),
    ...
    // constexpr int32_t deviceId = 1;
    // constexpr uint32_t deviceClass = INPUT_DEVICE_CLASS_KEYBOARD;
    std::string DEVICE_LOCATION = tester.ConsumeRandomLengthString(MAX_SIZE);
    // InputDevice* device = mReader->newDevice(deviceId, 0 /*controllerNumber*/, "fake",
    ...
    addDevice(deviceId, tester.ConsumeRandomLengthString(MAX_SIZE), deviceClass, nullptr,
    ...
    std::string displayViewPortParam = "local" + tester.ConsumeRandomLengthString(8) + ":" +
            (char)tester.ConsumeIntegralInRange(0, MAX_SIZE);

    mFakePolicy->addDisplayViewport(DISPLAY_ID, tester.ConsumeIntegralInRange(0, MAX_SIZE),
                                    tester.ConsumeIntegralInRange(0, MAX_SIZE),
