...
TEST_F(JoystickInputMapperTest, Configure_AssignsDisplayPort) {
    ...
    // We don't test mapper's return value here, because it may or may not have a notion of whether the viewport exists
    ASSERT_FALSE(mDevice->isEnabled());

    // 3. Ensure that when joystick is mapped and screen is attached, events goes to correct display
    // Prepare second display.
