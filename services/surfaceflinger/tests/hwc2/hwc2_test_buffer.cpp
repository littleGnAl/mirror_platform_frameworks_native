...
layout(location = 71) uniform vec4[10] clear_frames;
...
bool is_clear(in vec2 p, in int idx)
{
    return p.x >= clear_frames[idx].x && p.x < clear_frames[idx].z
            && p.y >= clear_frames[idx].y && p.y < clear_frames[idx].w;
}
...
public:
            ...
            const std::set<hwc2_layer_t> *clear_layers)
    {
        ...
        std::vector<vec4> clear_frames;

        if (clear_layers) {
            for (auto layer: *clear_layers) {
                const auto &frame = test_layers->get_display_frame(layer);
                clear_frames.push_back({
                        static_cast<float>(frame.left) / buffer_width,
                        static_cast<float>(frame.top) / buffer_height,
                        static_cast<float>(frame.right) / buffer_width,
                        static_cast<float>(frame.bottom) / buffer_height});
        ...
        clear_frames.resize(HWC2_TEST_BUFFER_MAX, {0.0, 0.0, 0.0, 0.0});

        program.bind_vec4fv(71, clear_frames.size(), clear_frames.data());
    }
    ...
    int get_next(const hwc2_test_layers *test_layers,
            ...
            const std::set<hwc2_layer_t> *clear_layers)
    {
        if (client_layers && clear_layers
                && client_layers->size() > HWC2_TEST_BUFFER_MAX
                && clear_layers->size() > HWC2_TEST_BUFFER_MAX)
            return -EINVAL;
        ...
        send_layer_properties(test_layers, client_layers, clear_layers);
...
int hwc2_test_buffer::get(buffer_handle_t *out_handle, int32_t *out_fence)
{
    ...
    return get(out_handle, out_fence, nullptr, nullptr, nullptr, true);
}
...
int hwc2_test_buffer::get(buffer_handle_t *out_handle, int32_t *out_fence,
        ...
        const std::set<hwc2_layer_t> *clear_layers)
{
    return get(out_handle, out_fence, test_layers, client_layers, clear_layers,
            false);
}
...
int hwc2_test_buffer::get(buffer_handle_t *out_handle, int32_t *out_fence,
        const hwc2_test_layers *test_layers,
        const std::set<hwc2_layer_t> *client_layers,
        const std::set<hwc2_layer_t> *clear_layers, bool save_handle)
{
