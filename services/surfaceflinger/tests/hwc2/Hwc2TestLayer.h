...
class Hwc2TestLayer {
public:
    Hwc2TestLayer(hwc2_test_coverage_t coverage, int32_t displayWidth,
            int32_t displayHeight);

    Hwc2TestLayer(hwc2_test_coverage_t coverage, int32_t displayWidth,
            int32_t displayHeight, const std::map<hwc2_test_property_t,
    ...
    std::pair<int32_t, int32_t> getBufferArea() const;
    hwc_color_t            getColor() const;
    hwc2_composition_t     getComposition() const;
    std::pair<int32_t, int32_t> getCursor() const;
...
private:
    std::array<Hwc2TestContainer*, 11> mProperties = {{
        &mBlendMode, &mTransform, &mColor, &mCursor, &mDataspace, &mPlaneAlpha,
        &mSourceCrop, &mSurfaceDamage, &mBufferArea, &mDisplayFrame,
        &mComposition
    }};
