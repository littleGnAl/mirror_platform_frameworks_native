 ...
 * The buffer cannot be guarenteed to be the same format across all devices so
...
int Hwc2TestClientTargetBuffer::get(buffer_handle_t* outHandle,
        int32_t* outFence, int32_t bufferWidth, int32_t bufferHeight,
        ...
        const std::set<hwc2_layer_t>* clearLayers)
{
    ...
    mGraphicBuffer = mGraphicBufferAlloc.createGraphicBuffer(bufferWidth,
            bufferHeight, mFormat,
            GRALLOC_USAGE_SW_READ_OFTEN | GRALLOC_USAGE_HW_RENDER,
    ...
    float bWDiv3 = bufferWidth / 3;
    float bW2Div3 = bufferWidth * 2 / 3;
    float bHDiv3 = bufferHeight / 3;
    float bH2Div3 = bufferHeight * 2 / 3;
    ...
    for (int32_t y = 0; y < bufferHeight; y++) {
        for (int32_t x = 0; x < bufferWidth; x++) {
            ...
            for (auto layer = clientLayers->rbegin();
                ...
                /* Calculate the layer display frame.*/
                if (testLayers->getComposition(*layer)
                        != HWC2_COMPOSITION_CURSOR) {
                    /* For non cursor layers, just use the display frame */
                    ...
                    /* For cursor layers, ignore the display frame and use
                     * the cursor position and buffer area. */
                    const auto bufferArea = testLayers->getBufferArea(*layer);
                    const auto cursor = testLayers->getCursor(*layer);
                    dfL = cursor.x;
                    dfT = cursor.y;
                    dfR = cursor.x + bufferArea.width;
                    dfB = cursor.y + bufferArea.height;
                }
                 ...
                 * be:
                 * if (yPos < (bufferHeight / 3 - sourceCrop.top)
                 *        //Select first section color
                 *
