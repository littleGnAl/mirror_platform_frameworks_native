...
protected:
            ...
            const std::vector<hwc2_layer_t> &layers, uint32_t num_requests)
    {
        ...
        for (size_t i = 0; i < requests.size(); i++) {
            EXPECT_TRUE(std::find(layers.begin(), layers.end(),
                    requested_layers.at(i)) != layers.end())
                    << "get display requests returned an unknown layer";
            EXPECT_NE(requests.at(i), 0) << "returned empty request for layer "
                    << requested_layers.at(i);
        }
    }
    ...
    for (auto display = get_displays_begin(); display != get_displays_end();
        ...
        if (err == HWC2_ERROR_NONE)
            EXPECT_TRUE(std::find(configs.begin(), configs.end(), active_config)
                    != configs.end()) << "active config is not found in configs"
                    " for display";
        else
            EXPECT_EQ(err, HWC2_ERROR_BAD_CONFIG) << "returned wrong error code";
    }
    ...
    for (auto display = get_displays_begin(); display != get_displays_end();
        ...
        for (hwc2_config_t config: configs) {
            ...
            do {
                ...
                EXPECT_NO_FATAL_FAILURE(set_layer_composition_type(*display,
                        layer, test_layer.get_composition()));
        ...
        for (hwc2_config_t config: configs) {
            ...
            do {
                ...
                ASSERT_NO_FATAL_FAILURE(set_layer_composition_type(*display,
                        layer, test_layer.get_composition(), &err));
