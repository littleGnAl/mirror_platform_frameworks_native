...
void Hwc2TestBufferArea::setDependent(Hwc2TestSurfaceDamage* surfaceDamage)
{
    ...
    if (mSurfaceDamage) {
        const Area& curr = get();
        mSurfaceDamage->updateBufferArea(curr.width, curr.height);
    }
}
...
Hwc2TestCursor::Hwc2TestCursor(Hwc2TestCoverage coverage,
        int32_t displayWidth, int32_t displayHeight)
...
{
    std::stringstream dmp;
    const Point& curr = get();
    dmp << "\tcursor: x " << curr.x << ", y " << curr.y << "\n";
    return dmp.str();
}
...
void Hwc2TestCursor::update()
{
    ...
    for (uint32_t leftIdx : mScalars) {
        for (uint32_t topIdx : mScalars) {
            mCursors.push_back(
                    {static_cast<int32_t>(mScalars.at(leftIdx) * mDisplayWidth),
                    static_cast<int32_t>(mScalars.at(topIdx) * mDisplayHeight)});
        }
    }
}
