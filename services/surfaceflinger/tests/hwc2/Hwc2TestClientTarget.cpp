...
int Hwc2TestClientTarget::getBuffer(const Hwc2TestLayers& testLayers,
        ...
        int32_t displayWidth, int32_t displayHeight, buffer_handle_t* outHandle,
        int32_t* outAcquireFence)
{
    ...
    mBuffer.updateBufferArea(displayWidth, displayHeight);
    return mBuffer.get(outHandle, outAcquireFence, &testLayers, &clientLayers,
            &clearLayers);
}
...
const std::pair<int32_t, int32_t> Hwc2TestClientTargetSupport::getBufferArea() const
{
