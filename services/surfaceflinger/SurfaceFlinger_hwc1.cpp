    ...
    if (!curr.isIdenticalTo(draw)) {
        ...
        for (size_t i = 0; i < cc; i++) {
            if (draw.indexOfKey(curr.keyAt(i)) < 0) {
                ...
                if (state.isVirtualDisplay()) {
                    ...
                    if (state.surface != NULL) {
                        ...
                        sp<VirtualDisplaySurface> vds =
                                new VirtualDisplaySurface(*mHwc, hwcDisplayId, state.surface,
                    ...
                    dispSurface = new FramebufferSurface(*mHwc, state.type, bqConsumer);
                    producer = bqProducer;
                }
                ...
                if (dispSurface != NULL) {
                    sp<DisplayDevice> hw =
                            new DisplayDevice(this, state.type, hwcDisplayId,
                    ...
                    if (state.isVirtualDisplay()) {
                        if (hwcDisplayId >= 0) {
                            mHwc->setVirtualDisplayProperties(hwcDisplayId, hw->getWidth(),
                                                              hw->getHeight(), hw->getFormat());
                        }
                    } else {
