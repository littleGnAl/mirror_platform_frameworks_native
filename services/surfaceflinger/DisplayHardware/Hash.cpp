...
uint64_t rotateByAtLeast1(uint64_t val, int shift) {
...
uint64_t hash64Len0To16(const char* s, uint64_t len) {
    if (len > 8) {
        ...
        return hash64Len16(a, rotateByAtLeast1(b + len, static_cast<int>(len))) ^ b;
    }
    if (len >= 4) {
...
uint64_t cityHash64Len0To16(std::string_view sv) {
    auto len = sv.length();
    if (len > 16) {
        ALOGE("cityHash64Len0To16 called with length %zu. Only hashing the first 16 chars", len);
        len = 16;
    }
