...
protected:
    ...
    class SyncPoint {
    public:
        explicit SyncPoint(uint64_t frameNumber, wp<Layer> requestedSyncLayer, wp<Layer> barrierLayer_legacy)
...
protected:
    ...
    std::list<State> mPendingStatesSnapshot;

    // these are protected by an external lock (mStateLock)
    State mCurrentState;
    std::atomic<uint32_t> mTransactionFlags{0};
    std::list<State> mPendingStates;
