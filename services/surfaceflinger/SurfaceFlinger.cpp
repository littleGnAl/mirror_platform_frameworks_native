...
void SurfaceFlinger::setActiveConfigInternal(const sp<DisplayDevice>& hw, int mode) {
    ...
    getHwComposer().setActiveConfig(hwcId, mode);
}
...
void SurfaceFlinger::createDefaultDisplayDevice() {
    ...
    mDrawingState.displays.add(token, DisplayDeviceState(DisplayDevice::DISPLAY_PRIMARY,
                               type, true));
            ...
            for (size_t i=0 ; i<dc ;) {
                ...
                if (j < 0) {
                    ...
                    if (!draw[i].isMainDisplay()) {
                        ...
                        if (draw[i].hwcId < DisplayDevice::NUM_BUILTIN_DISPLAY_TYPES)
...
void SurfaceFlinger::drawWormhole(const sp<const DisplayDevice>& displayDevice,
...
String8 SurfaceFlinger::getUniqueLayerName(const String8& name)
{
    ...
    ALOGD_IF(dupeCounter > 0, "duplicate layer name: changing %s to %s", name.c_str(),
	           uniqueName.c_str());
