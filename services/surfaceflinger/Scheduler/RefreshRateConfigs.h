...
public:
    ...
    const RefreshRate getRefreshRateFromType(RefreshRateType type) const EXCLUDES(mLock) {
    ...
    std::pair<RefreshRateType, RefreshRate> getCurrentRefreshRate() const EXCLUDES(mLock) {
    ...
    const RefreshRate getRefreshRateFromConfigId(int configId) const EXCLUDES(mLock) {
        std::lock_guard lock(mLock);
        LOG_ALWAYS_FATAL_IF(configId >= mRefreshRates.size());
        return mRefreshRates[configId];
    }

    RefreshRateType getRefreshRateTypeFromHwcConfigId(hwc2_config_t hwcId) const EXCLUDES(mLock) {
    ...
    void setCurrentConfig(int config) EXCLUDES(mLock) {
        std::lock_guard lock(mLock);
    ...
    void updateDisplayConfigs(
            ...
            int currentConfig) {
        LOG_ALWAYS_FATAL_IF(mRefreshRateSwitchingSupported,
                            "Changing the display configs is not allowed when refresh rate "
        ...
        init(mRefreshRateSwitchingSupported, inputConfigs, currentConfig);
    }

private:
    void init(bool refreshRateSwitching, const std::vector<InputConfig>& configs, int currentConfig)
            EXCLUDES(mLock) {
        std::lock_guard lock(mLock);
        ...
        auto findDefaultAndPerfConfigs =
                [&]() REQUIRES(mLock) -> std::optional<std::pair<int, int>> {
            if (configs.size() < 2) {
                return {};
            }
    ...
    // object is initialized.
    std::vector<RefreshRate> mRefreshRates GUARDED_BY(mLock);
