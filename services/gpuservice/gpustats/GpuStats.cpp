...
void GpuStats::purgeOldDriverStats() {
    ...
    // Create a list of package names and their last access times.
    int index = 0;
    for (const auto &it : mAppStats) {
        GpuStatsApp &gpuStatsApp = gpuStatsApps[index];
        gpuStatsApp.appStatsKey = &it.first;
        gpuStatsApp.lastAccessTime = &it.second.lastAccessTime;
        ++index;
    }
...
void GpuStats::insertDriverStats(const std::string& driverPackageName,
    ...
    if (!mAppStats.count(appStatsKey)) {
        if (mAppStats.size() >= MAX_NUM_APP_RECORDS) {
            ALOGV("GpuStatsAppInfo has reached maximum size. Removing old apps to make room.");
            purgeOldDriverStats();
        }
...
AStatsManager_PullAtomCallbackReturn GpuStats::pullAppInfoAtom(AStatsEventList* data) {
    ...
    if (data) {
        for (const auto& ele : mAppStats) {
            ...
            // TODO: b/154955572: Need to add `ele.second.angleInUse` and regenerate
            // addAStatsEvent(). It seems this is done with `stats-log-api-gen`...somehow.
