...
bool SensorDevice::isClientIdleLocked(void* ident) {
    ...
    return (mDisabledClients.count(ident) > 0) &&
           ((mDisabledClients[ident] & reasonMask) == reasonMask);
}
    ...
    for (size_t i = 0; i< mActivationCount.size(); ++i) {
        ...
        if (info.hasBatchParamsForIdent(ident)) {
            ...
            if (sensorType == static_cast<int>(SensorType::STEP_COUNTER)) {
                ...
                ALOGD("<<<STEP_COUNTER>>> setUidStateForConnection(), "
                    "to_disable_hardware = %s, to_enable_hardware = %s",
...
int SensorDevice::Info::numUndeadClients() const {
    ...
    numUndead = numActive + (numIdle - numGone);
        ...
        ALOGD("<<<STEP_COUNTER>>> numUndeadClients(), "
            "numActive = %d, numIdle = %d, numGone = %d, return numUndead = %d",
