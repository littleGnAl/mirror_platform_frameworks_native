...
bool SensorDevice::connectHidlService() {
    ...
    mApexService->getHalPackageVersion("sensors", &apex_version);
    ...
    if (!apex_version.empty()) {
        if (status == HalConnectionStatus::CONNECTED) {
            mApexService->certifyHalPackage("sensors", apex_version);
        } else {
            mApexService->rollbackHalPackage("sensors", apex_version);
        }
    }
