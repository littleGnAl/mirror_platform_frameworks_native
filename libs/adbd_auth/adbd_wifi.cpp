...
public:
    ...
    uint64_t NotifyAuthenticated(std::string_view guid) EXCLUDES(mutex_) {
        uint64_t id = NextId();
        std::lock_guard<std::mutex> lock(mutex_);
        guids_.emplace(id, guid);
        output_queue_.emplace_back(
                AdbdWifiPacketDisconnected{.guid = std::string(guid)});
        return id;
    }
...
private:
    ...
    void DispatchPendingPrompt() REQUIRES(mutex_) {
        ...
        auto [id, guid, arg] = std::move(pending_prompts_.front());
        pending_prompts_.pop_front();

        this->output_queue_.emplace_back(
                AdbdWifiPacketRequestAuthorization{.guid = guid});

        Interrupt();
        dispatched_prompt_ = std::make_tuple(id, guid, arg);
    }
...
private:
    ...
    uint64_t PromptUser(std::string_view guid, void* arg) EXCLUDES(mutex_) {
        uint64_t id = NextId();

        std::lock_guard<std::mutex> lock(mutex_);
        pending_prompts_.emplace_back(id, guid, arg);
...
uint64_t adbd_wifi_notify_auth(AdbdWifiContext* ctx, const char* guid, size_t len) {
    return ctx->NotifyAuthenticated(std::string_view(guid, len));
}
