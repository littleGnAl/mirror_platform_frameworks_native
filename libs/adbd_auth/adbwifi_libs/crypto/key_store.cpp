...
KeyStore::PeerInfo KeyStoreImpl::getPeerInfo(std::string_view guid) {
    ...
    return makePeerInfo(it->second.guid(), it->second.name(), it->second.certificate());
}
...
std::pair<std::string, KeyStore::PeerInfo> KeyStoreImpl::operator[](const size_t idx) const {
    ...
    return std::pair<std::string, PeerInfo>(it->first,
                                            makePeerInfo(key.guid(), key.name(), key.certificate()));
}
...
bool KeyStoreImpl::generateSystemCertificate() {
    ...
    if (!file) {
        PLOG(ERROR) << "Unable to open private system key file for writing";
        return false;
    }
    if (!PEM_write_PKCS8PrivateKey(file.get(), evpKey.get(), nullptr, nullptr,
                                   0, nullptr, nullptr)) {
        PLOG(ERROR) << "Unable to write private system key";
        return false;
    }
    ...
    if (!PEM_write_X509(file.get(), x509.get())) {
        PLOG(ERROR) << "Unable to write public system key file";
        return false;
    }
...
bool KeyStoreImpl::writeKeyStoreToFile(std::unordered_map<std::string, adbwifi::proto::Key>& keys) {
    ...
    if (temp_file->fd == -1) {
        PLOG(ERROR) << "Failed to open keystore file '" << temp_file->path
                    << "' for writing";
        return false;
    }
    ...
    if (!SafeReplaceFile(storeName, temp_file_name)) {
        LOG(ERROR) << "Failed to replace old key store";
        // Remove the temp keystore file.
...
// static
KeyStore::PeerInfo KeyStore::makePeerInfo(const std::string& guid,
                                          const std::string& name,
                                          const std::string& cert) {
    return std::make_tuple(guid, name, cert);
}
