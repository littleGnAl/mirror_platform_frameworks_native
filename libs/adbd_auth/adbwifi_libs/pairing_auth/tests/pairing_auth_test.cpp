...
TEST_F(AdbWifiPairingAuthTest, EmptyPassword) {
    ...
    ASSERT_DEATH({
        uint8_t p;
        client = PairingAuthUniquePtr(pairing_auth_client_new(&p, 0),
                                       PairingAuthDeleter);
    }, "");
}
...
TEST_F(AdbWifiPairingAuthTest, NoInitCipher) {
...
TEST_F(AdbWifiPairingAuthTest, DifferentPasswords) {
    ...
    EXPECT_TRUE(pairing_auth_init_cipher(client.get(),
                                         server_msg.data(),
                                         server_msg.size()));
    EXPECT_TRUE(pairing_auth_init_cipher(server.get(),
                                         client_msg.data(),
                                         client_msg.size()));
    ...
    ASSERT_TRUE(pairing_auth_encrypt(client.get(), msg.data(), msg.size(),
    ...
    ASSERT_GT(server_msg.size(), 0);
    ASSERT_TRUE(pairing_auth_encrypt(server.get(), msg.data(), msg.size(),
                                     server_msg.data(), &out_size));
    ASSERT_GT(out_size, 0);
    server_msg.resize(out_size);

    client_msg.resize(pairing_auth_safe_decrypted_size(client.get(), server_msg.data(),
                                                       server_msg.size()));
    ASSERT_GT(client_msg.size(), 0);
...
TEST_F(AdbWifiPairingAuthTest, CorruptedPayload) {
    ...
    std::vector<uint8_t> msg{0x2a, 0x2b, 0x2c, 0xff,
                             0x45, 0x12, 0x33, 0x45,
                             0x12, 0xea, 0xf2, 0xdb};
    {
