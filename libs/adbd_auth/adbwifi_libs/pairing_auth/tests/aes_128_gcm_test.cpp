...
TEST(Aes128GcmTest, init_null_material) {
    ...
    ASSERT_DEATH({
        cipher.reset(new Aes128Gcm(nullptr, 42));
    }, "");
}
...
TEST(Aes128GcmTest, init_empty_material) {
    ...
    std::unique_ptr<Aes128Gcm> cipher;
    ASSERT_DEATH({
        cipher.reset(new Aes128Gcm(material, 0));
    }, "");
}
...
TEST(Aes128GcmTest, encrypt_decrypt) {
    ...
    ASSERT_GE(alice.EncryptedSize(sizeof(msg)),
              sizeof(msg));
    int encrypted_size = alice.Encrypt(msg, sizeof(msg),
                                      encrypted, sizeof(encrypted));
    ASSERT_GT(encrypted_size, 0);
    size_t out_size = sizeof(out_buf);
    ASSERT_GE(bob.DecryptedSize(encrypted, sizeof(encrypted)),
              sizeof(msg));
    int decrypted_size = bob.Decrypt(encrypted, sizeof(encrypted),
                                    out_buf, out_size);
    ASSERT_EQ(sizeof(msg), decrypted_size);
    memset(out_buf + decrypted_size, 0, sizeof(out_buf) - decrypted_size);
    ASSERT_STREQ(reinterpret_cast<const char*>(msg),
...
TEST(Aes128GcmTest, padding) {
    ...
    // Test with aligned data
    {
        ...
        int encrypted_size = alice.Encrypt(msg.data(), msg.size(),
        ...
        int decrypted_size = bob.Decrypt(encrypted.data(), encrypted.size(),
