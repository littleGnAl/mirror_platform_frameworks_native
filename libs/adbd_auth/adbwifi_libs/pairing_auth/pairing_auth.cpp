...
PairingAuthCtx::PairingAuthCtx(Role role, const Data& pswd) :
        role_(role) {
    CHECK(!pswd.empty());
...
PairingAuthCtx::Data PairingAuthCtx::Encrypt(const PairingAuthCtx::Data& data) {
    ...
    int bytes = cipher_->Encrypt(data.data(),
                                 data.size(),
                                 encrypted.data(),
                                 encrypted.size());
    if (bytes < 0) {
...
PairingAuthCtx::Data PairingAuthCtx::Decrypt(const PairingAuthCtx::Data& data) {
    ...
    int bytes = cipher_->Decrypt(data.data(),
                                 data.size(),
                                 decrypted.data(),
                                 decrypted_size);
    if (bytes <= 0) {
...
bool pairing_auth_encrypt(PairingAuthCtx* ctx, const uint8_t* inbuf, size_t inlen,
...
bool pairing_auth_decrypt(PairingAuthCtx* ctx, const uint8_t* inbuf, size_t inlen,
                          uint8_t* outbuf, size_t* outlen) {
    CHECK(ctx);
    CHECK(inbuf);
    CHECK(outbuf);
