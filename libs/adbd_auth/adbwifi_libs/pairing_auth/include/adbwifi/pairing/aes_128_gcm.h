...
#include "adbwifi/pairing/counter.h"
...
public:
    ...
    // Decrypt a block of data in |in| of length |inLen|, this consumes all data
    // in |inLen| bytes of data. The decrypted output is placed in the |out|
    // buffer of length |outLen|. On successful decryption the number of bytes in
    // |out| will be placed in |outLen|.
    ...
    size_t decryptedSize(const uint8_t* encryptedData, size_t encryptedSize);
...
private:
    ...
    static const EVP_CIPHER* mCipher;
};
