...
public:
    explicit TlsConnectionImpl(Role role,
...
TlsConnectionImpl::TlsConnectionImpl(Role role,
                                     std::string_view cert,
                                     std::string_view priv_key) :
    role_(role) {
...
// static
bssl::UniquePtr<X509> TlsConnectionImpl::x509FromBuffer(
        bssl::UniquePtr<CRYPTO_BUFFER> buffer) {
    if (!buffer) {
        return nullptr;
    }
...
bool TlsConnectionImpl::doHandshake(int fd) {
    ...
    if (enable_cert_verification_) {
        ...
        for (auto const& cert : known_certificates_) {
            if (X509_STORE_add_cert(SSL_CTX_get_cert_store(ssl_ctx_.get()),
                                    cert.get()) == 0) {
        ...
        SSL_CTX_set_cert_verify_callback(ssl_ctx_.get(),
                                         [](X509_STORE_CTX*, void*) -> int { return 1; },
                                         nullptr);
    }
    ...
    std::vector<CRYPTO_BUFFER*> cert_chain = { cert_.get(), };
    if (!(err = SSL_CTX_set_chain_and_key(ssl_ctx_.get(),
                                          &cert_chain[0],
                                          1,
                                          priv_key_.get(),
                                          nullptr))) {
