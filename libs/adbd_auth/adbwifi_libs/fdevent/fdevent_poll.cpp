...
#include "adbwifi/fdevent/fdevent_poll.h"
...
static void fdevent_interrupt(int fd, unsigned, void*) {
    char buf[BUFSIZ];
    ssize_t rc = TEMP_FAILURE_RETRY(sysdeps::adb_read(fd, buf, sizeof(buf)));
    if (rc == -1) {
...
fdevent_context_poll::fdevent_context_poll() {
    int s[2];
    if (sysdeps::adb_socketpair(s) != 0) {
        PLOG(FATAL) << "failed to create fdevent interrupt socketpair";
    }

    if (!sysdeps::set_file_block_mode(s[0], false) || !sysdeps::set_file_block_mode(s[1], false)) {
...
void fdevent_context_poll::Set(fdevent* fde, unsigned events) {
    CheckMainThread();
    fde->state = events;
    LOG(INFO) << android::base::StringPrintf("fdevent_set: %s, events = %u", dump_fde(fde).c_str(), events);
}

static std::string dump_pollfds(const std::vector<sysdeps::adb_pollfd>& pollfds) {
    std::string result;
...
void fdevent_context_poll::Loop() {
    ...
    while (true) {
        ...
        LOG(INFO) << android::base::StringPrintf("--- --- waiting for events");
        std::vector<sysdeps::adb_pollfd> pollfds;
        for (const auto& [fd, fde] : this->installed_fdevents_) {
            sysdeps::adb_pollfd pfd;
            pfd.fd = fd;
            pfd.events = 0;
        ...
        LOG(INFO) << android::base::StringPrintf("poll(), pollfds = %s", dump_pollfds(pollfds).c_str());
        ...
        int ret = sysdeps::adb_poll(pollfds.data(), pollfds.size(), timeout_ms);
        if (ret == -1) {
        ...
        for (const auto& pollfd : pollfds) {
            ...
            if (events != 0) {
                LOG(INFO) << android::base::StringPrintf("%s got events %x", dump_fde(fde).c_str(), events);
...
void fdevent_context_poll::Interrupt() {
    int rc = sysdeps::adb_write(this->interrupt_fd_, "", 1);
