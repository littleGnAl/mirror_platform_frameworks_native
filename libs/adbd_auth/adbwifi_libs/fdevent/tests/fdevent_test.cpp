  ...
  private:
    static void FdEventCallback(int fd, unsigned events, void* userdata) {
        ...
        if (events & FDE_READ) {
            ...
            char c;
            ASSERT_EQ(1, sysdeps::adb_read(fd, &c, 1));
        ...
        if (events & FDE_WRITE) {
            ...
            handler->queue_.pop();
            ASSERT_EQ(1, sysdeps::adb_write(fd, &c, 1));
...
TEST_F(FdeventTest, smoke) {
    for (bool use_new_callback : {true, false}) {
        ...
        int fd_pair1[2];
        int fd_pair2[2];
        ASSERT_EQ(0, sysdeps::adb_socketpair(fd_pair1));
        ASSERT_EQ(0, sysdeps::adb_socketpair(fd_pair2));
        ...
        fdevent_run_on_main_thread([&thread_arg, &fd_handlers, use_new_callback]() {
            ...
            for (size_t i = 0; i < thread_arg.middle_pipe_count; ++i) {
                int fds[2];
                ASSERT_EQ(0, sysdeps::adb_socketpair(fds));
        ...
        TerminateThread();
        ASSERT_EQ(0, sysdeps::adb_close(writer));
        ASSERT_EQ(0, sysdeps::adb_close(reader));
    }
}
...
TEST_F(FdeventTest, timeout) {
    ...
    fdevent_run_on_main_thread([&]() {
        test.fde = fdevent_create(fds[0], [](fdevent* fde, unsigned events, void* arg) {
            ...
            if ((events & FDE_READ)) {
                char buf[2];
                ssize_t rc = sysdeps::adb_read(fde->fd.get(), buf, sizeof(buf));
                if (rc == 0) {
    ...
    ASSERT_EQ(1, sysdeps::adb_write(fds[1], "", 1));
