...
public:
    ...
    using ResultCallback = std::function<void(const PeerInfo& peer_info, void* opaque)>;
    enum class Role {
        Client,
        Server,
    };
    ...
    // Starts the pairing connection. This call is non-blocking, as it relies on
    // fdevents. Upon completion, if the pairing was successful,
    // |cb| will be called with the PeerInfo. Otherwise, |cb| will be
    // called with an empty |header|. |fd| should already be opened.
    // PairingConnection will take ownership of the |fd|.
    //
    ...
    // to create an instance. |pswd|, |certificate|, and |priv_key| cannot be empty.
    // |fd| must have a valid file descriptor.
