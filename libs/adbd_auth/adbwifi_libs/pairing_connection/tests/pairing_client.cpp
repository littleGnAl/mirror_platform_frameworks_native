...
public:
    ...
    // return false.
    virtual bool Start(std::string_view ip_addr,
...
private:
    ...
    static void PairingResultCallback(const PeerInfo* peer_info,
                                      int fd,
                                      void* opaque);
...
ConnectionPtr PairingClientImpl::CreatePairingConnection(const Data& pswd,
                                                         ...
                                                         const Data& cert,
                                                         const Data& priv_key) {
    return ConnectionPtr(pairing_connection_client_new(pswd.data(),
                                                       pswd.size(),
                                                       peer_info,
                                                       cert.data(),
                                                       cert.size(),
...
bool PairingClientImpl::Start(std::string_view ip_addr,
                              pairing_client_result_cb cb,
                              void* opaque) {
    CHECK(!ip_addr.empty());
    cb_ = cb;
    ...
    std::string err;
    CHECK(android::base::ParseNetAddress(std::string(ip_addr),
                                        &host_,
                                        &port_,
                                        nullptr,
                                        &err));
...
// static
void PairingClientImpl::PairingResultCallback(const PeerInfo* peer_info,
                                              int /* fd */,
                                              void* opaque) {
...
bool PairingClientImpl::StartConnection() {
    ...
    connection_ = CreatePairingConnection(pswd_,
                                          peer_info_,
                                          cert_,
                                          priv_key_);
...
std::unique_ptr<PairingClient> PairingClient::Create(const Data& pswd,
    ...
    return std::unique_ptr<PairingClient>(
            new PairingClientImpl(pswd, peer_info, cert, priv_key));
}
