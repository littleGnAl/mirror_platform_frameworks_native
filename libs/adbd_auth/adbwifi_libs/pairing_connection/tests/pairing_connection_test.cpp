...
protected:
    ...
    std::unique_ptr<PairingServer> createServer(const std::vector<uint8_t> pswd) {
        ...
        key.assign(reinterpret_cast<const uint8_t*>(kTestServerPrivKey.data()),
                   reinterpret_cast<const uint8_t*>(kTestServerPrivKey.data()) + kTestServerPrivKey.size() + 1);
        return PairingServer::create(pswd,
                                     server_info_,
                                     cert,
                                     key,
                                     kDefaultPairingPort);
...
TEST_F(AdbWifiPairingConnectionTest, SmokeValidPairing) {
    ...
    auto server_callback = [&](const PeerInfo* peer_info,
                               void* opaque) {
        ASSERT_NE(nullptr, peer_info);
        EXPECT_EQ(nullptr, opaque);

        // Verify the peer_info
        ASSERT_EQ(strlen(peer_info->info), strlen(client_info_.info));
        EXPECT_EQ(::memcmp(peer_info->info, client_info_.info, strlen(client_info_.info)), 0);
    ...
    auto client_callback = [&](const PeerInfo* peer_info,
                               void* opaque) {
        ASSERT_NE(nullptr, peer_info);
        EXPECT_EQ(nullptr, opaque);

        // Verify the peer_info
        ASSERT_EQ(strlen(peer_info->info), strlen(server_info_.info));
        EXPECT_EQ(::memcmp(peer_info->info, server_info_.info, strlen(server_info_.info)), 0);

        got_valid_pairing = (peer_info != nullptr);
...
TEST_F(AdbWifiPairingConnectionTest, MultipleClientsAllFail) {
    ...
    while (clients.size() < test_num_clients) {
        ...
        auto callback = [&](const PeerInfo* peer_info,
                            void* opaque) {
            ASSERT_EQ(nullptr, peer_info);
            EXPECT_EQ(nullptr, opaque);
