...
TEST_F(AdbWifiPairingConnectionTest, ServerCreation) {
    ...
    // Bad password
    ASSERT_DEATH({
        auto server = CreateServer({}, server_info_, {0x01}, {0x01}, 0);
    }, "");
    // Bad peer_info
    ASSERT_DEATH({
        auto server = CreateServer({0x01}, {}, {0x01}, {0x01}, 0);
    }, "");
    // Bad certificate
    ASSERT_DEATH({
        auto server = CreateServer({0x01}, server_info_, {}, {0x01}, 0);
    }, "");
    // Bad private key
    ASSERT_DEATH({
        auto server = CreateServer({0x01}, server_info_, {0x01}, {}, 0);
...
TEST_F(AdbWifiPairingConnectionTest, ClientCreation) {
    ...
    // Bad password
    ASSERT_DEATH({
    auto client = PairingClient::Create({}, client_info_, {0x01}, {0x01});
    }, "");
    // Bad peer_info
    ASSERT_DEATH({
    auto client = PairingClient::Create({0x01}, {}, {0x01}, {0x01});
    }, "");
    // Bad certificate
    ASSERT_DEATH({
    auto client = PairingClient::Create({0x01}, client_info_, {}, {0x01});
    }, "");
    // Bad private key
    ASSERT_DEATH({
    auto client = PairingClient::Create({0x01}, client_info_, {0x01}, {});
