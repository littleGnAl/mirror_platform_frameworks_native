...
using namespace adbwifi::pairing;
...
public:
    ...
    explicit PairingServerCtx(const Data& pswd,
    ...
    // of this object.
    //
    // Returns true if PairingServerCtx was successfully started. Otherwise,
    // returns false.
    bool Start(pairing_server_result_cb cb, void* opaque);
...
private:
    ...
    struct ConnectionDeleter {
        void operator()(PairingConnectionCtx* p) {
            pairing_connection_destroy(p);
        }
    };
    using ConnectionPtr = std::unique_ptr<PairingConnectionCtx,
                                          ConnectionDeleter>;
    static ConnectionPtr CreatePairingConnection(const Data& pswd,
                                                 const PeerInfo& peer_info,
                                                 const Data& cert,
                                                 const Data& priv_key);
...
PairingServerCtx::ConnectionPtr PairingServerCtx::CreatePairingConnection(const Data& pswd,
                                                                          ...
                                                                          const Data& cert,
                                                                          const Data& priv_key) {
    return ConnectionPtr(pairing_connection_server_new(pswd.data(),
                                                       pswd.size(),
                                                       peer_info,
                                                       cert.data(),
                                                       cert.size(),
...
PairingServerCtx::PairingServerCtx(const Data& pswd,
                                   const PeerInfo& peer_info,
                                   const Data& cert,
                                   const Data& priv_key,
                                   int port) :
...
bool PairingServerCtx::Start(pairing_server_result_cb cb,
                              void* opaque) {
    cb_ = cb;
    opaque_ = opaque;
...
bool PairingServerCtx::SetupServer() {
    ...
    for (;;) {
        ...
        while (conn_read_queue_.size() > 0) {
            ...
            if (auto* p = std::get_if<NewConnectionEvent>(&event)) {
                ...
                int fd = ufd.release();
                bool started = pairing_connection_start(connection.get(),
                                                        fd,
                                                        PairingConnectionCallback,
                                                        this);
                  if (!started) {
...
bool PairingServerCtx::HandleNewClientConnection(int fd) {
    ...
    auto connection = CreatePairingConnection(pswd_,
                                              peer_info_,
                                              cert_,
                                              priv_key_);
...
bool pairing_server_start(PairingServerCtx* ctx, pairing_server_result_cb cb, void* opaque) {
    return ctx->Start(cb, opaque);
}

PairingServerCtx* pairing_server_new(const uint8_t* pswd,
                                     size_t pswd_len,
                                     const PeerInfo& peer_info,
    ...
    return new PairingServerCtx(vec_pswd, peer_info,
                                vec_x509_cert, vec_priv_key, port);
}
