...
using PairingAuthPtr = std::unique_ptr<PairingAuthCtx, decltype(&PairingAuthDeleter)>;
...
public:
    ...
    explicit PairingConnectionCtx(Role role,
                               const Data& pswd,
...
PairingConnectionCtx::PairingConnectionCtx(Role role,
        ...
        cert_(cert),
        priv_key_(priv_key),
        auth_(CreatePairingAuthPtr(role, pswd)) {
    CHECK(!pswd_.empty() &&
          !cert_.empty() &&
          !priv_key_.empty());
}
...
bool PairingConnectionCtx::DoExchangePeerInfo() {
    ...
    CHECK(!outbuf.empty());
    size_t outsize;
    if (!pairing_auth_encrypt(auth_.get(), buf.data(), buf.size(),
                              outbuf.data(), &outsize)) {
    ...
    if (!tls_->writeFully(std::string_view(reinterpret_cast<const char*>(outbuf.data()),
                                           outbuf.size()))) {
    ...
    outbuf.resize(
            pairing_auth_safe_decrypted_size(auth_.get(), buf.data(), buf.size()));
    if (outbuf.empty()) {
        LOG(ERROR) << "Unsupported payload while decrypting peer info.";
        return false;
    }

    if (!pairing_auth_decrypt(auth_.get(), buf.data(), buf.size(),
                              outbuf.data(), &outsize)) {
...
static
PairingConnectionCtx* CreateConnection(PairingConnectionCtx::Role role,
                                       const uint8_t* pswd,
                                       size_t pswd_len,
                                       const PeerInfo& peer_info,
    ...
    return new PairingConnectionCtx(role, vec_pswd, peer_info,
                                    vec_x509_cert, vec_priv_key);
}
...
PairingConnectionCtx* pairing_connection_client_new(const uint8_t* pswd,
                                                    ...
                                                    const uint8_t* priv_key_pem,
                                                    size_t priv_size) {
    return CreateConnection(PairingConnectionCtx::Role::Client,
                            pswd,
                            pswd_len,
                            peer_info,
                            x509_cert_pem,
...
PairingConnectionCtx* pairing_connection_server_new(const uint8_t* pswd,
                                                    ...
                                                    const uint8_t* priv_key_pem,
                                                    size_t priv_size) {
    return CreateConnection(PairingConnectionCtx::Role::Server,
                            pswd,
                            pswd_len,
                            peer_info,
                            x509_cert_pem,
