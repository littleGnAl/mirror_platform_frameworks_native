...
// Initializes the KeyStoreCtx. If failed to create key store, null is returned.
// This can happen if, on Android, the data partition is not mounted, or we
// don't have sufficient permissions.
KeyStoreCtx keystore_new_ctx(const char* keystore_path);

// Deletes the KeyStoreCtx.
