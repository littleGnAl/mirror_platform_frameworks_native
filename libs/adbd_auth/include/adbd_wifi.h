...
// Let framework know that a paired device has connected.
uint64_t adbd_wifi_notify_connected(AdbdWifiContext* ctx, const char* guid, size_t len);

// Let framework know that a paired device has disconnected.
void adbd_wifi_notify_disconnected(AdbdWifiContext* ctx, uint64_t id);

// Let the framework know that the TLS server has connected.
void adbd_wifi_notify_server_connected(AdbdWifiContext* ctx, int port);

// Let the framework know that the TLS server has disconnected.
