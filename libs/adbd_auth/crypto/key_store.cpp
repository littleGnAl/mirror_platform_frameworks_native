...
bool KeyStore::readPublicKeys(adbwifi::proto::KeyStore& key_store) {
    ...
    if (key_store.keystore_version() != kKeyStoreVersion) {
        // TODO: Can we to version migration here instead?
        LOG(ERROR) << "adbwifi keystore mismatch. Deleting the keystore.";
        my_unlink(store_name.c_str());
        return false;
    }
...
bool KeyStore::writePublicKeys() {
    ...
    // Write out the keystore version
    key_store.set_keystore_version(kKeyStoreVersion);
