...
public:
    ...
    void NotifyWifiDeviceConnected(std::string_view guid) EXCLUDES(mutex_) {
        std::lock_guard<std::mutex> lock(mutex_);
        output_queue_.emplace_back(AdbdAuthPacketWifiDeviceConnected{.guid = std::string(guid)});
        Interrupt();
    }

    void NotifyWifiDeviceDisconnected(std::string_view guid) EXCLUDES(mutex_) {
        std::lock_guard<std::mutex> lock(mutex_);
        output_queue_.emplace_back(AdbdAuthPacketWifiDeviceDisconnected{.guid = std::string(guid)});
        Interrupt();
    }
...
void adbd_auth_wifi_device_connected(AdbdAuthContext* ctx, const char* guid, size_t len) {
    ctx->NotifyWifiDeviceConnected(std::string_view(guid, len));
}

void adbd_auth_wifi_device_disconnected(AdbdAuthContext* ctx, const char* guid, size_t len) {
    ctx->NotifyWifiDeviceDisconnected(std::string_view(guid, len));
}
