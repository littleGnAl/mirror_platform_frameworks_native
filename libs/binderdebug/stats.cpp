...
extern "C" int main() {
    ...
    for (const String16& name : defaultServiceManager()->listServices()) {
        ...
        CHECK_EQ(OK, getBinderClientPids(BinderDebugContext::BINDER, getpid(), pid, *handle, &clientPids));

        printf("%s,%" PRIu32 ",%" PRIu32 ",%zu\n", String8(name).c_str(), info.threadUsage, info.threadCount, clientPids.size());
    }
    return 0;
}
