...
TEST(SensorTest, FlattenAndUnflattenUnalignedBuffer) {
    sensor_t hwSensor = getTestSensorT();
    Sensor sensor1(&hwSensor, SENSORS_DEVICE_API_VERSION_1_4);
    Sensor sensor2;

    std::vector<uint8_t> buffer(sensor1.getFlattenedSize() + 1);
    ASSERT_EQ(OK, sensor1.flatten(buffer.data() + 1, buffer.size() - 1));
    ASSERT_EQ(OK, sensor2.unflatten(buffer.data() + 1, buffer.size() - 1));
