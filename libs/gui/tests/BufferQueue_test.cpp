...
TEST_F(BufferQueueTest, TestConsumerDetachProducerListener) {
    ...
    ASSERT_EQ(OK, mProducer->setMaxDequeuedBufferCount(2));
    do {
        // some platform returns IGraphicBufferProducer::RELEASE_ALL_BUFFERS
        result = mProducer->dequeueBuffer(&slots[0], &fence, 0, 0, 0, 0, nullptr, nullptr);
    } while (result != IGraphicBufferProducer::RELEASE_ALL_BUFFERS);
    ASSERT_EQ(IGraphicBufferProducer::BUFFER_NEEDS_REALLOCATION, result);
    ASSERT_EQ(OK, mProducer->requestBuffer(slots[0], &buffer));

    result = mProducer->dequeueBuffer(&slots[1], &fence, 0, 0, 0, 0, nullptr, nullptr);
    ASSERT_EQ(IGraphicBufferProducer::BUFFER_NEEDS_REALLOCATION, result);
