...
TEST_F(BufferQueueTest, TestConsumerDetachProducerListener) {
    ...
    ASSERT_EQ(OK, mProducer->setMaxDequeuedBufferCount(2));
    for (size_t i = 0; i < 2; ++i) {
        result = mProducer->dequeueBuffer(&slots[i], &fence, 0, 0, 0, 0, nullptr, nullptr);
        if (i == 0) {
            // x86_64 returns RELEASE_ALL_BUFFERS initially.
            bool allReleased = (result & IGraphicBufferProducer::RELEASE_ALL_BUFFERS);
            bool needsRealloc = (result & IGraphicBufferProducer::BUFFER_NEEDS_REALLOCATION);
            ASSERT_TRUE(allReleased || needsRealloc);
        } else {
            ASSERT_EQ(IGraphicBufferProducer::BUFFER_NEEDS_REALLOCATION, result);
        }
        ASSERT_EQ(OK, mProducer->requestBuffer(slots[i], &buffer));
    ...
    int slot;
    result = mProducer->dequeueBuffer(&slot, &fence, 0, 0, 0, 0, nullptr, nullptr);
    ASSERT_EQ(IGraphicBufferProducer::BUFFER_NEEDS_REALLOCATION, result);
    ASSERT_EQ(OK, mProducer->requestBuffer(slot, &buffer));
