...
void Surface::ProducerListenerProxy::onBufferDetached(int slot) {
    sp<Surface> parent = mParent.promote();
    Mutex::Autolock lock(parent->mMutex);

    if (!parent->mIsDequeued[slot]) {
        if (parent->mReportRemovedBuffers && (parent->mSlots[slot].buffer != nullptr)) {
            parent->mRemovedBuffers.push_back(parent->mSlots[slot].buffer);
        }
        parent->mSlots[slot].buffer = nullptr;
    }
}
