...
int Surface::queueBuffer(android_native_buffer_t* buffer, int fenceFd) {
    ...
    if (mDirtyRegion.bounds() == Rect::INVALID_RECT) {
        ...
        // The surface damage was specified using the OpenGL ES convention of the
        // origin being in the bottom-left corner. Here we flip to the convention
        // that the rest of the system uses (top-left corner) by subtracting all
        // top/bottom coordinates from the buffer height.
...
void Surface::setSurfaceDamage(int* rects, int numRects) {
    ...
    for (int r = 0; r < numRects; ++r) {
        int* currentRect = rects + r * 4;
        int x = currentRect[0];
        int y = currentRect[1];
        int width = currentRect[2];
        int height = currentRect[3];
