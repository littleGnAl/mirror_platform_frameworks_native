...
static const nsecs_t WAITING_FOR_VSYNC_TIMEOUT = ms2ns(300);
...
int DisplayEventDispatcher::handleEvent(int, int events, void*) {
    ...
    if (mWaitingForVsync) {
        ...
        nsecs_t vsyncScheduleTime = currentTime - mLastScheduleVsyncTime;
        if (vsyncScheduleTime > WAITING_FOR_VSYNC_TIMEOUT) {
            ALOGW("Vsync time out! vsyncScheduleTime=%" PRId64 "ms", ns2ms(vsyncScheduleTime));
            mWaitingForVsync = false;
            dispatchVsync(currentTime, 0 /* displayId is not used */, ++mLastVsyncCount);
        }
    }
