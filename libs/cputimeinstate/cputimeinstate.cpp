...
static bool initGlobals() {
    ...
    uint32_t zero = 0;
    pid_t zeroPid = 0;
    if (writeToMapEntry(trackedPidMapFd, &zero, &zeroPid, BPF_ANY) != 0) return false;
...
bool startTrackingProcessCpuTimes(pid_t pid) {
    ...
    unique_fd trackedPidHashMapFd(mapRetrieveWO(BPF_FS_PATH "map_time_in_state_pid_tracked_hash_map"));
