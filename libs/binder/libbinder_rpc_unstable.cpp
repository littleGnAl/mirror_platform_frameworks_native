...
RpcSession::FileDescriptorTransportMode toTransportMode(
    ...
    switch (mode) {
        ...
        default:
            LOG(ERROR) << "Unknown transport mode: " << mode;
            return RpcSession::FileDescriptorTransportMode::NONE;
    }
}
...
void ARpcSession_setMaxIncomingThreads(ARpcSession* handle, unsigned int threads) {
    auto session = handleToStrongPointer<RpcSession>(handle);
    session->setMaxIncomingThreads(threads);
}

void ARpcSession_setMaxOutgoingThreads(ARpcSession* handle, unsigned int threads) {
    auto session = handleToStrongPointer<RpcSession>(handle);
