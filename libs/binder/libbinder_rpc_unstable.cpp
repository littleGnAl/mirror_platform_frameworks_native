...
ARpcServer* ARpcServer_setupVsock(AIBinder* service, unsigned int cid, unsigned int port) {
    auto server = RpcServer::make();
...
ARpcServer* ARpcServer_setupInitUnixDomain(AIBinder* service, const char* name) {
    auto server = RpcServer::make();
