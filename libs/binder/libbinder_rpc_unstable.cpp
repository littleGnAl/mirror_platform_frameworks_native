...
ARpcServer* ARpcServer_newVsock(AIBinder* service, unsigned int cid, unsigned int port) {
    ...
    unsigned int bind_cid = VMADDR_CID_ANY; // bind to the remote interface
    if (cid == VMADDR_CID_LOCAL) {
        bind_cid = VMADDR_CID_LOCAL; // bind to the local interface
        cid = VMADDR_CID_ANY;        // no need for a connection filter
    }

    if (status_t status = server->setupVsockServer(bind_cid, port); status != OK) {
        LOG(ERROR) << "Failed to set up vsock server with port " << port
                   << " error: " << statusToString(status).c_str();
        return nullptr;
    }
    if (cid != VMADDR_CID_ANY) {
        server->setConnectionFilter([=](const void* addr, size_t addrlen) {
            return accept_cid == cidFromStructAddr(addr, addrlen);
        });
    }
