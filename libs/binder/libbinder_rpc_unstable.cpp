...
static android::sp<RpcServer> findInstance(RpcServerHandle handle) {
    return android::sp<RpcServer>::fromExisting(reinterpret_cast<RpcServer*>(handle));
}

static RpcServerHandle runRpcServer(android::sp<RpcServer> server, AIBinder* service) {
    ...
    auto handle = reinterpret_cast<RpcServerHandle>(server.get());
    {
        ...
        LOG_ALWAYS_FATAL_IF(!success.second);
    }
    return handle;
}
...
RpcServerHandle RunVsockRpcServer(AIBinder* service, unsigned int port) {
    ...
    return runRpcServer(std::move(server), service);
}
...
void JoinRpcServer(RpcServerHandle handle) {
    LOG(ERROR) << "JOIN RPCSERVER WITH HANDLE " << handle;
    findInstance(handle)->join();
}

void ShutdownRpcServer(RpcServerHandle handle) {
    LOG(ERROR) << "DROP RPCSERVER WITH HANDLE " << handle;
    findInstance(handle)->shutdown();
    {
        android::RpcMutexLockGuard _l(sInstancesLock);
