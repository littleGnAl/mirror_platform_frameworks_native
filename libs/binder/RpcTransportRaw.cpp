...
status_t RpcTransportRaw::pollRead(void) {
    ...
    if (ret < 0) {
        ...
        if (savedErrno == EAGAIN) {
            // There's no status_t enumerator for -EAGAIN,
            // but that code is equivalent to WOULD_BLOCK anyway
            return WOULD_BLOCK;
        }
        return -savedErrno;
    }
...
status_t RpcTransportRaw::interruptableReadOrWrite(
    ...
    while (true) {
        ssize_t processSize = TEMP_FAILURE_RETRY(sendOrReceiveFun(mSocket.get(), iovs, niovs));
        ...
        if (altPoll) {
            ...
            if (status_t status = fdTrigger->triggerablePoll(mSocket.get(), event); status != OK)
                return status;
...
// RpcTransportCtx for files.
class RpcTransportCtxRawFile : public RpcTransportCtx {
public:
    std::unique_ptr<RpcTransport> newTransport(android::base::unique_fd fd, FdTrigger*) const {
        return std::make_unique<RpcTransportRawFile>(std::move(fd));
    }
    std::vector<uint8_t> getCertificate(RpcCertificateFormat) const override { return {}; }
};

std::unique_ptr<RpcTransportCtx> RpcTransportCtxFactoryRawFile::newServerCtx() const {
    return std::make_unique<RpcTransportCtxRawFile>();
}

std::unique_ptr<RpcTransportCtx> RpcTransportCtxFactoryRawFile::newClientCtx() const {
    return std::make_unique<RpcTransportCtxRawFile>();
}

const char* RpcTransportCtxFactoryRawFile::toCString() const {
    return "file";
}

std::unique_ptr<RpcTransportCtxFactory> RpcTransportCtxFactoryRawFile::make() {
    return std::unique_ptr<RpcTransportCtxFactoryRawFile>(new RpcTransportCtxFactoryRawFile());
}
