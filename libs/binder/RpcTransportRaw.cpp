...
public:
    ...
    status_t interruptableWriteFully(FdTrigger* fdTrigger, iovec* iovs, int niovs,
        ...
        auto writeFn = [](int fd, iovec* iovs, size_t niovs, int flags) {
            msghdr msg{
                    .msg_iov = iovs,
        ...
        return interruptableReadOrWrite(mSocket, fdTrigger, iovs, niovs, writeFn, "sendmsg",
                                        POLLOUT, altPoll);
    }

    status_t interruptableReadFully(FdTrigger* fdTrigger, iovec* iovs, int niovs,
                                    const std::function<status_t()>& altPoll) override {
        auto readFn = [](int fd, iovec* iovs, size_t niovs, int flags) {
            msghdr msg{
                    .msg_iov = iovs,
                    // posix uses int, glibc uses size_t.  niovs is a
                    // non-negative int and can be cast to either.
        ...
        return interruptableReadOrWrite(mSocket, fdTrigger, iovs, niovs, readFn, "recvmsg", POLLIN,
                                        altPoll);
    }
