...
public:
    explicit RpcTransportRaw(android::FdState socket) : mSocket(std::move(socket)) {}
    ...
    virtual bool isWaiting() { return mSocket.getPollingState(); }
...
public:
    std::unique_ptr<RpcTransport> newTransport(android::FdState fd, FdTrigger*) const {
