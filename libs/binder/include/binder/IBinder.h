...
public:
     ...
     *
     * This should only be called once during set up in the main thread. It is
     * not thread safe.
     */
     ...
     * 2. spawns a new thread (if not exceeding RPC thread count)
     * 3. polls transactions from the owned fd in step 2
     * 4. on DEAD_OBJECT (See RpcState), thread exits.
     *
