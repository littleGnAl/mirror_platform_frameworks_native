...
public:
    ...
    // replacement of ::send(). Error code may not be set if TLS is enabled.
    virtual android::base::Result<size_t> send(const void *buf, size_t size) = 0;

    // replacement of ::recv(). Error code may not be set if TLS is enabled.
     ...
     */
    status_t interruptableWriteFully(FdTrigger *fdTrigger, const void *buf, size_t size);
    status_t interruptableReadFully(FdTrigger *fdTrigger, void *buf, size_t size);

protected:
    RpcTransport() = default;

    virtual status_t triggerablePoll(FdTrigger *fdTrigger, int16_t event) = 0;
};
