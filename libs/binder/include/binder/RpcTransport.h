...
struct FdState {
    ...
    FdState() : fd(-1), isPolling(false) {}

    FdState(int _fd) : fd(_fd), isPolling(false) {}

    FdState(base::unique_fd &&_fd) : fd(std::move(_fd)), isPolling(false) {}

    FdState(FdState &&_fdState) noexcept
          : fd(std::move(_fdState.fd)), isPolling(_fdState.isPolling) {}

    FdState &operator=(FdState &&fdState) noexcept {
