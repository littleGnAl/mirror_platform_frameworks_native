...
public:
    ...
    explicit RpcSocket(base::unique_fd &&_fd) : pollingCount(0), fd(std::move(_fd)) {}

    RpcSocket(RpcSocket &&_socket) noexcept
          : pollingCount(_socket.pollingCount.load()), fd(std::move(_socket.fd)) {}
    ...
    void incPolling() const { ++pollingCount; }
    void decPolling() const { --pollingCount; }
};
