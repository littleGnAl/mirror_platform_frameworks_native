...
public:
    ...
    sp<IBinder> lookupOrCreateProxy(const sp<RpcConnection>& connection, RpcAddress&& address);
...
private:
    ...
    status_t processTransact(const base::unique_fd& fd, const RpcCommand& command);
    status_t processDecRef(const base::unique_fd& fd, const RpcCommand& command);
    ...
    std::map<RpcAddress, wp<IBinder>> mBinderForAddress;
