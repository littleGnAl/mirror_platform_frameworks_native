...
public:
    ...
    static BpBinder*    create(const sp<RpcConnection>& connection, const RpcWireAddress* address);

    // FIXME: is bool the best type here for the future?
    bool                isRpcBinder() const;
    const RpcWireAddress*   address() const;
...
private:
    explicit            BpBinder(int32_t handle);
                        ...
                        BpBinder(const sp<RpcConnection>& connection, const RpcWireAddress* address);
    ...
    const   RpcWireAddress*     mAddress; // if mIsSocket
