...
public:
    ...
    status_t            readEnumVector(std::vector<T>* val) const
            { return readData(val); }
    template<typename T, std::enable_if_t<std::is_enum_v<T> && std::is_same_v<typename std::underlying_type_t<T>,int8_t>, bool> = 0>
    status_t            readEnumVect(std::unique_ptr<std::vector<T>>* val) const __attribute__((deprecated("use std::optional version instead")))
            { return readData(val); }
    template<typename T, std::enable_if_t<std::is_enum_v<T> && std::is_same_v<typename std::underlying_type_t<T>,int8_t>, bool> = 0>
    ...
    // If strong binder is nullptr, readStrongBinder() returns an error.
    // TODO: T must be derived from IBinder, fix interface for clarity.
    template<typename T>
...
private:
    ...
    //
    // Our approach to parceling is based on two templated functions
    // readData() and writeData() that generate parceling code for an
    // object automatically based on its type. The code from the template is generated at
    ...
    //
    // This approach unifies handling of complex objects (as compared with many separate
    // non-templated functions), resulting in fewer lines of code, greater consistency,
    ...
    //
    // 1) Alignment
    // Regardless of the parameter type, writeData() calls are designed to finish at a
    // multiple of 4 bytes, the default alignment of the Parcel.
    //
    ...
    // Since this is already part of the stable protocol, an update to the AIDL interface
    // would be needed to change to packed delivery.
    //
    ...
    //
    // Example #1
    // Parceling of a 3D vector (not currently emitted by AIDL)
    //
    ...
    //
    // Example #2
    // Parceling of mixed shared pointers (not currently emitted by AIDL)
    //
    // Parcel p1;
    ...
    // In the future, we cold use int16_t and uint16_t as new AIDL types.
    static_assert(!std::is_same_v<char16_t, int16_t>);
    ...
    std::enable_if_t<std::is_arithmetic_v<T>
            ...
            || std::is_base_of_v<Parcelable, T>
            || std::is_enum_v<T>,
    status_t> writeData(const T& t) {
        // TODO: inline the actual code here for better performance and reduced code line count.
        if constexpr (std::is_same_v<T, bool>) {
            ...
            status_t status = writeData(static_cast<int32_t>(1));
    ...
    //  --- recursive descent -- (calls into writeData above).

    // templated objects handled here
    template <template <typename ...> class V, typename T, typename... Args>
    status_t writeData(const V<T, Args...>& v) {
        using VT = V<T, Args...>;
        if constexpr (is_specialization_v<VT, std::optional>
                || is_specialization_v<VT, std::unique_ptr>
                || is_specialization_v<VT, std::shared_ptr>) {
            if constexpr (is_specialization_v<T, std::vector>
                    || std::is_same_v<T, String16>
                    || std::is_same_v<T, std::string>) {
                if (!v) return writeData(static_cast<int32_t>(-1));
            } else if constexpr (std::is_base_of_v<Parcelable, T>) {
                if (!v) return writeData(static_cast<int32_t>(0));
            } else /* constexpr */ {  // could define this, but raise as error.
                static_assert(dependent_false_v<VT>);
            }
            return writeData(*v);
        } else if constexpr (is_specialization_v<VT, std::vector>) {
            if (v.size() >  std::numeric_limits<int32_t>::max()) return BAD_VALUE;
            ...
            if constexpr (is_pointer_equivalent_array_v<T>) {
                ...
                if (v.size() > limit) return BAD_VALUE;
                ...
                auto data = reinterpret_cast<int32_t*>(writeInplace(v.size() * sizeof(int32_t)));
                if (data == nullptr) return BAD_VALUE;
                for (const auto d: v) {
                    *data++ = static_cast<int32_t>(d);
                }
            } else /* constexpr */ {
                for (const auto &d : v) {
                    const status_t status = writeData(d);
                    if (status != OK) return status;
                }
            }
            return OK;
        } else if constexpr (is_specialization_v<VT, sp> && std::is_base_of_v<IBinder, T>) {
            return writeStrongBinder(v);
        } else if constexpr (std::is_same_v<VT, std::string>) {
            return writeUtf8AsUtf16(v); // std::string
        } else if constexpr (std::is_same_v<VT, base::unique_fd>) {
            return writeUniqueFileDescriptor(v);
        } else /* constexpr */ {
            static_assert(dependent_false_v<VT>);
        }
    }
    ...
    status_t> readData(T* t) const {
        ...
        if constexpr (std::is_same_v<T, bool>) {
            ...
            if (status != OK) return status;
            if (present != 1) return UNEXPECTED_NULL;
    ...
    //  --- recursive descent -- (calls into readData above).

   // templated objects handled here
   // flags could incorporate AIDL annotations such as @Nullable
    enum ReadFlags {
        READ_FLAG_NONE = 0,
        READ_FLAG_SP_NULLABLE = 1 << 0,
    };
    template <template <typename ...> class V, typename T, typename... Args>
    status_t readData(V<T, Args...>* v, ReadFlags readFlags = READ_FLAG_NONE) const {
        using VT = V<T, Args...>;
        if constexpr (is_specialization_v<VT, std::optional>
                || is_specialization_v<VT, std::unique_ptr>
                || is_specialization_v<VT, std::shared_ptr>) {
            ...
            if constexpr (is_specialization_v<T, std::vector>) {
                return readData(&**v, READ_FLAG_SP_NULLABLE); // nullable sp<> allowed now
            } else {
                return readData(&**v);
            }
        } else if constexpr (is_specialization_v<VT, std::vector>) {
            ...
            v->clear(); // must clear before resizing/reserving otherwise move ctors may be called.
            if constexpr (is_pointer_equivalent_array_v<T>) {
                ...
                if (data == nullptr) return BAD_VALUE;
                v->insert(v->begin(), data, data + size); // insert should do a reserve().
            } else if constexpr (std::is_same_v<T, bool>
                    || std::is_same_v<T, char16_t>) {
                v->reserve(size); // avoids default initialization
                auto data = reinterpret_cast<const int32_t*>(
                        readInplace(static_cast<size_t>(size) * sizeof(int32_t)));
                if (data == nullptr) return BAD_VALUE;
                for (int32_t i = 0; i < size; ++i) {
                    v->emplace_back(static_cast<T>(*data++));
                }
            } else if constexpr (is_specialization_v<T, sp>) {
                v->resize(size); // calls ctor
            ...
            return readStrongBinder(v);  // Note: on null, returns failure
        } else if constexpr (std::is_same_v<VT, std::string>) {
            return readUtf8FromUtf16(v); // std::string
        } else if constexpr (std::is_same_v<VT, base::unique_fd>) {
            return readUniqueFileDescriptor(v);
        } else /* constexpr */ {
            static_assert(dependent_false_v<VT>);
        }
    }
