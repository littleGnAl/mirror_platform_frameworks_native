...
private:
    ...
    status_t writeData(const std::vector<T, Args...>& v) {
   ...
   // flags could incorporate AIDL annotations such as @Nullable
    ...
    template <template <typename ...> class V, typename T, typename... Args>
    status_t readData(V<T, Args...>* v) const {
        using VT = V<T, Args...>;
        if constexpr (is_specialization_v<VT, std::optional>
                || is_specialization_v<VT, std::unique_ptr>
                || is_specialization_v<VT, std::shared_ptr>) {
    ...
    // vector special case
    template <typename T, typename... Args>
    status_t readData(std::vector<T, Args...>* v, ReadFlags readFlags = READ_FLAG_NONE) const {
        int32_t size;
