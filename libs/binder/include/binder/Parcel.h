...
private:
    ...
    // writeData<T> / readData<T> handling, may call helper methods.
    template <typename T>
    ...
    status_t readData(T* t) const {
        ...
        if constexpr (std::is_same_v<T, bool>) {
            ...
            return readNullableData(t);
        } else if constexpr (is_specialization_v<T, std::vector>) {
            return readVectorData(t);
        } else if constexpr (is_specialization_v<T, sp>) {
    ...
    // Private helper methods - these avoid template overload resolution by enable_if.
    // --- recursive descent -- (may call back into writeData / readData above).
   ...
   // flags could incorporate AIDL annotations such as @Nullable
    ...
    template <typename T, typename... Args>
    status_t readVectorData(
            std::vector<T, Args...>* v, ReadFlags readFlags = READ_FLAG_NONE) const {
        int32_t size;
    ...
    status_t writeNullableData(const V<T, Args...>& v) {
        using VT = V<T, Args...>;
    ...
    template <template <typename ...> class V, typename T, typename... Args>
    status_t readNullableData(V<T, Args...>* v) const {
        using VT = V<T, Args...>;
        ...
        if constexpr (is_specialization_v<T, std::vector>) {
            return readVectorData(&**v, READ_FLAG_SP_NULLABLE); // nullable sp<> allowed now
        } else {
            return readData(&**v);
        }
    }
