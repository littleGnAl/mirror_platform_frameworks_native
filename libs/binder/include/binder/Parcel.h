...
private:
    ...
    // Determine if a type is a template class, some MyClass<T, ...>.
    template <typename Test>
    struct is_template_class : std::false_type {};

    template <template <typename...> class Ref, typename... Args>
    struct is_template_class<Ref<Args...>>: std::true_type {};

    template <typename Test>
    static inline constexpr bool is_template_class_v = is_template_class<Test>::value;
    ...
    template <typename T>
    std::enable_if_t<std::is_same_v<T, int8_t> || std::is_same_v<T, uint8_t>,
    status_t> writeData(const T& t) {
        return writeByte(t);  // this writes as int32_t
    }

    template <typename T>
    std::enable_if_t<std::is_same_v<T, char16_t>,
    status_t> writeData(const T& t) {
    ...
    template <typename T>
    std::enable_if_t<std::is_same_v<T, uint32_t>,
    status_t> writeData(const T& t) {
    ...
    template <typename T>
    std::enable_if_t<std::is_same_v<T, String16>,
    status_t> writeData(const T& t) {
        return writeString16(t);
    }

    template <typename T>
    std::enable_if_t<std::is_base_of_v<Parcelable, T>,
    status_t> writeData(const T& t) {
    ...
    template <typename T>
    std::enable_if_t<is_specialization_v<T, sp>,
    status_t> writeData(const T& t) {
    ...
    status_t> writeData(const T& t) {
        return writeUtf8AsUtf16(t); // std::string
    }

    template <typename T>
    std::enable_if_t<std::is_same_v<T, base::unique_fd>,
    status_t> writeData(const T& t) {
    ...
    std::enable_if_t<is_parcel_nullable_type_v<VT>,
    status_t> writeData(const VT& v) {
        using T = first_template_type_t<VT>;
        if constexpr (is_specialization_v<T, std::vector>
                || std::is_same_v<T, String16>
                || std::is_same_v<T, std::string>) {
            if (!v) return writeData(static_cast<int32_t>(kNullVectorSize));
        } else if constexpr (std::is_base_of_v<Parcelable, T>) {
            if (!v) return writeData(static_cast<int32_t>(kNullParcelableFlag));
    ...
    template <typename VT>
    std::enable_if_t<is_specialization_v<VT, std::vector>,
    status_t> writeData(const VT& v) {
        using T = first_template_type_t<VT>;
    ...
    std::enable_if_t<std::is_same_v<T, std::string>,
    status_t> readData(T* t) const {
        return readUtf8FromUtf16(t); // std::string
    }

    template <typename T>
    std::enable_if_t<std::is_same_v<T, base::unique_fd>,
    status_t> readData(T* t) const {
        return readUniqueFileDescriptor(t); // std::string
    }

    template <typename VT>
    std::enable_if_t<is_parcel_nullable_type_v<VT>,
    status_t> readData(VT* v) const {
        using T = first_template_type_t<VT>;
    ...
    // std::vector special case.
    // template <typename T, typename... Args>
    // status_t writeVectorData(const std::vector<T, Args...>& v) {
    ...
    template <typename VT>
    std::enable_if_t<is_specialization_v<VT, std::vector>,
    status_t> readData(VT* v, ReadFlags readFlags = READ_FLAG_NONE) const {
        using T = first_template_type_t<VT>;
        int32_t size;
