...
public:
    ...
    /**
     * If TLS is enabled, return the self-signed certificate generated by this
     * server. If TLS is disabled, return empty string.
     */
    std::string getX509Pem();

    /**
     * If TLS is enabled, add |cert| as the a client certificate trusted by
     * this server. Clients not presenting a certificate, or clients presenting
     * a non-trusted certificate, are rejected.
     *
     * If TLS is disabled, always return OK. All clients are trusted.
     */
    status_t addTrustedX509Pem(std::string_view cert);
