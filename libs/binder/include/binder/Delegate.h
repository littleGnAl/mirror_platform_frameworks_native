...
/*
 * Args needed by the make function to create the delegator and attach an
 * id to it to know that it's a delegator
 */
template <typename T>
struct MakeArgs {
    const sp<T>* binder;
    ...
    sp<IBinder> newDelegator = T::asBinder(binder)->lookupOrCreateWeak(
            ...
            [](const void* args) -> sp<IBinder> {
                auto delegator = sp<typename T::DefaultDelegator>::make(
                        *static_cast<const MakeArgs<T>*>(args)->binder);
                // make sure we know this binder is a delegator by attaching a unique ID
                (void)delegator->attachObject(static_cast<const MakeArgs<T>*>(args)->id,
