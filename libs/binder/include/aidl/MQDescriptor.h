...
struct MQDescriptor : public android::Parcelable {
    ...
    inline const std::vector<GrantorDescriptor> &grantors() const {
        return mGrantors;
    }

    inline std::vector<GrantorDescriptor> &grantors() {
        return mGrantors;
    }
    ...
    static inline size_t alignToWordBoundary(size_t length) {
        ...
        if (kAlignmentSize % __WORDSIZE != 0) {
            // TODO find a replacement for details::logAlwaysFatal
        }
...
MQDescriptor<T, flavor>::MQDescriptor() : MQDescriptor(
        std::vector<android::aidl::GrantorDescriptor>(),
        nullptr /* nHandle */,
        0 /* size */) {}
...
template<typename T, MQFlavor flavor>
status_t MQDescriptor<T, flavor>::writeToParcel(Parcel* parcel) const {
   status_t status = NO_ERROR;
   //TODO write vector of grantors
   status = parcel->writeNativeHandle(mHandle);
   status = parcel->writeInt32(mQuantum);
   status = parcel->writeInt32(mFlags);
   return status;
}

template<typename T, MQFlavor flavor>
status_t MQDescriptor<T, flavor>::readFromParcel(const Parcel* parcel) {
   status_t status = NO_ERROR;
   mFlags = parcel->readInt32();
   mQuantum = parcel->readInt32();
   mHandle = parcel->readNativeHandle();
   //TODO read vector of grantors
   return status;
}
