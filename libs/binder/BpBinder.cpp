...
BpBinder* BpBinder::create(const sp<RpcConnection>& connection, const RpcAddress& address) {
    ...
    LOG_ALWAYS_FATAL_IF(address == nullptr, "BpBinder::create null address");
...
{
    if (mIsSocket) return UNKNOWN_TRANSACTION;
...
{
    LOG_ALWAYS_FATAL_IF(mIsSocket, "Cannot send obituary for remote binder.");
...
BpBinder::~BpBinder()
{
    ...
    if (CC_UNLIKELY(mIsSocket)) return;
...
{
    if (CC_UNLIKELY(mIsSocket)) return true;
