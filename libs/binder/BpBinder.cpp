...
BpBinder* BpBinder::create(const sp<RpcConnection>& connection, const RpcWireAddress* address) {
...
BpBinder::BpBinder(const sp<RpcConnection>& connection, const RpcWireAddress* address)
    : BpBinder(0x7FFFFFFF)
{
...
const RpcWireAddress* BpBinder::address() const {
    return mAddress;
}
...
void BpBinder::onLastStrongRef(const void* /*id*/)
{
    ...
    if (CC_UNLIKELY(mIsSocket)) {
        // FIXME: this works, but specifically
        // - this is OBJECT_LIFETIME_WEAK, so this might be promoted again
        // - what about when this process dies and the connection is dropped?
        mConnection->sendDecStrong(mAddress);
        return;
    }
    IF_ALOGV() {
        printRefs();
