...
BufferedTextOutput::BufferedTextOutput(uint32_t flags)
      : mFlags(flags), mSeq(android_atomic_inc(&gSequence)), mIndex(allocBufferIndex()) {
...
void BufferedTextOutput::popBundle() {
    ...
    BufferState* b = getBuffer();
    b->bundle--;
    LOG_FATAL_IF(b->bundle < 0, "TextOutput::popBundle() called more times than pushBundle()");
