...
#include <android/debug/IAdbCallbackManager.h>
...
using android::debug::IAdbCallbackManager;
...
public:
    android::binder::Status debuggingChanged(bool enabled,
...
void exitOnAdbDebuggingDisabled() {
    auto adb = android::waitForService<IAdbCallbackManager>(
            String16(IAdbCallbackManager::descriptor + String16("/default")));
    CHECK(adb != nullptr) << "Unable to retrieve IAdbCallbackManager/default";
    auto status = adb->registerCallback(sp<AdbCallback>::make());
    CHECK(status.isOk()) << "Unable to call IAdbCallbackManager::registerCallback: " << status;
}
