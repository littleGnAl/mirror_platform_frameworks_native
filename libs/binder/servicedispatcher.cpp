...
int Dispatch(const char* name, const ServiceRetriever& serviceRetriever, const char* ip_address) {
    ...
    unsigned int port;
    const char* addr = ip_address == nullptr ? kLocalInetAddress : ip_address;
    if (status_t status = rpcServer->setupInetServer(addr, 0, &port); status != OK) {
...
int wrapServiceManager(const ServiceRetriever& serviceRetriever, const char* ip_address) {
    auto sm = defaultServiceManager();
    if (nullptr == sm) {
        LOG(ERROR) << "No servicemanager";
        return EX_SOFTWARE;
