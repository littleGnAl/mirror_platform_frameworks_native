...
status_t RpcState::rpcSend(const sp<RpcSession::RpcConnection>& connection,
    ...
    for (int i = 0; i < niovs; i++) {
        LOG_RPC_DETAIL("Sending %s[%d] on RpcTransport %p: %s", what, i, connection->rpcTransport.get(),
...
status_t RpcState::rpcRec(const sp<RpcSession::RpcConnection>& connection,
    ...
    for (int i = 0; i < niovs; i++) {
        LOG_RPC_DETAIL("Received %s[%d] on RpcTransport %p: %s", what, i, connection->rpcTransport.get(),
                       android::base::HexString(iovs[i].iov_base, iovs[i].iov_len).c_str());
    }
    return OK;
}
...
status_t RpcState::transactAddress(const sp<RpcSession::RpcConnection>& connection,
    ...
    if (status_t status =
                rpcSend(connection, session, "transaction", iovs, arraysize(iovs));
        status != OK) {
        // TODO(b/167966510): need to undo onBinderLeaving - we know the
        // refcount isn't successfully transferred.
        return status;
...
status_t RpcState::drainCommands(const sp<RpcSession::RpcConnection>& connection,
    ...
    while (true) {
        ...
        if (status != OK) {
            // TODO: we should return the error here,
            // but that breaks binderRpcTest
            break;
        }
        if (!num_bytes) break;
