...
status_t RpcState::transactAddress(const sp<RpcSession::RpcConnection>& connection,
    ...
    std::vector<base::borrowed_fd> objectFds;
    if (auto* rpcFields = data.maybeRpcFields()) {
        objectTable.reserve(rpcFields->mFds.size());
        for (const auto& fd : rpcFields->mFds) {
            // TODO: Check for overflow.
            objectTable.push_back(fd.dataPos);
            objectFds.emplace_back(std::visit([](const auto& x) { return x.get(); }, fd.fd));
        }
    }
    ...
    if (!objectFds.empty()) {
        ...
        if (status_t status = connection->rpcTransport->queueAncillarydata(objectFds);
            status != OK) {
            return status;
        }
    }
...
void RpcState::cleanupReplyData(Parcel* p, const uint8_t* data, size_t dataSize,
    ...
    const std::optional<uint32_t> version = rpcFields->mSession->getProtocolVersion();
    // TODO: Is this worth checking explicityl or is it OK to let the optional abort?
    LOG_ALWAYS_FATAL_IF(!version.has_value());

    if (*version >= RPC_WIRE_PROTOCOL_VERSION_RPC_HEADER_FEATURE_WIRE_REPLY_V1) {
        delete[] const_cast<uint8_t*>(data - offsetof(RpcWireReplyV1, data));
    } else {
        delete[] const_cast<uint8_t*>(data - offsetof(RpcWireReplyV0, data));
    }
...
status_t RpcState::waitForReply(const sp<RpcSession::RpcConnection>& connection,
    ...
    const auto genericRepyToParcel = [&](auto phantomWireHeader) -> status_t {
        ...
        if constexpr (!std::is_same_v<RpcWireReplyType, RpcWireReplyV0>) {
            ...
            std::optional<Span<uint32_t>> maybeSpan = parcelSpan.reinterpret<uint32_t>();
            if (!maybeSpan.has_value()) {
                ALOGE("Bad object table size in RpcWireReply. Saw bodySize=%" PRId32
