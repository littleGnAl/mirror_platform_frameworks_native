...
status_t RpcState::sendDecStrong(const base::unique_fd& fd, const RpcAddress& addr) {
    // FIXME: checks
    bool removed = mNodeForAddress.erase(addr) > 0;
    LOG_ALWAYS_FATAL_IF(!removed, "Dec strong on unknown address %s", rwaToString(addr).c_str());
...
status_t RpcState::processDecRef(const base::unique_fd& fd,
    ...
    if (target->localBinder() == nullptr) {
        ALOGE("Can only process dec ref on local binder, not a locally held proxy %s", rwaToString(*address).c_str());
        return UNKNOWN_ERROR;
    }
    if (target == nullptr) {
