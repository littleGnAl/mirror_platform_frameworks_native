...
status_t RpcState::onBinderLeaving(const sp<RpcConnection>& connection,
    ...
    // FIXME: move address out of BpBinder, and keep binder->address map in
    // RpcState
...

    {
        ...
        for (auto& [address, node] : mNodeForAddress) {
            ...
            LOG_ALWAYS_FATAL_IF(binder == nullptr,
                                "Binder %p expected to be owned.", binder.get());
...
status_t RpcState::transact(const base::unique_fd& fd,
    ...
    if (!address.isZero()) {
        ...
        LOG_ALWAYS_FATAL_IF(it == mNodeForAddress.end(),
                            "Sending transact on unknown address %s", address.toString().c_str());
...
status_t RpcState::processTransactInternal(const base::unique_fd& fd,
    ...
    // FIXME: heap allocation just for lookup in mNodeForAddress
        ...
        if (it == mNodeForAddress.end()) {
            ...
            if (target == nullptr) {
                ...
                if (transaction->asyncNumber != it->second.asyncNumber) {
                    ...
                    // first
                    // FIXME: limit enqueues/detect overfill for bad client
                    // FIXME: detect when an object is deleted when it still has
                    //        pending async transactions
                    it->second.asyncTodo.push(BinderNode::AsyncTodo {
                        .data = std::move(transactionData),
                        .asyncNumber = transaction->asyncNumber,
                    });
                    LOG_RPC_DETAIL("Enqueuing %" PRId64 " on %s",
    ...
    if (transaction->flags & IBinder::FLAG_ONEWAY) {
        ...
        LOG_RPC_DETAIL("Processed async transaction %" PRId64 " on %s",
                       transaction->asyncNumber, addr.toString().c_str());
        ...
        // transactions.
        {
            ...
            // FIXME: support > 2**64 async transactions
...
status_t RpcState::processDecStrong(const base::unique_fd& fd,
    ...
    LOG_ALWAYS_FATAL_IF(it->second.sentRef == nullptr,
                        "Inconsistent state, lost ref for %s",
                        addr.toString().c_str());
