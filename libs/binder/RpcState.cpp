...
status_t RpcState::transactAddress(const sp<RpcSession::RpcConnection>& connection,
    ...
    RpcWireHeader command{
            ...
            .bodySize = static_cast<uint32_t>(sizeof(RpcWireTransaction) + data.dataSize() + objectTable.size()*sizeof(uint32_t)),
    };
    ...
    RpcWireTransaction transaction{
            ...
            // TODO: Check for overflow.
            .objectTableOffset = static_cast<uint32_t>(data.dataSize()),
    };
...
status_t RpcState::waitForReply(const sp<RpcSession::RpcConnection>& connection,
    ...
    size_t parcelDataSize = rpcReply->objectTableOffset;
    uint8_t* objectTableBytes = rpcReply->data + rpcReply->objectTableOffset;
    ...
    data.release();
    reply->rpcSetDataReference(session, parcelData, parcelDataSize, objectTable, objectTableSize, std::move(pending_fds), cleanup_reply_data);
...
processTransactInternalTailCall:
    ...
    if (replyStatus == OK) {
        ...
        size_t parcelDataSize = transaction->objectTableOffset;
        uint8_t* objectTableBytes = transaction->data + transaction->objectTableOffset;
        size_t objectTableBytesSize = transactionData.size() - offsetof(RpcWireTransaction, data) - parcelDataSize;
        uint32_t* objectTable = reinterpret_cast<uint32_t*>(objectTableBytes);
        size_t objectTableSize = objectTableBytesSize / sizeof(uint32_t);
    ...
    RpcWireHeader cmdReply{
            ...
            .bodySize = static_cast<uint32_t>(sizeof(RpcWireReply) + reply.dataSize() + objectTable.size() * sizeof(uint32_t)),
    };
    RpcWireReply rpcReply{
            .status = replyStatus,
            // TODO: Check for overflow.
            .objectTableOffset = static_cast<uint32_t>(reply.dataSize()),
    };
