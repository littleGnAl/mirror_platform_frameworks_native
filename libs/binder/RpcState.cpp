...
sp<IBinder> RpcState::onBinderEntering(const sp<RpcConnection>& connection,
    ...
    if (it != mNodeForAddress.end()) {
        ...
        // Every proxy send is associated with a strong ref, but we don't
        // actually need to hold onto all of these, since we already have the
        // RefBase strong count for this object to use for counting them here.
        // Once that is depleted, we will actually send the final decStrong
        // from BpBinder::onLastDecStrong.
...
void RpcState::terminate() {
    ...
    for (auto& [address, node] : mNodeForAddress) {
        ...
        LOG_RPC_DETAIL("RpcState::terminate binder %p has %zu strong", binder.get(), node.strong);

        // FIXME: this assumes some logic in onBinderLeaving, since we only
        // support passing remote binders associated with its existing
        // connection. This is dangerous since it's not always true, and memory
        // is complicated. Instead, move this logic into the node object itself.
        if (binder->remoteBinder()) continue;

        // FIXME: if the destructor of a binder object makes another RPC call,
        // then this could deadlock.
        for (; node.strong > 0; node.strong--) {
...
status_t RpcState::processDecStrong(const base::unique_fd& fd,
    ...
    // FIXME: make sure this is a local binder
