...
sp<IBinder> RpcState::lookupOrCreateProxy(const sp<RpcConnection>& connection, RpcAddress&& address) {
...
const RpcAddress* RpcState::attachBinder(const sp<IBinder>& binder) {
    ...
    RpcAddress address = RpcAddress{(int)mBinderForAddress.size()};
    auto it = mBinderForAddress.insert({address, binder});
...
status_t RpcState::processTransact(const base::unique_fd& fd,
    ...
    auto it = mBinderForAddress.find(RpcAddress{transaction->address});
    if (it == mBinderForAddress.end()) {
    ...
    if (target == nullptr) {
        ALOGE("Binder has been deleted at address %d", transaction->address); // FIXME: combine with L248? or abstract lookup away
        // FIXME: add RpcCommand reply of type 'ERROR' so client can get a nicer
        // error?
        return UNKNOWN_ERROR;
    }
...
status_t RpcState::processDecRef(const base::unique_fd& fd,
    ...
    // FIXME: check bodySize < sizeof(RpcAddress)
    RpcAddress* address = reinterpret_cast<RpcAddress*>(commandData.data());

    // FIXME: this is pasted from transact
    auto bit = mBinderForAddress.find(*address);
    if (bit == mBinderForAddress.end()) {
        ALOGE("Unknown binder address %d", address->address);
        return UNKNOWN_ERROR;
    }
    sp<IBinder> target = bit->second.promote();
    if (target == nullptr) {
        ALOGE("Binder has been deleted at address %d", address->address); // FIXME: combine with L248? or abstract lookup away
        // FIXME: add RpcCommand reply of type 'ERROR' so client can get a nicer
        // error?
        return UNKNOWN_ERROR;
    }
    ...
    if (rit == mExternalStrongRefs.end()) {
        ...
        ALOGE("Requested dec ref for binder, but we have no record it is owned: %d", address->address);
        return UNKNOWN_ERROR;
    }
