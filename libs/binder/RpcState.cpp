...
RpcState::CommandData::CommandData(size_t size) : mSize(size) {
    ...
    // larger than a page, but we need to avoid allocated too large amount
    // of data from another service, or we can easily be requested to
    // allocate too much.
    //
    ...
    constexpr size_t kMaxTransactionAllocation = 100 * 1000 * 1000;
    if (size == 0) return;
