...
status_t RpcState::waitForReply(const sp<RpcSession::RpcConnection>& connection,
    ...
    const size_t headerSize = RpcWireReply::wireSize(session->getProtocolVersion().value());

    if (command.bodySize < headerSize) {
    ...
    CommandData data(command.bodySize - headerSize);
    ...
    // NOTE: `parcelDataSize` will always be 0 for protocol version 0.
...
processTransactInternalTailCall:
    ...
    uint32_t bodySize = 0;
    LOG_ALWAYS_FATAL_IF(__builtin_add_overflow(headerSize, reply.dataSize(), &bodySize) ||
                                __builtin_add_overflow(objectTableSpan.byteSize(), bodySize,
                                                       &bodySize),
