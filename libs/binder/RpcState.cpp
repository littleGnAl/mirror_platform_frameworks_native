...
status_t RpcState::sendDecStrongToTarget(const sp<RpcSession::RpcConnection>& connection,
...

    {
        ...
        LOG_ALWAYS_FATAL_IF(nullptr != tryEraseNode(std::move(_l), it),
                            "Bad state. RpcState shouldn't own received binder");
        // LOCK ALREADY RELEASED
        if (mTerminated) session->shutdownAndWait(false);
    }
...
status_t RpcState::processDecStrong(const sp<RpcSession::RpcConnection>& connection,
    ...
    sp<IBinder> tempHold = tryEraseNode(std::move(_l), it);
    // LOCK ALREADY RELEASED
...
sp<IBinder> RpcState::tryEraseNode(RpcMutexUniqueLock nodeLock,
    ...
    if (shouldShutdown) {
        ALOGI("RpcState is empty of binder, so triggering shutdown...");
