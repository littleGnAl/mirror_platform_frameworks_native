...
RpcAddress rwaNew() {
    ...
    ReadRandomBytes((uint8_t*)addr.get(), sizeof(RpcWireAddress));
...
status_t RpcState::onBinderLeaving(const sp<RpcConnection>& connection,
    ...
    auto it = mNodeForAddress.insert({rwaNew(), BinderNode{binder,1}});
...
static inline bool rpcRec(const base::unique_fd& fd, const char* what, void* data, size_t size) {
    ...
    if (recd != (ssize_t) size) {
        ...
        // FIXME: spam
        ALOGI("Received %s on fd %d: %s", what, fd.get(), hexString(data, size).c_str());
    }
...
status_t RpcState::sendDecStrong(const base::unique_fd& fd, const RpcAddress& addr) {
    {
        ...
        LOG_ALWAYS_FATAL_IF(it == mNodeForAddress.end(),
                            "Sending dec strong on unknown address %s", rwaToString(addr).c_str());
    ...
    if (memcmp(addr.get(), &ZERO, sizeof(RpcWireAddress)) != 0) {
        ...
        if (target == nullptr) {
            ...
            ALOGE("Binder has been deleted at address %s",
                  rwaToString(transaction->address).c_str());
    ...
    if (target) {
        ...
        switch(transaction->code) {
            case RPC_SPECIAL_TRANSACT_GET_ROOT: {
                ...
                ALOGE("Write status: %s, reply size: %zu",
                      statusToString(status).c_str(), reply.dataSize());
                break;
            }
            default: {
...
status_t RpcState::processDecStrong(const base::unique_fd& fd,
    ...
    if (target == nullptr) {
        // FIXME: combine with L248? or abstract lookup away
        ALOGE("Binder has been deleted at address %s", rwaToString(*address).c_str());
        // FIXME: add RpcWireHeader reply of type 'ERROR' so client can get a nicer
        // error?
        return UNKNOWN_ERROR;
    }
