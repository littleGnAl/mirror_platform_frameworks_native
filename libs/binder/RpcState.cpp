...
status_t RpcState::processTransact(const base::unique_fd& fd,
    ...
    if (command.bodySize < sizeof(RpcWireTransaction)) {
        ALOGE("Expecting %zu but got %" PRId32 " bytes for RpcWireTransaction. Terminating!",
    ...
    if (replyStatus == OK) {
        if (target) {
            sp<IBinder> local = target->localBinder();
            if (local) {
                replyStatus = local->transact(transaction->code, data, &reply, transaction->flags);
            } else {
