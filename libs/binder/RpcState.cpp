...
status_t RpcState::transact(const sp<RpcSession::RpcConnection>& connection,
    ...
    if (protocolVersion < RPC_WIRE_PROTOCOL_VERSION_RPC_HEADER_FEATURE_EXPLICIT_PARCEL_SIZE &&
        ...
        ALOGE("Parcel at %p has attached objects but the sessions protocol version (%" PRIu32
              ") is too old, requires %" PRIu32,
...
status_t RpcState::waitForReply(const sp<RpcSession::RpcConnection>& connection,
    ...
    reply->rpcSetDataReference(session, parcelSpan.data, parcelSpan.size, objectTableSpan.data,
                               objectTableSpan.size, std::move(pending_fds), cleanup_reply_data);
    return OK;
}
...
processTransactInternalTailCall:
    ...
    LOG_ALWAYS_FATAL_IF(rpcFields == nullptr);
    const uint32_t protocolVersion = session->getProtocolVersion().value();

    if (protocolVersion < RPC_WIRE_PROTOCOL_VERSION_RPC_HEADER_FEATURE_EXPLICIT_PARCEL_SIZE &&
        !rpcFields->mObjectPositions.empty()) {
        ALOGE("Reply Parcel at %p has attached objects but the sessions protocol version (%" PRIu32
              ") is too old, requires %" PRIu32,
        ...
        reply.freeData();
        reply.markForRpc(session);
        rpcFields = reply.maybeRpcFields();
        replyStatus = BAD_TYPE;
    }

    const size_t rpcReplyWireSize = RpcWireReply::wireSize(protocolVersion);
