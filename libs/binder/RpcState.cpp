...
status_t RpcState::rpcSend(const sp<RpcSession::RpcConnection>& connection,
                           ...
                           RpcTransport::IoVecList iovs, const std::function<status_t()>& altPoll) {
    for (size_t i = 0; i < iovs.len; i++) {
    ...
    if (status_t status =
                connection->rpcTransport->interruptableWriteFully(session->mShutdownTrigger.get(),
                                                                  iovs, altPoll);
        status != OK) {
        LOG_RPC_DETAIL("Failed to write %s (%zu iovs) on RpcTransport %p, error: %s", what,
                       iovs.len, connection->rpcTransport.get(), statusToString(status).c_str());
...
status_t RpcState::rpcRec(const sp<RpcSession::RpcConnection>& connection,
                          const sp<RpcSession>& session, const char* what,
                          RpcTransport::IoVecList iovs) {
    if (status_t status =
                connection->rpcTransport->interruptableReadFully(session->mShutdownTrigger.get(),
                                                                 iovs, {});
        status != OK) {
        LOG_RPC_DETAIL("Failed to read %s (%zu iovs) on RpcTransport %p, error: %s", what, iovs.len,
...
status_t RpcState::readNewSessionResponse(const sp<RpcSession::RpcConnection>& connection,
    ...
    if (status_t status =
                rpcRec(connection, session, "new session response", RpcTransport::IoVecList(iov));
        status != OK) {
        return status;
    }
...
status_t RpcState::sendConnectionInit(const sp<RpcSession::RpcConnection>& connection,
    ...
    return rpcSend(connection, session, "connection init", RpcTransport::IoVecList(iov));
}
...
status_t RpcState::readConnectionInit(const sp<RpcSession::RpcConnection>& connection,
    ...
    iovec iov{&init, sizeof(init)};
    if (status_t status =
                rpcRec(connection, session, "connection init", RpcTransport::IoVecList(iov));
        status != OK)
        return status;
...
status_t RpcState::transactAddress(const sp<RpcSession::RpcConnection>& connection,
    ...
    if (status_t status = rpcSend(connection, session, "transaction", RpcTransport::IoVecList(iovs),
                                  drainRefs);
        status != OK) {
...
status_t RpcState::waitForReply(const sp<RpcSession::RpcConnection>& connection,
    ...
    while (true) {
        ...
        if (status_t status = rpcRec(connection, session, "command header (for reply)",
    ...
    if (status_t status = rpcRec(connection, session, "reply body", RpcTransport::IoVecList(iov));
        status != OK)
        return status;
...
status_t RpcState::sendDecStrongToTarget(const sp<RpcSession::RpcConnection>& connection,
    ...
    return rpcSend(connection, session, "dec ref", RpcTransport::IoVecList(iovs));
}
...
status_t RpcState::getAndExecuteCommand(const sp<RpcSession::RpcConnection>& connection,
    ...
    RpcWireHeader command;
    iovec iov{&command, sizeof(command)};
    if (status_t status = rpcRec(connection, session, "command header (for server)",
                                 RpcTransport::IoVecList(iov));
        status != OK)
        return status;
...
status_t RpcState::processTransact(const sp<RpcSession::RpcConnection>& connection,
    ...
    if (status_t status =
                rpcRec(connection, session, "transaction body", RpcTransport::IoVecList(iov));
        status != OK)
        return status;
...
processTransactInternalTailCall:
    ...
    return rpcSend(connection, session, "reply", RpcTransport::IoVecList(iovs));
}
...
status_t RpcState::processDecStrong(const sp<RpcSession::RpcConnection>& connection,
    ...
    if (status_t status = rpcRec(connection, session, "dec ref body", RpcTransport::IoVecList(iov));
        status != OK)
        return status;
