...
status_t PersistableBundle::writeToParcelInner(Parcel* parcel) const {
    ...
    for (const auto& key_val_pair : mPersistableBundleMap) {
        ...
        RETURN_IF_FAILED(parcel->writePersistableBundle(key_val_pair.second));
    }
    return NO_ERROR;
}
...
status_t PersistableBundle::readFromParcelInner(const Parcel* parcel, size_t length) {
     ...
     * pairs themselves. This number will determine how many key-value pairs
     * to read, just like in readArrayMapInternal() in
    ...
    while (num_entries > 0) {
        ...
        switch (value_type) {
            ...
            case VAL_PERSISTABLEBUNDLE: {
                 ...
                 */
                RETURN_IF_FAILED(parcel->readPersistableBundle(&mPersistableBundleMap[key]));
                break;
            }
