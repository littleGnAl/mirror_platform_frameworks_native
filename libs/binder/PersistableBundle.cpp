...
void PersistableBundle::deleteEntryIfExists(const String16& key) {
    if (mBoolMap.erase(key)) return;
    if (mIntMap.erase(key)) return;
    if (mLongMap.erase(key)) return;
    if (mDoubleMap.erase(key)) return;
    if (mStringMap.erase(key)) return;
    if (mBoolVectorMap.erase(key)) return;
    if (mIntVectorMap.erase(key)) return;
    if (mLongVectorMap.erase(key)) return;
    if (mDoubleVectorMap.erase(key)) return;
    if (mStringVectorMap.erase(key)) return;
    mPersistableBundleMap.erase(key);
}

void PersistableBundle::putBoolean(const String16& key, bool value) {
    deleteEntryIfExists(key);
    mBoolMap[key] = value;
}
