...
bool PersistableBundle::isKeyUnique(const String16& key) const {
    return (mBoolMap.find(key) == mBoolMap.end() &&
            mIntMap.find(key) == mIntMap.end() &&
            mLongMap.find(key) == mLongMap.end() &&
            mDoubleMap.find(key) == mDoubleMap.end() &&
            mStringMap.find(key) == mStringMap.end() &&
            mBoolVectorMap.find(key) == mBoolVectorMap.end() &&
            mIntVectorMap.find(key) == mIntVectorMap.end() &&
            mLongVectorMap.find(key) == mLongVectorMap.end() &&
            mDoubleVectorMap.find(key) == mDoubleVectorMap.end() &&
            mStringVectorMap.find(key) == mStringVectorMap.end() &&
            mPersistableBundleMap.find(key) == mPersistableBundleMap.end());
}

void PersistableBundle::putBoolean(const String16& key, bool value) {
    if (!isKeyUnique(key)) return;
    mBoolMap[key] = value;
}
    ...
    while (length > 0) {
        ...
        switch (value_type) {
            ...
            case VAL_PERSISTABLEBUNDLE: {
                /*
                 * We assume that both the C++ and Java APIs ensure that only PersistableBundles,
                 * and not any other types of Parcelables, are serialized with the
                 * VAL_PERSISTABLEBUNDLE type.
                 */
                RETURN_IF_FAILED(parcel->readParcelable(&mPersistableBundleMap[key]));
                break;
            }
        ...
        length -= parcel->dataPosition() - start_pos;
    }
