...
private:
     ...
     *
     * FIXME: ensure once this is called, there is a way for RpcConnection to
     * know and invalidate threads/fds (all subsequent calls should return
     * errors).
     */
    void terminate();

    bool rpcSend(const base::unique_fd& fd, const char* what, const void* data, size_t size);
    bool rpcRec(const base::unique_fd& fd, const char* what, void* data, size_t size);
    ...
    struct BinderNode {
        ...
        //     each time is one implicit strong ref (incStrong/decStrong)
        //     (see onBinderLeaving)
        // For a remote binder:
        //     this represents the number of references we are holding
        //     on this binder (see onBinderEntering)
        //     This does NOT correspond to a local strong ref.
        size_t strong;
    };
