...
android::base::Result<CommandResult> execute(std::vector<std::string> argStringVec,
    ...
    while (true) {
        ...
        if (selectRet == -1) return android::base::ErrnoError() << "select()";
        ...
        if (ret.outPipe.ok() && FD_ISSET(ret.outPipe.get(), &readfds)) {
            ...
            if (n == -1) return android::base::ErrnoError() << "read(stdout)";
            if (n == 0) {
                ret.outPipe.reset();
            }
        }
        if (ret.errPipe.ok() && FD_ISSET(ret.errPipe.get(), &readfds)) {
            ...
            if (n == -1) return android::base::ErrnoError() << "read(stderr)";
            if (n == 0) {
                ret.errPipe.reset();
            }
        }
