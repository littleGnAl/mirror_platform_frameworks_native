...
Vector<String16> ServiceManagerShim::getUpdatableInstances(const String16& apexName) {
    std::vector<std::string> out;
    if (Status status =
                mTheRealServiceManager->getUpdatableInstances(String8(apexName).c_str(), &out);
        !status.isOk()) {
        ALOGW("Failed to getUpdatableInstances for %s: %s", String8(apexName).c_str(),
              status.toString8().c_str());
        return {};
    }
