...
use binder::{Binder, ProcessState, Service, TransactionCode, TransactionFlags};
...
// TODO: Make these lazy_static when we have that crate available in external
...
impl ProcResults {
    ...
    fn add_time(&mut self, time: u64) {
        ...
        self.buckets
            [(NUM_BUCKETS as usize - 1).min((time / unsafe { TIME_PER_BUCKET }) as usize)] += 1;
...
fn worker_fx(num: i32, worker_count: i32, iterations: i32, payload_size: i32, cs_pair: bool) {
    ...
    let binder_native = Service::new(Box::new(BinderWorkerService));
    sm.add_service(
        &generate_service_name(num),
        &binder_native,
        false,
        DumpFlags::PriorityDefault as i32,
    ).unwrap();
