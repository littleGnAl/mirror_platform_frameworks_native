    ...
    let read_funcs: Vec<Box<dyn Fn((Parcel, &[u8]))>> = vec![
        Box::new(|(parcel, data)| {
            ...
            unsafe {
                ...
                match parcel.set_data_position(pos) {
                    Ok(result) => result,
                    Err(e) => println!("error occured while setting data position: {:?}", e),
                }
            }
        }),
        ...
        Box::new(|(parcel, data)| {
            ...
            match holder.read_from_parcel(parcel.borrowed_ref()) {
                Ok(result) => result,
                Err(e) => {
                    println!("ParcelableHolder: error occured while reading from a parcel: {:?}", e)
                }
            }
        }),
        //read basic types
        ...
        Box::new(|(parcel, _data)| {
            ...
            match empty_parcelable.read_from_parcel(parcel.borrowed_ref()) {
                Ok(result) => result,
                Err(e) => {
                    println!("EmptyParcelable: error occured while reading from a parcel: {:?}", e)
                }
            }
        }),
        Box::new(|(parcel, _data)| {
            ...
            match single_parcelable.read_from_parcel(parcel.borrowed_ref()) {
                Ok(result) => result,
                Err(e) => println!(
                    "SingleDataParcelable: error occured while reading from a parcel: {:?}",
                    e
                ),
            }
        }),
        Box::new(|(parcel, _data)| {
            ...
            match generic_parcelable.read_from_parcel(parcel.borrowed_ref()) {
                Ok(result) => result,
                Err(e) => println!(
                    "GenericDataParcelable: error occured while reading from a parcel: {:?}",
                    e
                ),
            }
        }),
