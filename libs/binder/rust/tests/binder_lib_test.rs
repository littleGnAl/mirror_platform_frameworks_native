...
impl<'s> Client<'s> {
    ...
    fn test_pass_file_descriptor(&mut self) -> TestResult {
        ...
        // We will transfer ownership of the write fd to `data`, so we want `data` to drop prior to blocking
        // in `wait_for_read_data` so that it closes the fd for us (client side).
        {
            let mut data = Parcel::new();
            let mut reply = Parcel::new();
    ...
    fn test_set_error(&mut self) -> TestResult {
        ...
        for i in 0..test_value.len() {
            ...
            // Return enum wont make sense(ok, unknown, unknown) so we need to check error_check instead
            // to be equivalent to the original test
            let _ = self
                .m_server
