...
pub trait IATest<P>: Interface {
    /// Returns a test string
    fn test(&self) -> binder::BoxFuture<'static, binder::public_api::Result<String>>;

    /// Return the arguments sent via dump
    fn get_dump_args(&self) -> binder::BoxFuture<'static, binder::public_api::public_api::Result<Vec<String>>>;

    /// Returns the caller's SELinux context
    fn get_selinux_context(&self) -> binder::BoxFuture<'static, binder::public_api::public_api::Result<String>>;
}
...
impl<P: binder::BinderAsyncPool> IATest<P> for BpTest {
    ...
    fn get_dump_args(&self) -> binder::BoxFuture<'static, binder::public_api::Result<Vec<String>>> {
        let binder = self.binder.clone();
        P::spawn(
    ...
    fn get_selinux_context(&self) -> binder::BoxFuture<'static, binder::public_api::Result<String>> {
        let binder = self.binder.clone();
        P::spawn(
