...
pub trait IATest<P>: Interface {
    /// Returns a test string
    fn test(&self) -> binder::BoxFutureSync<'static, binder::Result<String>>;

    /// Return the arguments sent via dump
    fn get_dump_args(&self) -> binder::BoxFutureSync<'static, binder::Result<Vec<String>>>;

    /// Returns the caller's SELinux context
    fn get_selinux_context(&self) -> binder::BoxFutureSync<'static, binder::Result<String>>;
}
...
impl<P: binder::BinderAsyncPool> IATest<P> for BpTest {
    fn test(&self) -> binder::BoxFutureSync<'static, binder::Result<String>> {
        ...
        P::spawn(
            move || OwnedParcel::try_from(binder.transact(TestTransactionCode::Test as TransactionCode, 0, |_| Ok(()))).unwrap(),
    ...
    fn get_dump_args(&self) -> binder::BoxFutureSync<'static, binder::Result<Vec<String>>> {
        let binder = self.binder.clone();
        P::spawn(
            move || OwnedParcel::try_from(binder.transact(TestTransactionCode::GetDumpArgs as TransactionCode, 0, |_| Ok(()))).unwrap(),
            |reply| async move { reply?.into_parcel().read() }
        )
    }
    ...
    fn get_selinux_context(&self) -> binder::BoxFutureSync<'static, binder::Result<String>> {
        let binder = self.binder.clone();
        P::spawn(
            move || OwnedParcel::try_from(binder.transact(TestTransactionCode::GetSelinuxContext as TransactionCode, 0, |_| Ok(()))).unwrap(),
            |reply| async move { reply?.into_parcel().read() }
        )
    }
}
