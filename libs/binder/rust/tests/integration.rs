...
use binder::binder_impl::{
    Binder, IBinderInternal, Parcel, OwnedParcel, TransactionCode, FIRST_CALL_TRANSACTION,
};
...
impl<P: binder::binder_impl::BinderAsyncPool> IATest<P> for BpTest {
    fn test(&self) -> binder::BoxFuture<'static, Result<String, StatusCode>> {
...
impl<P: binder::binder_impl::BinderAsyncPool> IATest<P> for Binder<BnTest> {
    fn test(&self) -> binder::BoxFuture<'static, Result<String, StatusCode>> {
        let res = self.0.test();
