...
/// socket file name. The final socket file path will be /dev/socket/`socket_name`
///
...
///
/// Returns true if the server has shutdown normally, false if it failed in some way.
pub fn run_initialized_unix_domain_rpc_server<F: FnOnce()>(
    service: SpIBinder,
    socket_name: &str,
    on_ready: F,
) -> bool {
    let mut ready_notifier = ReadyNotifier(Some(on_ready));
    ready_notifier.run_initialized_unix_domain_server(service, socket_name)
}
    ...
    F: FnOnce(),
{
    ...
    fn run_initialized_unix_domain_server(
        &mut self,
        ...
        unsafe {
            binder_rpc_unstable_bindgen::RunInitializedUnixDomainRpcServer(
                service,
                socket_name.as_ptr(),
                Some(Self::ready_callback),
                param,
