...
impl RpcServer {
    ...
    // Creates a binder RPC server that bootstraps sessions using an existing
    // Unix domain socket pair, with a given root IBinder object.
    // Callers should create a pair of SOCK_STREAM Unix domain sockets, pass one
    // to the server and the other to the client. Multiple client session can be
    // created from the client end of the pair.
