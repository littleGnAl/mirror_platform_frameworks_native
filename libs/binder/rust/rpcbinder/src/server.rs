...
impl RpcServer {
    ...
    pub fn new_vsock(mut service: SpIBinder, port: u32) -> Result<RpcServer, Error> {
        ...
        let handle = unsafe { binder_rpc_unstable_bindgen::RunVsockRpcServer(service, port) };
    ...
    pub fn new_init_unix_domain(
        ...
        let handle = unsafe {
            binder_rpc_unstable_bindgen::RunInitUnixDomainRpcServer(service, socket_name.as_ptr())
        };
        Self::from_handle(handle)
    }
