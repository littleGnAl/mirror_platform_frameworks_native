...
impl RpcServer {
    ...
    pub fn setup_vsock(mut service: SpIBinder, cid: u32, port: u32) -> Result<RpcServer, Error> {
        let service = service.as_native_mut();

        // SAFETY: Service ownership is transferring to the server and won't be valid afterward.
        // Plus the binder objects are threadsafe.
        unsafe {
            Self::checked_from_ptr(binder_rpc_unstable_bindgen::ARpcServer_setupVsock(
                service, cid, port,
            ))
        }
    }
    ...
    pub fn setup_init_unix_domain(
        ...
        unsafe {
            Self::checked_from_ptr(binder_rpc_unstable_bindgen::ARpcServer_setupInitUnixDomain(
                service,
                socket_name.as_ptr(),
            ))
        }
