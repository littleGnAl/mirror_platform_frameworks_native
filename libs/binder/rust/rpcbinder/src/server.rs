...
///
/// If and when the server is ready for connections, `on_ready` is called to allow appropriate action to be
...
pub fn run_unix_domain_rpc_server<P: AsRef<Path>, F: FnOnce()>(
    ...
    if socket_path.as_ref().exists() && remove_file(socket_path.as_ref()).is_err() {
        log::error!("Failed to remove Unix Domain socket: {:?}", socket_path.as_ref());
        return false;
    }
    let mut ready_notifier = ReadyNotifier(Some(on_ready));
