...
use binder_rpc_unstable_bindgen::ARpcSession_FileDescriptorTransportMode;
...
impl RpcSessionRef {
    ...
    pub fn set_max_incoming_threads(&self, threads: u32) {
        unsafe {
    ...
    pub fn set_max_outgoing_threads(&self, threads: u32) {
        unsafe {
    ...
    pub fn setup_unix_domain_bootstrap_client<T: FromIBinder + ?Sized>(
        ...
        // SAFETY: ARpcSession_setupUnixDomainBootstrapClient takes ownership of
        // bootstrap_fd. The returned AIBinder has correct reference count, and
        // the ownership can safely be taken by new_spibinder.
        let service = unsafe {
...
/// File descriptor transport modes supported by RPC Binder.
#[derive(Clone, Copy)]
pub enum FileDescriptorTransportMode {
    /// Disable support for sending file descriptors.
    None,
    /// Send file descriptors via UDS ancillary data.
    Unix,
    /// Send file descriptors as Trusty IPC handles.
    Trusty,
}

impl From<FileDescriptorTransportMode> for ARpcSession_FileDescriptorTransportMode {
