...
use binderthroughput_rs_sys::cxx_timing;
...
fn worker_fx(num: i32, worker_count: i32, iterations: i32, payload_size: i32, cs_pair: bool) {
    ...
    let start = unsafe { cxx_timing::create_time_point() };
    let end = unsafe { cxx_timing::create_time_point() };
    ...
    for i in 0..iterations {
        ...
        // directly comparable.
        unsafe { cxx_timing::now(start) };
        let ret = workers[target].transact(BinderWorkerService::BINDER_NOP, &data, Some(&mut reply), 0);
        unsafe { cxx_timing::now(end) };
        let time = unsafe { cxx_timing::nanosecond_diff(start, end) };
        results.add_time(time);
