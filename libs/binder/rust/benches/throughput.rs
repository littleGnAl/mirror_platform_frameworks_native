...
fn worker_fx(num: i32, worker_count: i32, iterations: i32, payload_size: i32, cs_pair: bool) {
    ...
    let mut start = libc::timespec { tv_sec: 0, tv_nsec: 0 };
    let mut end = libc::timespec { tv_sec: 0, tv_nsec: 0 };
    ...
    for i in 0..iterations {
        ...
        unsafe { libc::clock_gettime(libc::CLOCK_REALTIME, &mut start); }
        let ret = workers[target].transact(BinderWorkerService::BINDER_NOP, &data, Some(&mut reply), 0);
        unsafe { libc::clock_gettime(libc::CLOCK_REALTIME, &mut end); }
        let nanoseconds = (end.tv_sec - start.tv_sec)*1_000_000_000 + (end.tv_nsec - start.tv_nsec);
