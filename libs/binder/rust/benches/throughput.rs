...
extern "C" {
    /// rand from libc, used for compatibility with C++ benchmark
    pub fn rand() -> std::os::raw::c_int;
    /// srand from libc, used for compatibility with C++ benchmark
...
pub trait IBinderWorker: Interface {
    /// Perform a nop transaction with given payload size
...
fn worker_fx(num: i32, worker_count: i32, iterations: i32, payload_size: i32, cs_pair: bool) {
    ...
    // Signal completion to manager and wait.
    signal(&mut stdout);
    wait(&mut stdin);

    // Send results to manager and wait for go to exit.
