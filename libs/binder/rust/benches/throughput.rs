...
use binder::{Binder, IBinder, ProcessState, Service, TransactionCode, TransactionFlags};
...
lazy_static! {
    static ref MAX_TIME_BUCKET: RwLock<u64> = RwLock::new(50u64 * 1000000);
    static ref TIME_PER_BUCKET: RwLock<u64> = RwLock::new(50u64 * 1000000 / NUM_BUCKETS);
}
...
impl ProcResults {
    fn new() -> Self {
        Self {
            ...
            worst: 0,
            best: *MAX_TIME_BUCKET.read().unwrap(),
            transactions: 0,
            long_transactions: 0,
            total_time: 0,
        }
    ...
    fn add_time(&mut self, time: u64) {
        let max_time_bucket = *MAX_TIME_BUCKET.read().unwrap();
        if time > max_time_bucket {
            self.long_transactions += 1;
        }
        let time_per_bucket = *TIME_PER_BUCKET.read().unwrap();
        self.buckets
            [(NUM_BUCKETS as usize - 1).min((time / time_per_bucket) as usize)] += 1;
...
impl fmt::Display for ProcResults {
    fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        ...
        let time_per_bucket_ms = *TIME_PER_BUCKET.read().unwrap() as f64 / 1.0E6;
...
impl BinderWorkerService {
    const BINDER_NOP: TransactionCode = binder::Interface::FIRST_CALL_TRANSACTION;
}

impl Binder for BinderWorkerService {
    const INTERFACE_DESCRIPTOR: &'static str = "BinderWorkerService";
    ...
    fn on_transact(
        ...
        match code {
            Self::BINDER_NOP => Ok(()),
            _ => Err(binder::Error::UNKNOWN_ERROR),
        }
    }
}
...
fn worker_fx(num: i32, worker_count: i32, iterations: i32, payload_size: i32, cs_pair: bool) {
    ...
    let binder_native = Service::new(BinderWorkerService);
    sm.add_service(
        &generate_service_name(num),
        &binder_native,
        false,
    ...
    for i in 0..server_count {
        if num != i {
            workers.push(sm.get_service(&generate_service_name(i)).unwrap());
        }
    }
    ...
    for i in 0..iterations {
        ...
        while sz >= mem::size_of::<u32>() as i32 {
            data.write_i32(0).expect("Parcel write failed");
            sz -= mem::size_of::<u32>() as i32;
        }

        let start = Instant::now();
        let ret = workers[target].transact(BinderWorkerService::BINDER_NOP, &data, None, 0);
        let duration = start.elapsed();
        results.add_time(duration.as_nanos().try_into().unwrap());
...
fn run_main(
    ...
    for i in 0..worker_count {
        workers.push(
            Command::new(env::current_exe().unwrap())
                ...
                .arg((*MAX_TIME_BUCKET.read().unwrap()).to_string())
                .stdin(Stdio::piped())
                .stdout(Stdio::piped())
                .spawn()
    ...
    if training_round {
        ...
        *MAX_TIME_BUCKET.write().unwrap() = 2 * tot_results.worst;
        *TIME_PER_BUCKET.write().unwrap() = 2 * tot_results.worst / NUM_BUCKETS;
    } else {
        println!("{}", tot_results);
    }
}
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if args.len() > 1 && args[1] == "--worker" {
        ...
        *MAX_TIME_BUCKET.write().unwrap() = max_time_bucket;
        *TIME_PER_BUCKET.write().unwrap() = max_time_bucket / NUM_BUCKETS;
        return Ok(worker_fx(
            num,
            worker_count,
            iterations,
    ...
    for i in 1..args.len() {
        match args[i].as_str() {
            ...
            "-m" => {
                ...
                *MAX_TIME_BUCKET.write().unwrap() = max_time * 1000;
                *TIME_PER_BUCKET.write().unwrap() = max_time * 1000 / NUM_BUCKETS;
            }
            _ => {}
        }
    }
