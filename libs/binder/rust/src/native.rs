...
impl<T: Remotable> Binder<T> {
    ...
    /// Retrieve a shared reference to the Binder interface implemented by this
    /// object.
    pub fn as_interface(&self) -> Arc<T::Interface> {
        unsafe {
            // Safety: As long as `self` is alive, this `rust_object` pointer
            // will stay valid, as it can only be destroyed after we release the
            // strong reference to the `AIBinder` object in `Drop`.
            (*self.rust_object).as_interface()
        }
    }
