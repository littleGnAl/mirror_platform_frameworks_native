...
use crate::binder::{AsNative, InterfaceClassMethods, Remotable, TransactionCode};
use crate::binder_fn_get_class;
...
impl<B: Remotable> TryFrom<SpIBinder> for Binder<B> {
    ...
    fn try_from(mut ibinder: SpIBinder) -> Result<Self> {
        ...
        if userdata.is_null() {
            return Err(StatusCode::UNEXPECTED_NULL.into());
        }
...
/// Declare a native type for a binder interface.
///
/// Creates a declaration of a wrapper type called `$name` which holds an
/// object that implements a given interface (`$interface`). The outer object
/// implements [`Remotable`]($crate::Remotable) which cannot be implemented
/// directly for the inner object.
///
/// # Examples
///
/// The following example declares the handle type `BnServiceManager` (short for
/// Binder Native ServiceManager). `BnServiceManager` will hold a handle to an
/// object that implements the `IServiceManager` trait.
///
/// ```rust
...
/// }
///
/// declare_binder_native!(BnServiceManager{}: IServiceManager);
/// ```
// TODO: make this a derive proc-macro
#[macro_export]
macro_rules! declare_binder_native {
    ($name:ident { $($fname:ident: $fty:ty = $finit:expr),* }: $interface:path) => {
        declare_binder_native!(@doc
            $name,
            { $($fname:$fty=$finit),* },
            $interface,
            concat!("A binder [`Remotable`]($crate::Remotable) that holds an [`", stringify!($interface), "`] object.")
        );
    };

    (@doc $name:ident, { $($fname:ident: $fty:ty = $finit:expr),* }, $interface:path, $doc:expr) => {
        #[doc = $doc]
        pub struct $name<T: $interface + Sync> {
            inner: T,
            $($fname: $fty,)*
        }

        impl<T: $interface + Sync> $name<T> {
            pub fn new(inner: T) -> $crate::Binder<$name<T>> {
                $crate::Binder::new($name { inner, $($fname: $finit),* })
            }
        }

        impl<T: $interface + Sync> std::ops::Deref for $name<T> {
            type Target = T;
