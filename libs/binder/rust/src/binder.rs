...
impl InterfaceClass {
    ...
    pub fn get_descriptor(&self) -> String {
        unsafe {
            ...
            let raw_descriptor: *const u16 = sys::AIBinder_Class_getDescriptor(self.0);


            let mut len = 0;
            while *raw_descriptor.offset(len) != 0 {
                len += 1;
            }

            let utf16_slice = slice::from_raw_parts(raw_descriptor, len as usize);
            String::from_utf16(utf16_slice)
                .expect("Expected valid UTF-16 string from AIBinder_Class_getDescriptor")
        }
    }
}
