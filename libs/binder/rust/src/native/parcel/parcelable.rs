...
use crate::native::binder_native::BinderNative;
use crate::native::parcel::Parcel;
use crate::native::String16;
use crate::native::utils::{Sp, String8};
...
impl<P: Copy + Default + Parcelable> Parcelable for [P] {
    ...
    fn deserialize(parcel: &Parcel) -> BinderResult<Vec<P>> {
        let mut vec = Vec::new();
...
impl Parcelable for [String8] {
    ...
    fn deserialize(parcel: &Parcel) -> BinderResult<Vec<String8>> {
        let size = parcel.try_read_i32()?;
        let mut vec = Vec::with_capacity(size.try_into().unwrap());

        for _ in 0..size {
            vec.push(parcel.try_read_string8()?);
        }
...
impl Parcelable for [String16] {
    ...
    fn deserialize(parcel: &Parcel) -> BinderResult<Vec<String16>> {
        let size = parcel.try_read_i32()?;
        let mut vec = Vec::with_capacity(size.try_into().unwrap());

        for _ in 0..size {
            vec.push(parcel.try_read_string16()?);
        }
...
fn test_custom_parcelable() {
    ...
    let mut s1 = String8::from("str1");
    let mut s2 = String8::from("str2");
    let mut s3 = String8::from("str3");

    let mut str8s = vec![s1, s2, s3];
...
fn test_slice_parcelables() {
    ...
    assert_eq!(parcel.data(), [8, 0, 0, 0, 255, 127, 0, 128, 42, 0, 139, 255]);
    ...
    assert_eq!(parcel.data(), [16, 0, 0, 0, 255, 255, 255, 255, 57, 48, 0, 0, 42, 0, 0, 0, 117, 0, 0, 0]);
    ...
    assert_eq!(parcel.data(), [16, 0, 0, 0, 255, 255, 255, 127, 0, 0, 0, 128, 42, 0, 0, 0, 139, 255, 255, 255]);
    ...
    let mut s1 = String8::from("Hello, Binder!");
    let mut s2 = String8::from("This is a utf8 string.");
    let mut s3 = String8::from("Some more text here.");
    ...
    let mut s4 = "Hello, Binder!".into();
    let mut s5 = "This is a utf8 string.".into();
    let mut s6 = "Some more text here.".into();
