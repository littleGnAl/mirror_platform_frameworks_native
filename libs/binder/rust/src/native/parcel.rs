...
impl Parcel {
    ...
    pub fn data(&self) -> &[u8] {
        let mut data = unsafe { android_Parcel_data(&self.0) };
    ...
    pub fn set_data_position(&self, pos: size_t) -> BinderResult<()> {
        ...
        unsafe {
            android_Parcel_setDataPosition(&self.0, pos);
        }
    ...
    pub unsafe fn write_interface_token(&mut self, interface: &String16) -> BinderResult<()> {
        binder_status(android_Parcel_writeInterfaceToken(&mut self.0, &interface.0))
    }
    ...
    pub unsafe fn enforce_interface(
        ...
        android_Parcel_enforceInterface(&self.0, &interface.0, threadState)
    }

    pub unsafe fn check_interface(&self, ibinder: &mut IBinder) -> bool {
        android_Parcel_checkInterface(&self.0, ibinder.as_native_mut())
    }
    ...
    pub fn error_check(&self) -> status_t {
        unsafe { android_Parcel_errorCheck(&self.0) }
    }
    ...
    pub fn write_string16(&mut self, str: &String16) -> BinderResult<()> {
        unsafe { binder_status(android_Parcel_writeString16(&mut self.0, &str.0)) }
    }
    ...
    pub fn write_binder_native<T: Binder>(&mut self, binder: &Sp<BinderNative<T>>) -> BinderResult<()> {
        unsafe {
    ...
    pub(crate) fn write_binder(&mut self, binder: &Sp<IBinder>) -> BinderResult<()> {
        unsafe {
            binder_status(android_Parcel_writeStrongBinder(
                &mut self.0,
                binder.as_native(),
    ...
    // There is an implicit `Sized` bound on P, so you cant do something really weird like P: [P2] here
    // and the `Copy` bound ensures only simple types like ints and floats are byte copied.
    pub fn write_slice<P: Copy + Parcelable>(&mut self, slice: &[P]) -> BinderResult<()> {
        let p_len: size_t = slice.len().try_into().unwrap();
        let byte_size = size_of::<P>().try_into().expect("Conversion to always succeed");
        let byte_len = p_len.checked_mul(byte_size).ok_or(BinderError::BAD_VALUE)?;
    ...
    pub fn write_u8_slice(&mut self, slice: &[u8]) -> BinderResult<()> {
    ...
    pub fn write_i8_slice(&mut self, slice: &[i8]) -> BinderResult<()> {
        let len = slice.len().try_into().unwrap();

        unsafe {
            binder_status(android_Parcel_writeByteArray(&mut self.0, len, slice.as_ptr() as *const u8))
        }
    }

    pub fn write_u16_slice(&mut self, slice: &[u16]) -> BinderResult<()> {
        let len: size_t = slice.len().try_into().map_err(|_| BinderError::BAD_VALUE)?;
        let byte_len = len.checked_mul(2).ok_or(BinderError::BAD_VALUE)?;

        unsafe {
            binder_status(android_Parcel_writeByteArray(&mut self.0, byte_len, slice.as_ptr() as *const u8))
        }
    }
    ...
    pub fn write_i16(&mut self, val: i16) -> BinderResult<()> {
        unsafe { binder_status(android_Parcel_writeChar(&mut self.0, val as u16)) }
    }
    ...
    pub fn write_utf8_as_utf16(&mut self, s: &str) -> BinderResult<()> {
        self.write_string16(&s.into())
    }

    /// Takes multiple UTF8 encoded strings, convert to UTF16, write it to the parcel.
    pub fn write_utf8_slice_as_utf16<S: Deref<Target=str>>(&mut self, slice: &[S]) -> BinderResult<()> {
    ...
    pub fn write_slice_size<T>(&mut self, slice: &[T]) -> BinderResult<()> {
        self.write_i32(slice.len().try_into().map_err(|_| BinderError::BAD_VALUE)?)
    }
    ...
    pub unsafe fn write_dup_file_descriptor(&mut self, fd: RawFd) -> BinderResult<()> {
    ...
    pub unsafe fn write_dup_parcel_file_descriptor(&mut self, fd: RawFd) -> BinderResult<()> {
    ...
    pub unsafe fn write_unique_file_descriptor(&mut self, fd: RawFd) -> BinderResult<()> {
        let fd = android_base_unique_fd { fd_: fd };

        binder_status(android_Parcel_writeUniqueFileDescriptor(&mut self.0, &fd))
    }
    ...
    pub fn try_read<P: Parcelable + ?Sized>(&self) -> BinderResult<P::Deserialized> {
        P::deserialize(self)
    }
    ...
    // There is an implicit `Sized` bound on P, so you cant do something really weird like P: [P2] here
    // and `Copy` ensures only simple types are byte copied
    pub fn read_to_slice<P: Copy + Parcelable>(&self, slice: &mut [P]) -> BinderResult<()> {
        let byte_size = size_of::<P>().try_into().expect("Conversion to always succeed");
        let len: size_t = slice.len().try_into().map_err(|_| BinderError::BAD_VALUE)?;
        let byte_len = len.checked_mul(byte_size).ok_or(BinderError::BAD_VALUE)?;
        let status = unsafe {
            android_Parcel_read(
                &self.0,
    ...
    pub fn resize_vec<P: Default + Parcelable>(&self, vec: &mut Vec<P>) -> BinderResult<()> {
        let byte_len: usize = self.try_read_i32()?.try_into().expect("Conversion to always succeed");
        let byte_size = size_of::<P>().try_into().expect("Conversion to always succeed");
        let new_len = byte_len.checked_div(byte_size).ok_or(BinderError::BAD_VALUE)?;
    ...
    pub fn read_to_vec<P: Copy + Default + Parcelable>(&self, vec: &mut Vec<P>) -> BinderResult<()> {
        self.resize_vec(vec)?;
    ...
    pub fn read_inplace(&self, len: size_t) -> &[u8] {
        let mut data = unsafe { android_Parcel_readInplace(&self.0, len) };
    ...
    pub fn read_i16(&self) -> i16 {
        unsafe { android_Parcel_readChar(&self.0) as i16 }
    }

    pub fn try_read_i16(&self) -> BinderResult<i16> {
        let mut ch = 0u16;
        let result = unsafe { android_Parcel_readChar1(&self.0, &mut ch) };
    ...
    pub fn try_read_string16(&self) -> BinderResult<String16> {
        ...
        let status = unsafe { android_Parcel_readString161(&self.0, &mut s.0) };
    ...
    pub fn read_string16(&self) -> String16 {
        ...
        self.try_read_string16()
            .unwrap_or(String16::new())
    }

    /// Returns a utf16 slice into this `Parcel`'s buffer inplace.
    pub fn read_string16_inplace(&self) -> &[u16] {
        let mut out_len = 0;
        let mut data = unsafe { android_Parcel_readString16Inplace(&self.0, &mut out_len) };
        ...
        unsafe {
            slice::from_raw_parts(data, out_len.try_into().unwrap())
        }
    }

    pub(crate) unsafe fn read_strong_binder(&self) -> Sp<IBinder> {
    ...
    pub(crate) unsafe fn try_read_strong_binder(&self) -> BinderResult<Sp<IBinder>> {
        let mut sp_binder: Sp<IBinder> = Sp::null();
        let status = android_Parcel_readStrongBinder1(&self.0, sp_binder.as_native_mut());
    ...
    pub fn read_utf8_slice_from_utf16(&self) -> BinderResult<Vec<String8>> {
    ...
    pub fn read_exception_code(&self) -> i32 {
        unsafe { android_Parcel_readExceptionCode(&self.0) }
    }
    ...
    pub unsafe fn read_file_descriptor(&self) -> BinderResult<RawFd> {
        let fd = android_Parcel_readFileDescriptor(&self.0);
        ...
        if fd == BinderError::BAD_TYPE as i32 {
            return Err(BinderError::BAD_TYPE);
        }
    ...
    /// in the parcel, which you do not own -- use dup() to get your own copy.
    pub unsafe fn read_parcel_file_descriptor(&self) -> BinderResult<RawFd> {
        let fd = android_Parcel_readParcelFileDescriptor(&self.0);
    ...
    pub unsafe fn read_unique_file_descriptor(&self) -> BinderResult<RawFd> {
        let mut fd = MaybeUninit::uninit();
        let status = android_Parcel_readUniqueFileDescriptor(&self.0, fd.as_mut_ptr());

        binder_status(status).map(|_| unsafe { fd.assume_init().fd_ })
    }

    /// Retrieve a Java "parcel file descriptor" from the parcel.
    pub unsafe fn read_unique_parcel_file_descriptor(&self) -> BinderResult<RawFd> {
        let mut fd = MaybeUninit::uninit();
        let status = android_Parcel_readUniqueParcelFileDescriptor(&self.0, fd.as_mut_ptr());
