...
impl ProcessState {
    ...
    ///
    /// This should be done before creating any Binder client or server. If not,
    /// some things (such as `IBinder::link_to_death`) will silently not work.
    ...
    /// threadpool.
    ///
    /// By default, after [`start_thread_pool`] is called, this is 15. If it is
    /// called additional times, the thread pool size can only be increased.
    ...
    /// created by [`set_thread_pool_max_thread_count`] and
    /// [`start_thread_pool`].
