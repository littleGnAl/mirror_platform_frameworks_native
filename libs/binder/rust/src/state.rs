...
impl ThreadState {
    ...
    unsafe fn get_calling_sid<'a>() -> Option<&'a std::ffi::CStr> {
    ...
    /// given, the implmementation should fall back to using the PID
    /// instead.
    ...
    where
        ...
        // reference to th scope of the callback, we prevent it beeing
        // used beond the guarrantieed lifetime.
        check_permission(unsafe { Self::get_calling_sid() })
    }
}
