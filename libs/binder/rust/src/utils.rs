...
macro_rules! wrap_sp {
    {
        $(#[$attr:meta])*
        $vis:vis struct $wrapper:ident(Sp<$cxx_ty:ty>) {
        ...
        #[repr(transparent)]
        #[derive(PartialEq)]
        $vis struct $wrapper(crate::utils::Sp<$cxx_ty>);

        impl $wrapper {
            pub(crate) unsafe fn from_raw(ptr: *mut android_sp<$cxx_ty>) -> Option<Self> {
                if ptr.is_null() {
                    return None;
                }

                let sp = Sp(ptr.as_mut()?);
                let null_pointee = AsNative::<$cxx_ty>::as_native(&sp).is_null();
        ...
        unsafe impl crate::utils::AsNative<$cxx_ty> for Sp<$cxx_ty> {
            fn as_native(&self) -> *const $cxx_ty {
                if self.0.is_null() {
                    ptr::null()
                } else {
            ...
            fn as_native_mut(&mut self) -> *mut $cxx_ty {
                if self.0.is_null() {
                    ptr::null_mut()
                } else {
                    unsafe { $cxx_getter(self.0) }
        ...
        unsafe impl crate::utils::AsNative<$cxx_ty> for $wrapper {
            fn as_native(&self) -> *const $cxx_ty {
                self.0.as_native()
            }
