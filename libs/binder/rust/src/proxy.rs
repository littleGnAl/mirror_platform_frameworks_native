...
use crate::binder::{
    AsNative, IBinder, Interface, InterfaceClass, TransactionCode, TransactionFlags,
};
...
///
/// This struct encapsulates the C++ `sp<IBinder>` class. However, this wrapper
...
impl SpIBinder {
    ...
    pub fn into_interface(mut self, class: InterfaceClass) -> Option<Self> {
        let success = unsafe {
            // Safety: `SpIBinder` guarantees that it always contains a valid
...
impl<T: AsNative<sys::AIBinder>> IBinder for T {
    ...
    fn transact<F: FnOnce(&mut Parcel) -> Result<()>>(
        ...
        Status::from(status).and_then(|| unsafe {
            ...
            Parcel::owned(reply).ok_or(StatusCode::UNEXPECTED_NULL.into())
        })
    }
...
impl Serialize for Option<&SpIBinder> {
    fn serialize(&self, parcel: &mut Parcel) -> Result<()> {
        parcel.write_binder(*self)
    }
}

impl Deserialize for Option<SpIBinder> {
    fn deserialize(parcel: &Parcel) -> Result<Option<SpIBinder>> {
        parcel.read_binder()
    }
}
...
pub fn get_service(name: &str) -> Option<SpIBinder> {
    unsafe {
        ...
        SpIBinder::from_raw(sys::AServiceManager_getService(
...
pub fn get_interface<T: Interface>(name: &str) -> Result<T> {
    let service = get_service(name);
    match service {
        Some(service) => T::new(service),
        None => Err(StatusCode::NAME_NOT_FOUND.into()),
    }
}

/// Declare a proxy type for a binder interface.
///
/// Creates a declaration of an [`Interface`] type called `$name` which holds a
...
macro_rules! declare_binder_proxy {
    ($name:ident { $($fname:ident: $fty:ty = $finit:expr),* }: $interface:path $(, $service:path)?) => {
    ...
    (@doc $name:ident, { $($fname:ident: $fty:ty = $finit:expr),* }, $interface:path, $doc:expr) => {
        ...
        impl $name {
            $crate::binder_fn_get_class!($name);
        }

        declare_binder_proxy!(@doc $name, { $($fname:$fty=$finit),* }, $interface, $name, $doc);
    };

    (@doc $name:ident, { $($fname:ident: $fty:ty = $finit:expr),* }, $interface:path, $class:path, $doc:expr) => {
        #[doc = $doc]
        pub struct $name {
        ...
        where
            $name: $interface,
        {
            const DESCRIPTOR: &'static str = <Self as $interface>::DESCRIPTOR;

            fn new(client: $crate::SpIBinder) -> $crate::Result<Self> {
                let binder = client.into_interface(Self::get_class())
                    .ok_or($crate::StatusCode::BAD_TYPE)?;
