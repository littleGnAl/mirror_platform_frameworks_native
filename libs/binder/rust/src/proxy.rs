...
impl IBinder for Interface {
    ...
    fn get_interface_descriptor(&self) -> &Str16 {
        unsafe {
            let descriptor = android_c_interface_IBinder_getInterfaceDescriptor(self.0.as_native());
            Str16::from_ptr(descriptor)
        }
    }

    fn is_binder_alive(&self) -> bool {
        unsafe { android_c_interface_IBinder_isBinderAlive(self.0.as_native()) }
    }
...
impl Parcelable for Interface {
    ...
    fn deserialize(parcel: &Parcel) -> Result<Interface> {
        let ibinder = unsafe { parcel.try_read_strong_binder()? };
...
impl<H: Handle> IBinder for H {
    fn get_interface_descriptor(&self) -> &Str16 {
        self.remote().get_interface_descriptor()
    }

    fn is_binder_alive(&self) -> bool {
        self.remote().is_binder_alive()
    }

    fn ping_binder(&mut self) -> Result<()> {
        self.remote_mut().ping_binder()
    }

    fn dump<F: AsRawFd>(&mut self, fp: &F, args: &[String16]) -> Result<()> {
        self.remote_mut().dump(fp, args)
    }

    fn get_extension(&mut self) -> Result<Option<Interface>> {
        self.remote_mut().get_extension()
    }
    ...
    fn transact(
        ...
        self.remote_mut().transact(code, data, reply, flags)
    }
}
    ...
    (@doc $name:ident, $interface:path, $descriptor:expr, $doc:expr) => {
            ...
            $name: $interface,
        {
            fn new(client: $crate::Interface) -> $crate::Result<Self> {
                if client.get_interface_descriptor().to_string() == <$name as $interface>::INTERFACE_DESCRIPTOR {
                    Ok(Self(client))
                } else {
