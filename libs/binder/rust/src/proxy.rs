...
use crate::binder::{AsNative, IBinder, InterfaceClass, TransactionCode, TransactionFlags};
...
impl SpIBinder {
    ...
    pub fn associate_class(&mut self, class: InterfaceClass) -> bool {
        unsafe {
            // Safety: `SpIBinder` guarantees that it always contains a valid
...
pub fn get_interface<T: Proxy>(name: &str) -> Result<T> {
    let service = get_service(name);
    match service {
        Some(service) => T::from_binder(service),
