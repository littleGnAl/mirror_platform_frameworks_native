...
impl Parcel {
    ...
    /// Wrap an existing `android_Parcel` pointer, but do not take ownership of
    /// the pointee parcel.
    ...
    pub fn data(&self) -> &[u8] {
        unsafe {
            let data = android_Parcel_data(self.ptr);
    ...
    pub unsafe fn set_data_size(&mut self, size: size_t) -> Result<()> {
        let status = android_Parcel_setDataSize(self.ptr, size);
    ...
    pub unsafe fn set_data(&mut self, data: &[u8]) -> Result<()> {
        let status = android_Parcel_setData(self.ptr, data.as_ptr(), data.len().try_into().unwrap());
    ...
    pub fn push_allow_fds(&mut self, allow_fds: bool) -> bool {
        unsafe { android_Parcel_pushAllowFds(self.ptr, allow_fds) }
    }

    pub fn restore_allow_fds(&mut self, allow_fds: bool) {
        unsafe { android_Parcel_restoreAllowFds(self.ptr, allow_fds) }
    }
    ...
    // There is an implicit `Sized` bound on P, so you cant do something really
    ...
    pub fn write_blob<'b, 'p: 'b>(
        ...
        let status =
            unsafe { android_c_interface_Parcel_writeBlob(self.ptr, len, mutable_copy, &mut blob) };
    ...
    pub fn resize_vec<P: Default + Parcelable>(&self, vec: &mut Vec<P>) -> Result<()> {
        let byte_len: usize = self.try_read_i32()?.try_into().or(Err(Error::BAD_VALUE))?;
        let byte_size = size_of::<P>()
            .try_into()
            .expect("Conversion to always succeed");
    ...
    pub fn read_inplace(&self, len: size_t) -> &[u8] {
        unsafe {
            let data = android_Parcel_readInplace(self.ptr, len);
    ...
    pub fn read_string16_inplace(&self) -> &[u16] {
        unsafe {
            let mut out_len = 0;
            let data = android_Parcel_readString16Inplace(self.ptr, &mut out_len);
...
impl Ord for Parcel {
    fn cmp(&self, other: &Parcel) -> Ordering {
        let ord = unsafe { android_Parcel_compareData(self.ptr, other.ptr) };
        ord.cmp(&0)
    }
}
...
fn test_read_data() {
    ...
    unsafe {
        assert_eq!(parcel.try_read_strong_binder().err(), Some(Error::BAD_TYPE));
    }
