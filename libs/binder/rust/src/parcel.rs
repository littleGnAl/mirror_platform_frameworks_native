...
impl Clone for Parcel {
    fn clone(&self) -> Self {
        match self {
            // Cloning a borrowed `Parcel` gives us another borrow
            // of the same parcel. This is consistent with
            // `std::borrow::Cow` which is the closest type in the
            // standard Rust library to our design.
            Self::Borrowed(x) => Self::Borrowed(*x),
            owned @ Self::Owned(_) => {
                let mut new_parcel = Self::new();
                new_parcel
                    .append_all_from(owned)
