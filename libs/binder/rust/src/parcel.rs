...
pub use self::parcelable_holder::ParcelableHolder;
...
impl Parcel {
    ...
    /// size of the length element itself (4 bytes).
    ///
    /// # Examples
    ///
    ...
    pub fn try_new() -> Option<Parcel> {
        unsafe {
            // Safety: `Parcel::owned` checks whether `AParcel_create` returns
            // a null.
            Self::owned(sys::AParcel_create())
        }
    }
    ...
    pub fn append_from(&mut self, other: &Self, start: i32, size: i32) -> Result<()> {
        // `start` and `size` are `size_t` in libbinder, but `i32` in
        // libbinder_ndk and libbinder_rs. We need to explicitly check
        ...
        let status = unsafe {
            // Safety: We check that the start and size are positive above.
            // `Parcel::appendFrom` from C++ checks that the start and end
            // are in bounds, and returns an error otherwise. Both `self` and
            // `other` always contain valid pointers.
...
impl<'a> WritableSubParcel<'a> {
    ...
    /// Write a parcelable that implements [`Parcelable`] to the sub-parcel.
    pub(crate) fn write_parcelable<P: Parcelable + ?Sized>(
        &self,
        parcelable: &P,
    ) -> Result<()> {
        parcelable.serialize_parcelable(&mut *self.0.borrow_mut())
    }
}
...
impl std::fmt::Debug for Parcel {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_fmt(format_args!("Parcel[{}]", self.get_data_size()))
    }
}
...
#[test]
fn test_append_from() {
    let mut parcel1 = Parcel::try_new().unwrap();
    ...
    let mut parcel3 = Parcel::try_new().unwrap();
    assert_eq!(Ok(()), parcel3.append_from(&parcel1, 0, 2));
    assert_eq!(2, parcel3.get_data_size());
    unsafe {
        parcel2.set_data_position(0).unwrap();
    }
    assert_eq!(Err(StatusCode::NOT_ENOUGH_DATA), parcel3.read::<i32>());

    let mut parcel4 = Parcel::try_new().unwrap();
    assert_eq!(Err(StatusCode::BAD_VALUE), parcel4.append_from(&parcel1, 4, 2));
    assert_eq!(Err(StatusCode::BAD_VALUE), parcel4.append_from(&parcel1, 2, 4));
    assert_eq!(Err(StatusCode::BAD_VALUE), parcel4.append_from(&parcel1, -1, 4));
    assert_eq!(Err(StatusCode::BAD_VALUE), parcel4.append_from(&parcel1, 2, -1));
}
