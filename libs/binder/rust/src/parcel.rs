...
pub use self::parcelable::{Deserialize, DeserializeArray, Serialize, SerializeArray};
...
impl Parcel {
    ...
    pub fn get_data_position(&self) -> ParcelPosition {
        let position = unsafe {
    ...
    pub fn set_data_position(&self, pos: ParcelPosition) -> Result<()> {
        if !ptr::eq(pos.parcel, self.as_native()) {
            return Err(StatusCode::BAD_VALUE.into());
        }
        unsafe {
            // Safety: `Parcel` always contains a valid pointer to an `AParcel`,
            // and this call is otherwise safe. We check above that the new
            // position is one previously retrieved by `AParcel_getDataPosition`
            // for this parcel, which is required by
            // `AParcel_setDataPosition`. This ensures that users cannot
            // construct an arbitrary data position out of bounds of the parcel
            // data.
            status_result(sys::AParcel_setDataPosition(self.as_native(), pos.position))
        }
    }
}
...
impl Parcel {
    ...
    pub(crate) fn new_for_test(binder: &mut SpIBinder) -> Result<Self> {
        ...
        unsafe {
            ...
            // pointer.
            Parcel::owned(input).ok_or(StatusCode::UNEXPECTED_NULL.into())
        }
    }
}
...
fn test_read_write() {
    ...
    assert_eq!(
        parcel.read::<bool>(),
        Err(StatusCode::NOT_ENOUGH_DATA.into())
    );
    assert_eq!(parcel.read::<i8>(), Err(StatusCode::NOT_ENOUGH_DATA.into()));
    assert_eq!(
        parcel.read::<u16>(),
        Err(StatusCode::NOT_ENOUGH_DATA.into())
    );
    ...
    assert_eq!(
        parcel.read::<String>(),
        Err(StatusCode::UNEXPECTED_NULL.into())
    );

    assert_eq!(
        parcel.read_binder().err(),
        Some(StatusCode::BAD_TYPE.into())
    );
