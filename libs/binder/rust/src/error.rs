...
/// Convert a low-level status code integer into a binder result with a
/// high-level [`Status`] failure type.
...
impl Status {
    ...
    /// as a whole is okay, use [`is_ok`] instead.
    ...
    /// [`exception_code`] returns `ExceptionCode::SERVICE_SPECIFIC`. If this
    /// function returns 0, the status object may still represent a different
    /// exception or status. To find out if this transaction as a whole is okay,
    /// use [`is_ok`] instead.
    ...
    /// `self`.
    pub fn and_then<T, F>(self, op: F) -> Result<T>
    where
        F: FnOnce() -> Result<T>,
    {
        <Result<()>>::from(self)?;
