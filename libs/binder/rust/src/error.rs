...
pub trait IntoBinderResult<T, E> {
    ...
    /// message of the exception is set by formatting the error for display.
    ...
    /// is set by formatting the error for display.
...
impl<T, E: std::fmt::Display> IntoBinderResult<T, E> for result::Result<T, E> {
    fn or_binder_exception(self, exception: ExceptionCode) -> result::Result<T, Status> {
