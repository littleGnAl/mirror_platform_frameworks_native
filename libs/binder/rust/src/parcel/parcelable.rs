...
impl Serialize for Option<&CStr> {
    fn serialize(&self, parcel: &mut Parcel) -> Result<()> {
        match self {
            None => unsafe {
                // Safety: `Parcel` always contains a valid pointer to an
...
impl Serialize for Option<CString> {
    fn serialize(&self, parcel: &mut Parcel) -> Result<()> {
...
impl Deserialize for Option<CString> {
    fn deserialize(parcel: &Parcel) -> Result<Self> {
        ...
        vec.map(|mut s| {
            ...
            s.pop();
            CString::new(s).or(Err(StatusCode::BAD_VALUE.into()))
        })
        .transpose()
    }
}
...
impl Deserialize for Option<String> {
    fn deserialize(parcel: &Parcel) -> Result<Self> {
        ...
        c_str
            .map(|s| s.into_string().or(Err(StatusCode::BAD_VALUE.into())))
            .transpose()
    }
}
...
impl Deserialize for String {
    fn deserialize(parcel: &Parcel) -> Result<Self> {
        Deserialize::deserialize(parcel)
            .transpose()
            .unwrap_or(Err(StatusCode::UNEXPECTED_NULL.into()))
    }
}
...
impl<T: DeserializeArray> Deserialize for Option<Vec<T>> {
    fn deserialize(parcel: &Parcel) -> Result<Self> {
        DeserializeArray::deserialize_array(parcel)
    }
}
