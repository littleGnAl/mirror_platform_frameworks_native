...
/// Assert that the `T::UninitType` for the given type matches the size
/// and alignment of `T` so that they can be transmuted to each other.
macro_rules! assert_uninit_type {
    {$t:ty} => {
        static_assertions::const_assert_eq!(
            std::mem::size_of::<<$t as Deserialize>::UninitType>(),
            std::mem::size_of::<$t>()
        );
        static_assertions::const_assert_eq!(
            std::mem::align_of::<<$t as Deserialize>::UninitType>(),
            std::mem::align_of::<$t>()
        );
    }
}
pub(crate) use assert_uninit_type;
...
assert_uninit_type!(Strong<AssertIBinder>);
