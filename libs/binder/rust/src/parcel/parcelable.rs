...
unsafe fn vec_assume_init<T: Deserialize>(vec: Vec<T::UninitType>) -> Vec<T> {
    // We can convert from Vec<T::UninitType> to Vec<T> because MaybeUninit<T>
...
const_assert_eq!(mem::size_of::<<Status as Deserialize>::UninitType>(), mem::size_of::<Status>());
...
const_assert_eq!(
    mem::size_of::<<Strong<AssertIBinder> as Deserialize>::UninitType>(),
    mem::size_of::<Strong<AssertIBinder>>()
);
...
const_assert_eq!(mem::size_of::<<Box<u8> as Deserialize>::UninitType>(), mem::size_of::<Box<u8>>());
