...
use super::{Deserialize, DeserializeArray, Parcel, Serialize, SerializeArray};
...
impl Serialize for Option<ParcelFileDescriptor> {
    fn serialize(&self, parcel: &mut Parcel) -> Result<()> {
        if let Some(f) = self {
            f.serialize(parcel)
        } else {
...
impl Deserialize for Option<ParcelFileDescriptor> {
