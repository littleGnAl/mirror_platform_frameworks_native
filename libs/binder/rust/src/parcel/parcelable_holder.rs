...
use crate::parcel::{Deserialize, Parcel, Parcelable, Serialize};
...
trait AnyHoldable: Downcast + Serialize + std::fmt::Debug {}
impl_downcast!(AnyHoldable);
impl<T: Downcast + Serialize + std::fmt::Debug> AnyHoldable for T {}
...
impl ParcelableHolder {
    ...
    /// TODO
    pub fn set_parcelable<T: Any + Parcelable>(&mut self, p: Option<Rc<T>>) -> Result<()> {
        if let Some(p) = p {
    ...
    /// TODO
    pub fn get_parcelable<T: Any + Parcelable>(&self) -> Result<Option<Rc<T>>> {
        ...
        match *data {
            ...
            ParcelableHolderData::Parcel(ref parcel) => {
                ...
                let parcelable: Rc<T> = Rc::new(parcel.read()?);
