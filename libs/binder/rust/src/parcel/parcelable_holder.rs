...
impl<T: Downcast + Parcelable + std::fmt::Debug> AnyParcelable for T {}
...
impl ParcelableHolder {
    ...
    pub fn set_parcelable<T: Any + Parcelable + ParcelableMetadata + std::fmt::Debug>(
        &mut self,
        p: Rc<T>,
    ) -> Result<()> {
    ...
    pub fn get_parcelable<T: Any + Parcelable + ParcelableMetadata + Default + std::fmt::Debug>(
        &self,
    ) -> Result<Option<Rc<T>>> {
...
impl Clone for ParcelableHolder {
    fn clone(&self) -> Self {
        let new_data = match *self.data.borrow() {
            ...
            ParcelableHolderData::Parcel(ref parcel) => {
                let mut new_parcel = Parcel::try_new().expect("Failed to allocate parcel");
                new_parcel
                    .append_all_from(parcel)
...
impl Parcelable for ParcelableHolder {
    ...
    fn read_from_parcel(&mut self, parcel: &Parcel) -> Result<()> {
        ...
        let mut new_parcel = Parcel::try_new().ok_or(StatusCode::NO_MEMORY)?;
