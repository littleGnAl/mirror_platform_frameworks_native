...
class RpcTlsTransportServerCtx;
class RpcTlsTransportClientCtx;
...
int socketRead(BIO *bio, char *buf, int size) {
    ...
    return ::recv(fd.get(), buf, size, MSG_NOSIGNAL);
}

int socketWrite(BIO *bio, const char *buf, int size) {
    android::base::borrowed_fd fd(static_cast<int>(reinterpret_cast<intptr_t>(BIO_get_data(bio))));
    return ::send(fd.get(), buf, size, MSG_NOSIGNAL);
}
...
private:
    friend RpcTlsTransportServerCtx;
    friend RpcTlsTransportClientCtx;
...
class RpcTlsTransportServerCtx : public RpcTransportServerCtx {
public:
    using RpcTransportServerCtx::RpcTransportServerCtx;
    static std::unique_ptr<RpcTlsTransportServerCtx> create();
    std::unique_ptr<RpcTransport> sslAccept(android::base::unique_fd acceptedFd) override {
        ...
        auto rpcTlsTransport =
                std::unique_ptr<RpcTlsTransport>(new RpcTlsTransport(std::move(acceptedFd)));
...
class RpcTlsTransportClientCtx : public RpcTransportClientCtx {
public:
    using RpcTransportClientCtx::RpcTransportClientCtx;
    static std::unique_ptr<RpcTlsTransportClientCtx> create();
    std::unique_ptr<RpcTransport> sslConnect(android::base::unique_fd connectedFd) override {
        // TODO(b/190868302): certificate should be sent in a different channel, and verify it.
        auto ssl = connectAccept(mCtx.get(), connectedFd, &SSL_connect, "SSL_connect");
        auto rpcTlsTransport =
                std::unique_ptr<RpcTlsTransport>(new RpcTlsTransport(std::move(connectedFd)));
        rpcTlsTransport->mSsl = std::move(ssl);
        return rpcTlsTransport;
    }
...
std::unique_ptr<RpcTlsTransportServerCtx> RpcTlsTransportServerCtx::create() {
    ...
    auto rpcTlsTransportServerCtx =
            std::unique_ptr<RpcTlsTransportServerCtx>(new RpcTlsTransportServerCtx());
    rpcTlsTransportServerCtx->mCtx = std::move(ctx);

    return rpcTlsTransportServerCtx;
}

std::unique_ptr<RpcTlsTransportClientCtx> RpcTlsTransportClientCtx::create() {
    bssl::UniquePtr<SSL_CTX> ctx(SSL_CTX_new(TLS_method()));
    ...
    auto rpcTlsTransportClientCtx =
            std::unique_ptr<RpcTlsTransportClientCtx>(new RpcTlsTransportClientCtx());
    rpcTlsTransportClientCtx->mCtx = std::move(ctx);

    return rpcTlsTransportClientCtx;
}

#endif // BINDER_ENABLE_TLS

class RpcRawTransport;
class RpcRawTransportServerCtx;
class RpcRawTransportClientCtx;
...
private:
    friend RpcRawTransportServerCtx;
    friend RpcRawTransportClientCtx;
};

class RpcRawTransportServerCtx : public RpcTransportServerCtx {
public:
    using RpcTransportServerCtx::RpcTransportServerCtx;
    std::unique_ptr<RpcTransport> sslAccept(android::base::unique_fd acceptedFd) override;
};
class RpcRawTransportClientCtx : public RpcTransportClientCtx {
public:
    using RpcTransportClientCtx::RpcTransportClientCtx;
...
std::unique_ptr<RpcTransport> RpcRawTransportServerCtx::sslAccept(
...
std::unique_ptr<RpcTransport> RpcRawTransportClientCtx::sslConnect(
...
std::unique_ptr<RpcTransportServerCtx> RpcTransportServerCtx::create(bool tls) {
#ifdef BINDER_ENABLE_TLS
...
std::unique_ptr<RpcTransportClientCtx> RpcTransportClientCtx::create(bool tls) {
#ifdef BINDER_ENABLE_TLS
