...
bool convertSharedFileRegionToIMemory(const os::SharedFileRegion& shmem,
    ...
    int64_t endOffset = shmem.offset + shmem.size;

    // Round down to page boundary.
    int64_t heapStartOffset = shmem.offset & pageMask;
    // Round up to page boundary.
    int64_t heapEndOffset = (endOffset + pageSize - 1) & pageMask;
    int64_t heapSize = heapEndOffset - heapStartOffset;
...
bool convertIMemoryToSharedFileRegion(const sp<IMemory>& mem,
    ...
    if (heap != nullptr) {
        ...
        if (size > std::numeric_limits<int64_t>::max() ||
                ...
                static_cast<int64_t>(heap->getOffset()) + offset
