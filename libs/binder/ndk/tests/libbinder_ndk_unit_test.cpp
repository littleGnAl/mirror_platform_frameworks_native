...
TEST(NdkBinder, DeathRecipient) {
    ...
    EXPECT_EQ(STATUS_OK, AIBinder_linkToDeathWithCleanup(
                                 binder, recipient, static_cast<void*>(&cookie), LambdaOnUnlink));
    ...
    EXPECT_TRUE(deathCv.wait_for(lockDeath, 1s, [&] { return deathRecieved; }));
    EXPECT_TRUE(deathRecieved);

    std::unique_lock<std::mutex> lockUnlink(unlinkMutex);
    EXPECT_TRUE(deathCv.wait_for(lockUnlink, 1s, [&] { return unlinkRecieved; }));
    EXPECT_TRUE(unlinkRecieved);
