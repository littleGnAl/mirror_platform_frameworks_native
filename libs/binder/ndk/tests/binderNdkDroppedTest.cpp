...
static constexpr size_t kSends = 4;

class MyBinderNdkDroppedTest : public aidl::BnBinderNdkDroppedTest {
   public:
    std::atomic<size_t> sendCount = 0;

    ::ndk::ScopedAStatus send(const std::shared_ptr<aidl::IBinderNdkDroppedTest>& server) {
        sendCount++;
        EXPECT_NE(nullptr, server);

        if (server == nullptr) {
            EXPECT_NE(nullptr, server->asBinder().get());
        }

        std::cout << "server got binder i " << sendCount << std::endl;
        if (sendCount <= kSends / 2) sleep(1);
