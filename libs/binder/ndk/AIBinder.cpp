...
AIBinder::AIBinder(const AIBinder_Class* clazz, AIBinder_Class_Impl impl,
                   ::android::sp<::android::IBinder> binder)
      : mIsRemote(binder->remoteBinder() != nullptr), mClazz(clazz), mImpl(impl), mBinder(binder) {}

AIBinder::~AIBinder() {
    mClazz->onDestroy(mImpl);
}
...
AIBinder* AIBinder::newLocalBinder(const AIBinder_Class* clazz, AIBinder_Class_Impl impl) {
    ...
    sp<AIBinder> ret = new AIBinder(clazz, impl, localBinder);
...
AIBinder* AIBinder::newFromBinder(sp<IBinder> binder) {
    ...
    // be handled for the future. So, we still need a way to distinguish a LocalNdkBinder from, say,
    // a Bp* which is traditionally created by AIDL.
    LocalNdkBinder* localBinder = static_cast<LocalNdkBinder*>(binder->localBinder());
    if (localBinder != nullptr) {
        ret = localBinder->getAIBinder().promote();
    } else {
        // This is a remote object
        ret = new AIBinder(nullptr /*clazz*/, nullptr /*impl*/, binder);
    }
...
void AIBinder_incStrong(AIBinder* binder) {
    if (binder == nullptr) {
        LOG(ERROR) << "incStrong on null binder";
        return;
    }
...
void AIBinder_decStrong(AIBinder* binder) {
    if (binder == nullptr) {
        LOG(ERROR) << "decStrong on null binder";
        return;
    }
