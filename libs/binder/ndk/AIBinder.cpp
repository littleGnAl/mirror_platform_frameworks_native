...
sp<AIBinder> AIBinder::associateClass(const AIBinder_Class* clazz) {
    ...
    if (mClazz != nullptr) {
        ...
        if (newDescriptor == currentDescriptor) {
            LOG(ERROR) << "Class descriptors '" << currentDescriptor
                       << "' match during associateClass, but they are different class objects. "
                          "Class descriptor collision?";
            return nullptr;
        }

        LOG(ERROR) << "Class cannot be associated on object which already has a class. Trying to "
                      "associate to '"
...
binder_status_t AIBinder_finalizeTransaction(const AIBinder* binder, AParcel* out) {
    ::android::finalizeRemoteNdkBinderTransaction(binder, out);
