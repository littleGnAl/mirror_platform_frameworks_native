...
class PersistableBundle {
   public:
    ...
    PersistableBundle(APersistableBundle* bundle) noexcept : mPBundle(bundle) {}
    // takes ownership of the APersistableBundle*
    ...
    // duplicate the inStr into the outStr
    static bool strDupAllocator(char** outStr, size_t bufferSizeBytes, const char* inStr) {
        if (outStr) {
            *outStr = strndup(inStr, bufferSizeBytes);
            if (*outStr) return true;
        }
        return false;
    }
    ...
    template <typename T>
    bool getVecInternal(size_t (*getVec)(const APersistableBundle* _Nonnull, const char*,
                                         T* _Nullable, size_t),
                        const APersistableBundle* pBundle, const char* key,
    ...
    template <typename T>
    T moveStringsInternal(char** strings, size_t bufferSizeBytes) {
        if (strings && bufferSizeBytes > 0) {
            size_t num = bufferSizeBytes / sizeof(char*);
            T ret;
            for (size_t i = 0; i < num; i++) {
    ...
    bool getStringVector(const std::string& key, std::vector<std::string>* _Nonnull vec) {
        ssize_t bytes = APersistableBundle_getStringVector(mPBundle, key.c_str(), nullptr, 0,
                                                           &strDupAllocator);
        if (bytes > 0) {
    ...
    std::set<std::string> getKeys(
            ssize_t (*getTypedKeys)(const APersistableBundle* _Nonnull pBundle,
                                    char* _Nullable* _Nullable outKeys, size_t bufferSizeBytes,
                                    bool (*_Nonnull strDupAllocator)(char* _Nullable* _Nullable,
                                                                     size_t, const char* _Nonnull)),
            const APersistableBundle* _Nonnull pBundle) {
        // call first with nullptr to get required size in bytes
        ssize_t bytes = getTypedKeys(pBundle, nullptr, 0, &strDupAllocator);
        if (bytes > 0) {
            char** keys = (char**)malloc(bytes);
            if (keys) {
                bytes = getTypedKeys(pBundle, keys, bytes, &strDupAllocator);
