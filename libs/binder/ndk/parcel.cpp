...
template <typename T>
using ArrayGetter = T (*)(void* arrayData, size_t index);
template <typename T>
using ArraySetter = void (*)(void* arrayData, size_t index, T value);

template <typename T>
binder_status_t WriteArray(AParcel* parcel, T* array, size_t length, status_t (Parcel::*write)(T)) {
    ...
    for (size_t i = 0; i < length; i++) {
        status = (rawParcel->*write)(array[i]);
...
template <typename T>
binder_status_t WriteArray(AParcel* parcel, void* arrayData, ArrayGetter<T> getter, size_t length,
                           status_t (Parcel::*write)(T)) {
    if (length > std::numeric_limits<int32_t>::max()) return STATUS_BAD_VALUE;
...
binder_status_t AParcel_writeInt32Array(AParcel* parcel, int32_t* value, size_t length) {
    return WriteArray<int32_t>(parcel, value, length, &Parcel::writeInt32);
}

binder_status_t AParcel_writeUint32Array(AParcel* parcel, uint32_t* value, size_t length) {
    return WriteArray<uint32_t>(parcel, value, length, &Parcel::writeUint32);
}

binder_status_t AParcel_writeInt64Array(AParcel* parcel, int64_t* value, size_t length) {
    return WriteArray<int64_t>(parcel, value, length, &Parcel::writeInt64);
}

binder_status_t AParcel_writeUint64Array(AParcel* parcel, uint64_t* value, size_t length) {
    return WriteArray<uint64_t>(parcel, value, length, &Parcel::writeUint64);
}

binder_status_t AParcel_writeFloatArray(AParcel* parcel, float* value, size_t length) {
    return WriteArray<float>(parcel, value, length, &Parcel::writeFloat);
}

binder_status_t AParcel_writeDoubleArray(AParcel* parcel, double* value, size_t length) {
    return WriteArray<double>(parcel, value, length, &Parcel::writeDouble);
}

binder_status_t AParcel_writeBoolArray(AParcel* parcel, void* arrayData,
                                       AParcel_bool_array_getter getter, size_t length) {
    return WriteArray<bool>(parcel, arrayData, getter, length, &Parcel::writeBool);
}

binder_status_t AParcel_writeCharArray(AParcel* parcel, char16_t* value, size_t length) {
    return WriteArray<char16_t>(parcel, value, length, &Parcel::writeChar);
}

binder_status_t AParcel_writeByteArray(AParcel* parcel, int8_t* value, size_t length) {
    return WriteArray<int8_t>(parcel, value, length, &Parcel::writeByte);
}
...
binder_status_t AParcel_readCharArray(const AParcel* parcel, void** arrayData,
    ...
    return ReadArray<char16_t>(parcel, arrayData, reallocator, getter, &Parcel::readChar);
}

binder_status_t AParcel_readByteArray(const AParcel* parcel, void** arrayData,
                                      AParcel_array_reallocator reallocator,
                                      AParcel_byte_array_getter getter) {
    return ReadArray<int8_t>(parcel, arrayData, reallocator, getter, &Parcel::readByte);
}
