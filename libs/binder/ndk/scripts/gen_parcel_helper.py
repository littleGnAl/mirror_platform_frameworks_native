...
def main():
    ...
    for pretty, cpp in data_types:
        ...
        if nca:
            ...
            header += " * \\param arrayData an array of length length.\n"
            header += " * \\param length the length of arrayData.\n"
        header += " *\n"
        header += " * \\return STATUS_OK on successful write.\n"
        header += " */\n"
    ...
    for pretty, cpp in data_types:
        ...
        if nca:
            ...
            pre_header += " * \\param length the length to allocate arrayData to.\n"
            pre_header += " *\n"
            pre_header += " * \\return a buffer of " + cpp + " of length length.\n"
            pre_header += " */\n"
        ...
        header += " * \\param parcel the parcel to read from.\n"
        header += " * \\param arrayData some external representation of an array.\n"
        header += " * \\param allocator the callback which will be called to allocate the array.\n"
        if nca:
            header += " * \\param setter the callback which will be called to set a value at a specific location in the array.\n"
        header += " *\n"
        header += " * \\return STATUS_OK on successful read.\n"
        header += " */\n"
