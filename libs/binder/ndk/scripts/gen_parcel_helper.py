...
def main():
    ...
    for pretty, cpp in data_types:
        ...
        cpp_helper += " */\n"
        cpp_helper += "template<>\n"
        cpp_helper += "static inline binder_status_t AParcel_writeVector<" + cpp + ">(AParcel* parcel, const std::vector<" + cpp + ">& vec) {\n"
        write_args = "vec.data()"
        if nca: write_args = "static_cast<const void*>(&vec), AParcel_std_vector_getter<" + cpp + ">"
        ...
        cpp_helper += " */\n"
        cpp_helper += "template<>\n"
        cpp_helper += "static inline binder_status_t AParcel_readVector<" + cpp + ">(const AParcel* parcel, std::vector<" + cpp + ">* vec) {\n"
        cpp_helper += "    void* vectorData = static_cast<void*>(vec);\n"
        read_args = []
        read_args += ["parcel"]
        read_args += ["&vectorData"]
        read_args += ["&AParcel_std_vector_reallocator<" + cpp + ">"]
        read_args += ["AParcel_std_vector_" + read_using + "<" + cpp + ">"]
