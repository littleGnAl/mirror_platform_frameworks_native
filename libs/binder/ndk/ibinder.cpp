...
bool AIBinder::associateClass(const AIBinder_Class* clazz) {
    ...
    // of this is that no API can add functionality to the proxy class. The functionality should
    // be implemented by adding AIBinder APIs, by setting things on AIBinder_Class which get
    // transferred along with the binder, so that they can be read along with the BpBinder, or by
    // modifying APIs directly (e.g. an option in onTransact).
    //
...
// WARNING: When multiple classes exist with the same inteface descriptor in different
