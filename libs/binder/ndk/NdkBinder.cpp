...
binder_status_t initRemoteNdkBinderTransaction(const AIBinder* binder, AParcel* in) {
    ...
    if (clazz == nullptr) {
        LOG(ERROR) << "Class must be defined for a remote binder transaction. See "
...
binder_status_t finalizeRemoteNdkBinderTransaction(const AIBinder* binder, AParcel* out) {
    const AIBinder_Class* clazz = binder->getClass();
    ...
    // FIXME: add status to the out parcel?
