...
binder_status_t IFoo::addService(const char* instance) {
    ...
    if (mWeakBinder != nullptr) {
        ...
        binder = AWeak_AIBinder_promote(mWeakBinder);
    }
    if (binder == nullptr) {
        ...
        mWeakBinder = AWeak_AIBinder_new(binder);
    }

    binder_status_t status = AIBinder_addService(binder, instance);
...
sp<IFoo> IFoo::getService(const char* instance) {
    ...
    AIBinder* held = AWeak_AIBinder_promote(ret->mWeakBinder);
    CHECK(held == binder);
