...
TEST(DoubleBinder, MakeSimultaneousCalls) {
    for (const std::string& serviceName :
         {kLocalCppServerName, kLocalNdkServerName, kRemoteNdkServerName, kRemoteCppServerName}) {
        for (size_t i = 0; i < 100; i++) {
            ...
            {
                sp<IBinder> binder =
                        defaultServiceManager()->getService(String16(serviceName.c_str()));
                ASSERT_NE(nullptr, binder.get()) << serviceName;
