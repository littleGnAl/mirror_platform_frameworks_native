...
class ICInterface : public SharedRefBase {
   public:
     ...
     */
    virtual inline binder_status_t shellCommand(int /*in*/, int /*out*/, int /*err*/,
...
class BpCInterface : public INTERFACE {
   public:
    ...
    binder_status_t shellCommand(int in, int out, int err, const char** argv,
                                 uint32_t argc) override {
        return AIBinder_shellCommand(asBinder().get(), in, out, err, argc, argc);
    }
...
binder status_t ICInterface::shellCommand(int /*in*/, int /*out*/, int /*err*/,
                                          const char** /*argv*/, uint32_t /*argc*/) {
    return STATUS_OK;
}
...
AIBinder_Class* ICInterface::defineClass(const char* interfaceDescriptor,
    ...
    // We can't know if these methods are overriden by a subclass interface, so we must register
...
binder status_t ICInterface::ICInterfaceData::handleShellCommand(AIBinder* binder, int in, int out,
