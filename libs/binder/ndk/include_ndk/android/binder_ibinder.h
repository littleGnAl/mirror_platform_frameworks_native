 ...
 * transaction has FLAG_ONEWAY, this will only include a binder_status_t result parceled (and can by
 * definition contain no data from the remote process). Otherwise, this will block until the remote
 * process has processed the transaction, and the out parcel will contain the output data from
 * transaction.
 *
 * This does not affect the ownership of binder.
 */
