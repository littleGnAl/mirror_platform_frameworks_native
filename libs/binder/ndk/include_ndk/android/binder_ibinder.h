...
/**
 * This tells users of this class not to write a transaction header. By default,
 * libbinder_ndk users write transaction headers implicitly (in the SDK, this
 * must be manually written by android.os.Parcel#writeInterfaceToken). In order
 * to talk to legacy code which does not write an interface token, this method
 * is provided. When this is disabled, type safety is reduced, so you must have
 * a separate way of determining the binder you are talking to is the right
 * type. Must be called before any instance of the class is created.
 *
