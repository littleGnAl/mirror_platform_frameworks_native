 ...
 * when two AIBinder pointers point to the same address, they represent the same object. This
 * correspondance can be broken accidentally if AIBinder_new is erronesouly called to create the
 ...
 *
 * Once an AIBinder object is created using this API, this API shouldn't be called again unless the
 * original AIBinder object has been destroyed. If this is done, pointer equality of AIBinder
 * objects is not guaranteed to be preserved. So, for instance, if someone erroneously created two
 * of these objects and passed one to a hypothetical addCallback function and then later another one
 * to a hypothetical removeCallback function, the remote process would have no way to determine that
 * these two objects are actually equal using the AIBinder pointer alone (which they should be able
 * to do). Please also see the suggested memory ownership model suggested above.
 */
 ...
 * null. The cookie can also be used to unlink this transaction (see AIBinder_linkToDeath). This
 * function may return a binder transaction failure.
 */
