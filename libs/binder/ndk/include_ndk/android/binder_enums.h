...
constexpr aidl_invalid_type<EnumType> enum_values;
}  // namespace details
...
struct enum_range {
     ...
     */
    constexpr auto begin() const { return std::begin(details::enum_values<EnumType>); }
    /**
     * Return an iterator pointing to one past the last enum value.
     */
    constexpr auto end() const { return std::end(details::enum_values<EnumType>); }
};
