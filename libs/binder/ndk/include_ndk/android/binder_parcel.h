 ...
 * \param size Output parameter that will store the length (number of elements) that will
 * be used to resize the vector on success, or -1 on falure.
 * \return STATUS_OK on successful read. STATUS_NO_MEMORY if the allocation is
 * unreasonably large.
 */
