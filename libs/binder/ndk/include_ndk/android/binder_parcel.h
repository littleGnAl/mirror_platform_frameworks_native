 ...
 * AParcel_getPosition. Positions are constant for a given parcel between processes.
 *
 * \return STATUS_OK on success. If position is larger than INT32_MAX, then STATUS_BAD_VALUE will be
 * returned.
 */
binder_status_t AParcel_setPosition(const AParcel* parcel, size_t position) __INTRODUCED_IN(29);
 ...
 *
 * \return The size of the parcel. This will always be less than INT32_MAX. The values returned by
 * this function before and after calling various reads and writes are not defined. Only the delta
