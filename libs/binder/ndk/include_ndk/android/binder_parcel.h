 ...
 *
 * In order to read the string, data must be copied from string into data where data is the argument
 * to AParcel_readString.
 *
 * If allocation fails, null should be returned.
 */
typedef void (*AParcel_string_allocator)(void* data, const char* string, int32_t length);

/**
 * Reads and allocates string value from the next location in a non-null parcel.
 *
 * Data is passed to the string allocator to be filled when the size is known.
 */
binder_status_t AParcel_readString(const AParcel* parcel, AParcel_string_allocator allocator,
                                   void* data) __INTRODUCED_IN(29);
