...
status_t RpcJoinThread::initialize(const sp<IBinder>& rootObject, size_t maxRpcThreads,
    ...
    auto pipe = RpcServer::Pipe::make();
    if (pipe == nullptr) {
        ALOGE("No pipe: %s", strerror(errno));
        return NO_MEMORY;
    }

    LOG_ALWAYS_FATAL_IF(mRpcServer != nullptr, "already initialized");
    LOG_ALWAYS_FATAL_IF(mPipe != nullptr, "pipe was already set up?!");
...
void RpcJoinThread::run() {
    ...
    mRpcServer->join(mPipe.get());
    LOG_RPC_DETAIL("%s: RpcServer::join() exits, thread stopping", __PRETTY_FUNCTION__);
}

RpcJoinThread::~RpcJoinThread() {
    if (mPipe) {
        LOG_RPC_DETAIL("%s: shutting down pipe to signal join() to stop", __PRETTY_FUNCTION__);
        mPipe->shutDown();
    }
