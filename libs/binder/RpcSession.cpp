...
status_t RpcSession::setupPreconnectedClient(unique_fd fd, std::function<unique_fd()>&& request) {
    // Why passing raw fd? When fd is passed as reference, Clang analyzer seems to incorrectly think
    // that the variable `fd` is a moved-from object. To work-around the issue, unwrap the raw fd
    // from the outer `fd`, pass the raw fd by value to the lambda, and then finally wrap it in
    // unique_fd inside the lambda.
