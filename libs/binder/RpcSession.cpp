...
sp<RpcSession> RpcSession::make(std::unique_ptr<RpcTransportCtxFactory> rpcTransportCtxFactory) {
    ...
    if (rpcTransportCtxFactory == nullptr)
        rpcTransportCtxFactory = std::make_unique<RpcTransportCtxFactoryRaw>();
...
bool RpcSession::addNullDebuggingClient() {
    LOG_ALWAYS_FATAL_IF(mRpcTransportCtxFactory->tlsEnabled(),
...
status_t RpcSession::FdTrigger::interruptableWriteFully(RpcTransport* rpcTransport,
    ...
    while ((status = triggerablePoll(rpcTransport, POLLOUT)) == OK) {
        ssize_t writeSize = TEMP_FAILURE_RETRY(rpcTransport->send(buffer, end - buffer));
