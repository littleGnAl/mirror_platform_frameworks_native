...
status_t RpcSession::setupPreconnectedClient(base::unique_fd fd,
    ...
    return setupClient([&](const std::vector<uint8_t>& sessionId, bool incoming) -> status_t {
        ...
        RpcSocket socket(std::move(fd));
        status_t status = initAndAddConnection(std::move(socket), sessionId, incoming);
...
status_t RpcSession::addNullDebuggingClient() {
    ...
    RpcSocket socket(std::move(serverFd));
    auto server = mCtx->newTransport(std::move(socket), mShutdownTrigger.get());
    if (server == nullptr) {
...
status_t RpcSession::setupOneSocketConnection(const RpcSocketAddress& addr,
    ...
    for (size_t tries = 0; tries < 5; tries++) {
        ...
        if (0 != TEMP_FAILURE_RETRY(connect(socket.fd.get(), addr.addr(), addr.addrSize()))) {
            int connErrno = errno;
            if (connErrno == EAGAIN || connErrno == EINPROGRESS) {
                ...
                status_t pollStatus = mShutdownTrigger->triggerablePoll(socket, POLLOUT);
                if (pollStatus != OK) {
                ...
                int ret = getsockopt(socket.fd.get(), SOL_SOCKET, SO_ERROR, &connErrno,
                                     &connErrnoLen);
                if (ret == -1) {
                    int savedErrno = errno;
        ...
        LOG_RPC_DETAIL("Socket at %s client with fd %d", addr.toString().c_str(), socket.fd.get());

        return initAndAddConnection(std::move(socket), sessionId, incoming);
    }
...
status_t RpcSession::initAndAddConnection(RpcSocket fd, const std::vector<uint8_t>& sessionId,
                                          bool incoming) {
