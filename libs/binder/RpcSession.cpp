...
public:
    JavaThreadAttacher() {
        ...
        // libbinder.
        auto sym = dlsym(RTLD_DEFAULT, "androidJavaAttachThread");
        if (sym == nullptr) return;
        auto attachFn = reinterpret_cast<decltype(&androidJavaAttachThread)>(sym);
    ...
    ~JavaThreadAttacher() {
        if (!mAttached) return;
        auto sym = dlsym(RTLD_DEFAULT, "androidJavaDetachThread");
        LOG_ALWAYS_FATAL_IF(sym == nullptr,
                            "androidJavaAttachThread exists but androidJavaDetachThread doesn't");
        auto detachFn = reinterpret_cast<decltype(&androidJavaDetachThread)>(sym);
