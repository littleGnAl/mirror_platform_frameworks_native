...
bool RpcSession::addNullDebuggingClient() {
    ...
    auto ctx = RpcTransportClientCtx::create(false /* enable TLS */);
    if (ctx == nullptr) {
        ALOGE("Unable to set up RpcTransportClientCtx");
        return false;
    }
    auto server = ctx->sslConnect(std::move(serverFd));
...
bool RpcSession::setupOneSocketConnection(const RpcSocketAddress& addr, const RpcAddress& id,
    ...
    for (size_t tries = 0; tries < 5; tries++) {
        ...
        auto ctx = RpcTransportClientCtx::create();
        if (ctx == nullptr) {
            ALOGE("Unable to set up RpcTransportClientCtx");
            return false;
        }
        auto server = ctx->sslConnect(std::move(serverFd));
        if (server == nullptr) {
            ALOGE("Could not connect because sslConnect fails at %s", addr.toString().c_str());
            return false;
        }
