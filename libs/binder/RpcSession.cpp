...
status_t RpcSession::setupClient(const std::function<status_t(const std::vector<uint8_t>& sessionId,
    ...
    if (outgoingThreads != numThreadsAvailable) {
        ...
        // start less threads
        ALOGE("Server hints client to start %zu outgoing threads with RpcServer::setMaxThreads, "
              "but client will only start %zu because it is preconfigured to start at most %zu "
              "outgoing threads.",
