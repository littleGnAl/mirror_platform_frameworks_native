...
status_t IBinder::setRpcClient(android::base::unique_fd socketFd, uint32_t maxRpcThreads) {
    ...
    BBinder* local = this->localBinder();
    if (local != nullptr) {
        return local->BBinder::setRpcClient(std::move(socketFd), maxRpcThreads);
    }
...
status_t BBinder::setRpcClient(const Parcel& data) {
    ...
    return setRpcClient(std::move(clientFd), maxRpcThreads);
}

status_t BBinder::setRpcClient(android::base::unique_fd socketFd, uint32_t maxRpcThreads) {
    if constexpr (!kEnableRpcDevServers) {
        ALOGW("%s: disallowed because RPC is not enabled", __PRETTY_FUNCTION__);
        return INVALID_OPERATION;
    }
    ...
    rpcServer->setRootObjectWeak(wp<BBinder>::fromExisting(this));
