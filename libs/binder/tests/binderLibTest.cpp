    ...
    virtual status_t onTransact(uint32_t code, const Parcel &data, Parcel *reply,
        ...
        switch (code) {
            ...
            case BINDER_LIB_TEST_CREATE_TEST_SERVICE: {
                ...
                reply->writeStrongBinder(sp<BinderLibTestServiceBase>::make(id));
                return NO_ERROR;
            }
        }
        return UNKNOWN_TRANSACTION;
...
TEST_P(BinderLibRpcClientTest, Test) {
    ...
    auto id = 0xC0FFEE00 + numThreads;
    sp<IBinder> server = isRemote ? sp<IBinder>(CreateRemoteService(id))
                                  : sp<IBinder>(sp<BinderLibTestServiceBase>::make(id));
    ASSERT_EQ(isRemote, server->isRemote());
