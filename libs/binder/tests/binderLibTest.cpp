...
TEST_F(BinderLibTest, RpcClient) {
    ...
    // FIXME this doesn't work on rpcServer
    auto getId = [](sp<IBinder> server, const char* name) {
        int32_t result = 0;
        Parcel data, reply;
        EXPECT_EQ(OK, server->transact(BINDER_LIB_TEST_GET_ID_TRANSACTION, data, &reply))
                << "for " << name << " server";
        EXPECT_EQ(OK, reply.readInt32(&result)) << "for " << name << " server";
        return result;
    };
    EXPECT_EQ(getId(server, "binder"), getId(rpcServer, "rpc"));
}
