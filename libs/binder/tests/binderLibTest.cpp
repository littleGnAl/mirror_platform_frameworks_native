...
TEST_F(BinderLibTest, TooManyFdsFlattenable) {
    rlimit nofile_orig;
    int ret = getrlimit(RLIMIT_NOFILE, &nofile_orig);
    ASSERT_EQ(0, ret);

    // Restore the original file limits when the test finishes
    base::ScopeGuard guardUnguard([&]() { setrlimit(RLIMIT_NOFILE, &nofile_orig); });

    rlimit nofile_new = {1024, 1024};
    ret = setrlimit(RLIMIT_NOFILE, &nofile_new);
    ASSERT_EQ(0, ret);
