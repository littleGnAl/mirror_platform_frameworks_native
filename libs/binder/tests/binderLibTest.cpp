...
// e.g. Result<int32_t> v = 0; EXPECT_THAT(result, HasResult(0));
MATCHER_P(HasResult, resultMatcher, "") {
    if (!arg.ok()) {
...
TEST_F(BinderLibTest, GetId) {
    EXPECT_THAT(GetId(m_server), HasResult(0));
}
...
public:
    ...
    SocketPort CreateSocket() {
        auto rpcServer = RpcServer::make();
        if (rpcServer == nullptr) return {};
        rpcServer->iUnderstandThisCodeIsExperimentalAndIWillNotUseItInProduction();
        SocketPort ret;
        if (!rpcServer->setupInetServer(0, &ret.port)) return {};
        ret.socket = rpcServer->releaseServer();
        return ret;
    }
};
...
TEST_P(BinderLibRpcTest, SetRpcClientNoFd) {
    ...
    ASSERT_TRUE(binder != nullptr);
    EXPECT_THAT(binder->setRpcClient(android::base::unique_fd()), StatusEq(BAD_VALUE));
}

TEST_P(BinderLibRpcTest, SetRpcMaxThreadsWithoutClient) {
    auto binder = GetService();
    ASSERT_TRUE(binder != nullptr);
    EXPECT_THAT(binder->setRpcMaxThreads(1), StatusEq(NO_INIT));
}
...
TEST_P(BinderLibRpcTest, SetRpcMaxThreadsZero) {
    ...
    EXPECT_THAT(binder->setRpcClient(std::move(socket)), StatusEq(OK));
    EXPECT_THAT(binder->setRpcMaxThreads(0), StatusEq(BAD_VALUE));
}

INSTANTIATE_TEST_CASE_P(BinderLibTest, BinderLibRpcTest, testing::Bool(),
                        BinderLibRpcTest::ParamToString);

class BinderLibTestServiceBase : public BBinder {
public:
    explicit BinderLibTestServiceBase(int32_t id) : m_id(id){};
...
TEST_P(BinderLibRpcClientTest, Test) {
    ...
    sp<IBinder> server = isRemote ? sp<IBinder>(addServer())
                                  : sp<IBinder>(sp<BinderLibTestServiceBase>::make(0xDEADBEEF));
    auto numThreads = numThreadsParam; // ... to be captured in lambda
    ...
    // Fake servicedispatcher.
    {
        ...
        ASSERT_TRUE(socket.ok());
        port = socketPort;
        ASSERT_THAT(server->setRpcClient(std::move(socket)), StatusEq(OK));
        ASSERT_THAT(server->setRpcMaxThreads(numThreads), StatusEq(OK));
    }
    ...
    auto threadFn = [&](size_t threadNum) {
        ...
        EXPECT_THAT(GetId(rpcServerBinder), HasResult(*serverId));
...
class BinderLibTestService : public BinderLibTestServiceBase {
public:
    explicit BinderLibTestService(int32_t id)
          : BinderLibTestServiceBase(id),
            m_nextServerId(id + 1),
    ...
    virtual status_t onTransact(uint32_t code, const Parcel &data, Parcel *reply,
        ...
        switch (code) {
            ...
            default:
                return BinderLibTestServiceBase::onTransact(code, data, reply, flags);
        };
    }
