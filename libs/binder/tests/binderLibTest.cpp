...
enum BinderLibTestTranscationCode {
    ...
    BINDER_LIB_TEST_PROCESS_LOCK_DEADLOCK
};
...
TEST_F(BinderLibTest, ThreadPoolAvailableThreads) {
    ...
    sp<IBinder> server = addServer();
    ASSERT_TRUE(server != nullptr);
    // usleep(500); // sleep for #us to allow addServer to finish.
    ...
    /*
     * Create kKernelThreads (15) threads that try to lockUnlock
    ...
    data.writeInt32(1);
    // unlock after #ms, to give a chance for all threads to be used
...
TEST_F(BinderLibTest, HangingServices) {
    ...
    EXPECT_THAT(server->transact(BINDER_LIB_TEST_PROCESS_LOCK_DEADLOCK, data, &reply), NO_ERROR);
    std::vector<std::thread> ts;
    ...
    EXPECT_GE(epochMsAfter,
              epochMsBefore + 1000); // deadlock occurred and threads only finished after 1s passed.
}
...
public:
    ...
    virtual status_t onTransact(uint32_t code, const Parcel &data, Parcel *reply,
        ...
        switch (code) {
            ...
            case BINDER_LIB_TEST_PROCESS_LOCK_DEADLOCK: {
                blockMutex.lock();
                std::thread t([&] { unlockInMs(1000); }); // start local thread to unlock in 1s
                t.detach();
                return NO_ERROR;
            }
            default:
