...
TEST_P(BinderLibRpcTest, SetRpcMaxThreads) {
    ...
    EXPECT_THAT(binder->setRpcClient(std::move(socket), 1), StatusEq(OK));
}

TEST_P(BinderLibRpcTest, SetRpcClientNoFd) {
    auto binder = GetService();
    ASSERT_TRUE(binder != nullptr);
    EXPECT_THAT(binder->setRpcClient(android::base::unique_fd(), 1), StatusEq(BAD_VALUE));
}
...
TEST_P(BinderLibRpcTest, SetRpcMaxThreadsZero) {
    ...
    auto [socket, port] = CreateSocket();
    ASSERT_TRUE(socket.ok());
    EXPECT_THAT(binder->setRpcClient(std::move(socket), 0), StatusEq(BAD_VALUE));
}
...
TEST_P(BinderLibRpcTest, SetRpcMaxThreadsTwice) {
    ...
    EXPECT_THAT(binder->setRpcClient(std::move(socket1), 1), StatusEq(OK));

    auto [socket2, port2] = CreateSocket();
    ASSERT_TRUE(socket2.ok());
    EXPECT_THAT(binder->setRpcClient(std::move(socket2), 1), StatusEq(ALREADY_EXISTS));
}
...
TEST_P(BinderLibRpcClientTest, Test) {
    ...
    // Fake servicedispatcher.
    {
        ...
        ASSERT_TRUE(socket.ok());
        port = socketPort;
        ASSERT_THAT(server->setRpcClient(std::move(socket), numThreads), StatusEq(OK));
    }
