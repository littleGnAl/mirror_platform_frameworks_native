...
class BinderLibTest : public ::testing::Test {
    ...
    protected:
        sp<IBinder> addServer(int32_t *idPtr = NULL, bool usePoll = false)
        {
            int ret;
            int32_t id;
            Parcel data, reply;
            sp<IBinder> binder;
            int code = usePoll ? BINDER_LIB_TEST_ADD_POLL_SERVER : BINDER_LIB_TEST_ADD_SERVER;
