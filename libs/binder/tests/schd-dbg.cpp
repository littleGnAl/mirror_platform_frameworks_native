...
void thread_dump(const char* prefix) {
    ...
    ASSERT(!pthread_getschedparam(pthread_self(), &policy, &param));
    string s = (policy == SCHED_OTHER) ? "SCHED_OTHER"
...
struct Results {
    ...
    void dump() {
        ...
        cout << setprecision(DUMP_PRESICION) << "{ \"avg\":" << setw(W) << left << average
             << ",\"wst\":" << setw(W) << left << worst << ",\"bst\":" << setw(W) << left << best
             << ",\"miss\":" << left << m_miss << ",\"meetR\":" << left
             << setprecision(DUMP_PRESICION + 3) << (1.0 - (double)m_miss / m_transactions) << "}";
    }
};
...
void worker_fx(int num, int no_process, int iterations, int payload_size, Pipe p) {
    int dummy;
...
Pipe make_process(int num, int iterations, int no_process, int payload_size) {
    ...
    if (pid) {
        ...
        worker_fx(num, no_process, iterations, payload_size, move(get<1>(pipe_pair)));
        // never get here
        return move(get<0>(pipe_pair));
    }
}
...
int main(int argc, char** argv) {
    ...
    // TODO: libjson?
    cout << "{" << endl;
    cout << "\"cfg\":{\"pair\":" << (no_process / 2) << ",\"iterations\":" << iterations
