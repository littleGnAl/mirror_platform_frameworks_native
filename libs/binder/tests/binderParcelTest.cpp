...
// Tests operations that shoud NOT be attempted after the binder object
// has already been sent to another process (after addService() is called)
class BinderAlreadySentDeathTest : public testing::TestWithParam<const char*> {
public:
    void InvalidOperation() {
        if (kParam == "setRequestingSid") {
            ASSERT_DEATH({ service_->setRequestingSid(true); }, kDeathMessage);
        } else if (kParam == "setMinSchedulerPolicy") {
            ASSERT_DEATH({ service_->setMinSchedulerPolicy(0, 0); }, kDeathMessage);
        } else if (kParam == "setInheritRt") {
            ASSERT_DEATH({ service_->setInheritRt(true); }, kDeathMessage);
        } else if (kParam == "setExtension") {
    ...
    const std::string kParam = GetParam();
    const std::string kDeathMessage = ".*should only be called before binder object is sent.*";
};

TEST_P(BinderAlreadySentDeathTest, AbortsOnInvalidOperation) {
    InvalidOperation();
}

const char* invalid_operations[] = {
        "setRequestingSid",
...
INSTANTIATE_TEST_SUITE_P(, BinderAlreadySentDeathTest, testing::ValuesIn(invalid_operations));
