...
std::vector<ParcelRead<NdkParcelAdapter>> BINDER_NDK_PARCEL_READ_FUNCTIONS{
        ...
        [](const NdkParcelAdapter& p, FuzzedDataProvider& provider) {
            size_t pos = provider.ConsumeIntegralInRange<size_t>(0, AParcel_getDataSize(p.aParcel()) + 1024);
        ...
        [](const NdkParcelAdapter& p, FuzzedDataProvider& provider) {
            ...
            binder_status_t status = AParcel_appendFrom(p.aParcel(), parcel, 0, pos);
