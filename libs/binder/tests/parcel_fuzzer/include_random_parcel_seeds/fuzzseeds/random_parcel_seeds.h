...
void generateSeedsFromRecording(base::unique_fd fd, binder::debug::RecordedTransaction&& transaction) {
    ...
    //TODO: This wont work because ConsumeIntergralInRange performs series of operations
    // and takes the bytes from the end of the fuzzed data. We need to reverse the operation listed here
    // in order to read back correct byte. Another option is to generate all sequences of bytes and see if
    // that reaches target value for the same API and write those bytes.
    ...
    //TODO: Reorder writeData operation so that if we are we want to read something first, it should be
    // at the end of the buffer. We can have a separate buffer which will grow backward and append both buffers
    // while writing the chunk in order to keep this block readable and in sync with fuzzService.
    ...
    // This should be sum of data sizes from random parcel and all the needed flags in fillRandomParcel
    ...
    bool writeHeaderInternal = true; // always write interface Descriptor or this can be retrieved from recorded transaction
    writeData(fd, &writeHeaderInternal, sizeof(bool));// implicit branch options->writeHeader(p, provider);
    ...
    writeData(fd, &toWrite, sizeof(size_t));// Write parcel data size from recorded transaction

    writeData(fd, dataParcel.data(), toWrite); // Write parcel data with size towrite from recorded transaction
}
