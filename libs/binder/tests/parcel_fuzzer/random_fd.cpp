...
#else
                 snprintf(name, sizeof(name), "/data/android-tempfd-test-%d-XXXXXX", getpid());
#endif
                 int fd = mkstemp(name);
                 CHECK(fd != -1) << "Failed to create file " << name << ", errno: " << errno;
                 unlink(name);
                 CHECK(TEMP_FAILURE_RETRY(
                               ftruncate(fd, provider->ConsumeIntegralInRange<size_t>(0, 4096))) !=
                       -1)
