...
void doShimThings() {
    ...
    // valid check to make sure remote call in the default impl works
    int result = 0;
    auto aidl_status = interface_cast<IBinderDefaultImplTest>(session->getRootObject())
                               ->returnHalf(4, &result);
    ASSERT_TRUE(aidl_status.isOk()) << aidl_status;
    ASSERT_EQ(result, 2) << "Failed returnHalf valid check";
    result = 0;
    aidl_status = proxyBinder->returnHalf(4, &result);
    ASSERT_TRUE(aidl_status.isOk()) << aidl_status;
    ASSERT_EQ(result, 2) << "Failed returnHalf defautlImpl valid check";
