...
extern testing::Environment* binder_env;
...
class BinderDriverInterfaceTest : public ::testing::Test {
 public:
  virtual void SetUp() {
    m_binderFd =
        static_cast<BinderDriverInterfaceTestEnv *>(binder_env)->getBinderFd();
  }
  virtual void TearDown() {}

 protected:
  void binderTestIoctlRetErr2(int cmd, void *arg, int expect_ret,
                              int expect_errno, int accept_errno) {
    int ret;
  ...
  void binderTestIoctlErr2(int cmd, void *arg, int expect_errno,
                           int accept_errno) {
...
TEST_F(BinderDriverInterfaceTest, SetMaxThreadsNull) {
  binderTestIoctlErr2(BINDER_SET_MAX_THREADS, NULL, EFAULT,
...
TEST_F(BinderDriverInterfaceTest, VersionNull) {
  binderTestIoctlErr2(BINDER_VERSION, NULL, EFAULT,
                      EINVAL); /* TODO: don't accept EINVAL */
}
...
TEST_F(BinderDriverInterfaceTest, Read) { binderTestReadEmpty(); }
  ...
  if (bwr.read_consumed >= offsetof(typeof(br), pad)) {
    ...
    struct {
      ...
      binder_uintptr_t arg1;
        ...
        .cmd1 = BC_FREE_BUFFER, .arg1 = br.arg2.data.ptr.buffer,
    };
...
testing::Environment* getBinderDriverInterfaceTestEnv()
{
	return new BinderDriverInterfaceTestEnv();
}
