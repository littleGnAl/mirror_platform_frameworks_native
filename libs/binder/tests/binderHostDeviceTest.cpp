...
public:
    ...
    static std::vector<ParamType> CreateTestParams() {
        ...
        auto result = execute(Split("adb shell service list", " "));
    ...
    void SetUp() override {
        ...
        auto pidResult = execute({"adb", "shell", "dumpsys", "--pid", serviceName});
        ASSERT_THAT(pidResult, HasValue(ExitCode(0)));
        ASSERT_FALSE(Trim(pidResult->stdout).empty());
        auto exeResult = execute({"adb", "shell", "realpath",
                                  StringPrintf("/proc/%s/exe", Trim(pidResult->stdout).c_str())});
        if (exeResult.ok() && exeResult->exitCode == 0 &&
            StartsWith(exeResult->stdout, "/system/bin/app_process"))
            GTEST_SKIP() << "b/190450693 Skipping service on Java process \""
                         << Trim(exeResult->stdout) << "\"";
        ...
        ASSERT_THAT(*dispatchResult, ExitCode(0));
        mDispatched = true;
