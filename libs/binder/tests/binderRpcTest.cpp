...
constexpr bool kDisableSharedLibs = true;
#else
constexpr bool kDisableSharedLibs = false;
#endif
...
using StartServiceFn = void (*)(const BinderRpcOptions& options, SocketType socketType,
                                RpcSecurity rpcSecurity, unsigned int vsockPort,
                                const std::string& addr, android::base::borrowed_fd writeEnd,
                                android::base::borrowed_fd readEnd);

class BinderRpc : public ::testing::TestWithParam<std::tuple<SocketType, RpcSecurity, bool, bool>> {
public:
    static inline std::string PrintParamInfo(const testing::TestParamInfo<ParamType>& info) {
        auto [type, security, singleThreaded, noKernel] = info.param;
        auto ret = PrintToString(type) + "_" + newFactory(security)->toCString();
        if (singleThreaded) {
    ...
    ProcessSession createRpcTestSocketServerProcessEtc(const BinderRpcOptions& options) {
        ...
        bool singleThreaded = std::get<2>(GetParam());
        bool noKernel = std::get<3>(GetParam());
        ...
        auto ret = ProcessSession{
                .host = Process([&](android::base::borrowed_fd writeEnd,
                    ...
                    auto startServiceFn =
                            reinterpret_cast<StartServiceFn>(dlsym(library, "startService"));
                    LOG_ALWAYS_FATAL_IF(startServiceFn == nullptr, "dlsym failed: %s", dlerror());

                    startServiceFn(options, socketType, rpcSecurity, vsockPort, addr, writeEnd,
                                   readEnd);
...
public:
    void SetUp() override {
        if constexpr (kDisableRpcThreads) {
...
TEST_P(BinderRpc, Callbacks) {
    ...
    bool singleThreaded = kDisableRpcThreads || std::get<2>(GetParam());

    for (bool callIsOneway : {true, false}) {
        for (bool callbackIsOneway : {true, false}) {
            if (singleThreaded && (callIsOneway || callbackIsOneway)) {
            ...
            for (bool delayed : {true, false}) {
                auto proc = createRpcTestSocketServerProcess(
                        {.numThreads = 1,
                         .numSessions = 1,
                         .numIncomingConnections = singleThreaded ? 0UL : 1UL});
...
TEST_P(BinderRpc, WorksWithLibbinderNdkPing) {
    if constexpr (kDisableSharedLibs) {
            ...
            [&](android::base::borrowed_fd writeEnd, android::base::borrowed_fd /*readEnd*/) {
                ...
                if (status_t status = server->setupVsockServer(vsockPort); status != OK) {
                    if (status == -EAFNOSUPPORT) {
                        writeString(writeEnd, "EAFNOSUPPORT");
                    } else {
                        writeString(writeEnd, statusToString(status));
                    }
                    return;
                }
