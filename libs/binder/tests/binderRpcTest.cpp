...
TEST(BinderRpc, RepeatBinder) {
    ...
    // Actually - we should make sure socket binder has something like
    // flushIfNeeded
    EXPECT_EQ(OK, proc.rootBinder->pingBinder());
...
TEST(BinderRpc, SameBinderEqualityWeak) {
    ...
    GTEST_SKIP() << "FIXME: Weak binders don't work properly";
...
TEST(BinderRpc, ThreadPoolOverSaturated) {
    ...
    // FIXME: potential flake, but making sure that the additional calls
    // are all handled in parallel, not serial
...
TEST(BinderRpc, OnewayCallDoesNotWait) {
    constexpr size_t kReallyLongTime = 1000;  // for the scheduler
    constexpr size_t kSleepMs = kReallyLongTime * 60;
    ...
    GTEST_SKIP() << "FIXME: this will freeze the test 60s, because subsequent calls will get stuck "
                 "behind 60s call. Need better dispatch.";
    EXPECT_OK(proc.rootIface->sleepMsAsync(kSleepMs));

    size_t epochMsAfter = epochMillis();
    EXPECT_LT(epochMsAfter, epochMsBefore + kReallyLongTime);
}

TEST(BinderRpc, OnewayCallQueueing) {
    constexpr size_t kNumSleeps = 3;
    constexpr size_t kNumExtraServerThreads = 2;
    constexpr size_t kSleepMs = 500;
    ...
    // FIXME: this happens to work because all calls are processed on the first
    // thread, but it's not a good behavior in general. We could easily break it
    // by sending another call on the first thread inside this forloop.
    GTEST_SKIP() << "FIXME: test appears to pass, but it's because it's a bad test.";
...
TEST(BinderRpc, Die) {
    ...
    GTEST_SKIP() << "FIXME: Need to cleanup dangling refcounts when this happens";
