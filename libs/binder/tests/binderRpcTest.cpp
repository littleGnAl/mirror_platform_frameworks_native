...
public:
    ...
    Status countBinders(int32_t* out) override {
        *out = RpcState::self().countBinders();
        if (*out != 1) {
            RpcState::self().dump();
        }
        return Status::ok();
...
public:
    ...
    void TearDown() override {
        ...
        EXPECT_EQ(0, RpcState::self().countBinders()) << (RpcState::self().dump(), "dump:");
    }

    // the service as viewed over the socket connection
    sp<IBinder> binder;
...
extern "C" int main(int argc, char** argv) {
    ...
    if (childPid == 0) {
        ...
        defaultServiceManager()->addService(kServiceName, service);
