...
TEST_P(RpcTransportTest, UntrustedServer) {
    ...
    auto untruestedServer = mServers.emplace_back(std::make_unique<Server>()).get();
    ASSERT_TRUE(untruestedServer->setUp());

    Client client(untruestedServer->getConnectToServerFn());
    ASSERT_TRUE(client.setUp());

    ASSERT_EQ(OK,
              untruestedServer->getCtx()->addTrustedPeerCertificate(certificateFormat,
                                                                    client.getCtx()->getCertificate(
                                                                            certificateFormat)));

    untruestedServer->start();
