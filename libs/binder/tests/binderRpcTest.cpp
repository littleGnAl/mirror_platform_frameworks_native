...
// FIXME: this is because in 'main', we setup the server with 3 threads
TEST_F(BinderRpc, SaturateThreadPool) {
    EXPECT_OK(iface->lock());

    auto t1 = std::thread([&]{iface->lockUnlock();});
    auto t2 = std::thread([&]{iface->lockUnlock();});

    usleep(10000);

    // other calls still work
    sp<IBinder> out;
    EXPECT_OK(iface->repeatBinder(binder, &out));
    EXPECT_EQ(out, binder);

    EXPECT_OK(iface->unlock());
...
TEST_F(BinderRpc, ThreadingStressTest) {
...
extern "C" int main(int argc, char** argv) {
    ...
    // FIXME: switch to having a method which creates all of the sockets and
    // allows configuring this dynamically, instead of having static in 'main'
    // here

    const char* kSock1 = "/dev/binderRpcTest_socket_1";
    const char* kSock1_1 = "/dev/binderRpcTest_socket_1_1";
    const char* kSock1_2 = "/dev/binderRpcTest_socket_1_2";
    ...
    if (childPid == 0) {
        ...
        std::thread([=] {
            // FIXME: most of this can be behind common utility functions
            sp<MyBinderRpcTest> service = new MyBinderRpcTest;
            sp<RpcServer> server = RpcServer::make();
            sp<RpcConnection> connection = server->addClientConnection();
            CHECK(connection->addUnixDomainServer(kSock1));
            CHECK(connection->addUnixDomainServer(kSock1_1));
            CHECK(connection->addUnixDomainServer(kSock1_2));
            service->connection = connection;  // FIXME: sp loop
        ...
        sp<MyBinderRpcTest> service = new MyBinderRpcTest;
        sp<RpcServer> server2 = RpcServer::make();
        sp<RpcConnection> connection = server2->addClientConnection();
        CHECK(connection->addUnixDomainServer(kSock2));
    ...
    // FIXME
    usleep(100000); // time for connection to be created (might be refused) ?

    // FIXME: this connection should be automatically created in the background
    // by whatever method we get ahold of a server.
    gConnection = RpcConnection::make();
    CHECK(gConnection->addUnixDomainClient(kSock1));
    CHECK(gConnection->addUnixDomainClient(kSock1_1));
    CHECK(gConnection->addUnixDomainClient(kSock1_2));
    gConnectionOther = RpcConnection::make();
    CHECK(gConnectionOther->addUnixDomainClient(kSock2));
