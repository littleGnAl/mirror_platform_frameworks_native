...
public:
...
#ifdef __BIONIC__
                case SocketType::VSOCK:
                    if (ret.connection->setupVsockClient(VMADDR_CID_LOCAL, port)) goto success;
                    break;
#endif // __BIONIC__
                case SocketType::INET:
                    if (ret.connection->setupInetClient("127.0.0.1", port)) goto success;
                    break;
                default:
                    LOG_ALWAYS_FATAL("Unknown socket type");
            }
    ...
    success:
        ...
        EXPECT_EQ(numThreads, ret.connection->getMaxThreads());
