...
public:
    ...
    void SetUp() override {
        if (socketType() == SocketType::UNIX_BOOTSTRAP && rpcSecurity() == RpcSecurity::TLS) {
            GTEST_SKIP() << "TLS is not supported over UDS bootstrap";
        }
    }
        ...
        for (const auto& session : sessions) {
            ...
            switch (socketType) {
                ...
                case SocketType::UNIX_BOOTSTRAP: {
                    android::base::unique_fd sessionFd(dup(bootstrapClientFd.get()));
                    status = session->setupUnixDomainSocketBootstrapClient(std::move(sessionFd));
                } break;
