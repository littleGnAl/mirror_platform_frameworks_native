...
public:
    ...
    static LinuxProcessSession& fromPtr(const std::unique_ptr<ProcessSession>& ps) {
        return *static_cast<LinuxProcessSession*>(ps.get());
    }
...
TEST_P(BinderRpc, SingleDeathRecipientOnShutdown) {
    ...
    LinuxProcessSession::fromPtr(proc.proc).host.terminate();
    LinuxProcessSession::fromPtr(proc.proc).host.setCustomExitStatusCheck([](int wstatus) {
