...
static base::unique_fd initUnixSocket(std::string addr) {
    ...
    base::unique_fd fd(TEMP_FAILURE_RETRY(
            socket(socket_addr.addr()->sa_family, SOCK_STREAM | SOCK_CLOEXEC | SOCK_NONBLOCK, 0)));
    CHECK_GE(fd.get(), 0);
...
public:
    ...
    ProcessSession createRpcTestSocketServerProcessEtc(const BinderRpcOptions& options) {
        ...
        if (socketType == SocketType::UNIX_INIT) {
            auto fd = initUnixSocket(serverConfig.addr);
            serverConfig.socketFd = fd.get();
        } else {
            serverConfig.socketFd = -1;
        }
    ...
    public:
        ...
        [[nodiscard]] AssertionResult setUp(
            ...
            switch (socketType) {
                ...
                case SocketType::UNIX_INIT: {
                    auto addr = allocateSocketAddress();
                    auto status = rpcServer->setupInitSocketServer(initUnixSocket(addr));
                    if (status != OK) {
                        return AssertionFailure()
