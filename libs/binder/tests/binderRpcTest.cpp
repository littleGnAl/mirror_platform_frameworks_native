    ...
    ProcessSession createRpcTestSocketServerProcess(
        ...
        auto ret = ProcessSession{
                .host = Process([&](android::base::borrowed_fd writeEnd,
                    ...
                    serverInfo.cert =
                            String16(server->getCertificate(CertificateFormat::PEM).data());
                    writeToFd(writeEnd, serverInfo);
                    auto clientInfo = readFromFd<BinderRpcTestClientInfo>(readEnd);

                    if (rpcSecurity == RpcSecurity::TLS) {
                        for (const auto& clientCert16 : clientInfo.certs) {
                            std::string clientCert(String8(clientCert16).c_str());
                            CHECK_EQ(OK,
        ...
        for (const auto& session : sessions) {
            clientInfo.certs.emplace_back(session->getCertificate(CertificateFormat::PEM).data());
        }
        ...
        if (rpcSecurity == RpcSecurity::TLS) {
            std::string serverCert(String8(serverInfo.cert).c_str());
            CHECK_EQ(OK,
...
public:
    ...
    status_t trust(A* a, B* b) {
        ...
        return a->getCertVerifier()->addTrustedPeerCertificate(certificateFormat,
                                                               b->getCtx()->getCertificate(
                                                                       certificateFormat));
    }
