...
TEST(BinderRpc, Java) {
    ...
    GTEST_SKIP() << "This test is only run on Android. Though it can technically run on host with "
                    "a device attached on createRpcDelegateServiceManager(), such test belongs to "
                    "binderHostDeviceTest. Hence, just disable this test on host.";
#endif // __ANDROID__
