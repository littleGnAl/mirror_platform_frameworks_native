...
ProcessConnection createRpcTestSocketServerProcess(
    ...
    for (size_t i = 0; i < numThreads; i++) {
       ...
       CHECK(ret.connection->addUnixDomainClient(addr.c_str()) ||
             (usleep(10000), ret.connection->addUnixDomainClient(addr.c_str()))) << i;
    }
...
TEST(BinderRpc, OnewayCallDoesNotWait) {
    constexpr size_t kReallyLongTimeMs = 1000;  // for the scheduler
    constexpr size_t kSleepMs = kReallyLongTimeMs * 60;
...
TEST(BinderRpc, Die) {
    ...
    for (bool doDeathCleanup : {true, false}) {
        ...
        EXPECT_EQ(DEAD_OBJECT, proc.rootIface->die(doDeathCleanup).transactionError()) << doDeathCleanup;
