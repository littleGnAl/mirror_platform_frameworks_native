...
constexpr const RpcCertificateFormat kFormat = RpcCertificateFormat::DER;
...
static sp<RpcSession> gSessionTls;
#ifdef __BIONIC__
...
void forkRpcServer(const char* addr, sp<RpcServer> server) {
    if (0 == fork()) {
...
int main(int argc, char** argv) {
    ...
    auto tlsServer = RpcServer::make(makeFactoryTls());
    forkRpcServer(tlsAddr.c_str(), tlsServer);
    auto serverCert = tlsServer->getCertificate(kFormat);
    for (size_t tries = 0; tries < 5; tries++) {
        usleep(10000);
        gSessionTls = RpcSession::make(makeFactoryTls());
        auto clientCert = gSessionTls->getCertificate(kFormat);
        status = gSessionTls->setupUnixDomainClient(tlsAddr.c_str());
        if (status == OK) break;
    }
    CHECK_EQ(status, OK) << "Could not connect: " << tlsAddr << ": "
