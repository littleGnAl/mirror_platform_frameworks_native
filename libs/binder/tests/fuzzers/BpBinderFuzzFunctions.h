 ...
 */
static const std::vector<std::function<void(FuzzedDataProvider*, const sp<BpBinder>&)>> gBPBinderOperations =
        {[](FuzzedDataProvider*, const sp<BpBinder>& bpbinder) -> void { bpbinder->handle(); },
         [](FuzzedDataProvider* fdp, const sp<BpBinder>& bpbinder) -> void {
             ...
             bpbinder->unlinkToDeath(nullptr, reinterpret_cast<void*>(&kBpBinderCookie), flags,
                                     &out_recipient);
             s_recipient = new FuzzDeathRecipient();
             kBpBinderCookie = fdp->ConsumeIntegral<int8_t>();
             
             bpbinder->linkToDeath(s_recipient, reinterpret_cast<void*>(&kBpBinderCookie), flags);
         },
         [](FuzzedDataProvider* fdp, const sp<BpBinder>& bpbinder) -> void {
             wp<IBinder::DeathRecipient> out_recipient(nullptr);
             uint32_t flags = fdp->ConsumeIntegral<uint32_t>();
             int8_t random_cookie = fdp->ConsumeIntegral<int8_t>();
             bpbinder->unlinkToDeath(nullptr, reinterpret_cast<void*>(&random_cookie), flags,
                                     &out_recipient);
         },
         [](FuzzedDataProvider*, const sp<BpBinder>& bpbinder) -> void { bpbinder->remoteBinder(); },
         [](FuzzedDataProvider*, const sp<BpBinder>& bpbinder) -> void { bpbinder->sendObituary(); },
         [](FuzzedDataProvider* fdp, const sp<BpBinder>& bpbinder) -> void {
             uint32_t uid = fdp->ConsumeIntegral<uint32_t>();
             bpbinder->getBinderProxyCount(uid);
         },
         [](FuzzedDataProvider*, const sp<BpBinder>& bpbinder) -> void { bpbinder->enableCountByUid(); },
         [](FuzzedDataProvider*, const sp<BpBinder>& bpbinder) -> void { bpbinder->disableCountByUid(); },
         [](FuzzedDataProvider*, const sp<BpBinder>& bpbinder) -> void {
