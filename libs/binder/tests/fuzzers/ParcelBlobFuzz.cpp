...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    while (dataProvider.remaining_bytes() > 0) {
        ...
        uint8_t function_id =
                dataProvider.ConsumeIntegralInRange<uint8_t>(0, parcelBlob_operations.size() - 1);
        parcelBlob_operations[function_id](&dataProvider, &readableBlobVector, &writableBlobVector);
    }
