...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    while (fdp.remaining_bytes() > 0) {
        uint8_t function_id = fdp.ConsumeIntegralInRange<uint8_t>(0, status_operations.size() - 1);
        status_operations[function_id](&fdp, &status, &parcel);
        if (fdp.ConsumeBool()) ss << status;
    }
    return 0;
}
} // namespace android
