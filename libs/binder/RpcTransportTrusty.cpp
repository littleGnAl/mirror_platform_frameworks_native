...
public:
    ...
    status_t interruptableWriteFully(FdTrigger* fdTrigger, iovec* iovs, int niovs,
        ...
        auto writeFn = [](int fd, iovec* iovs, size_t niovs, int) -> ssize_t {
            if (niovs == 0) {
                return 0;
            }
            ...
            return tipc_send(fd, iovs, 1, nullptr, 0);
        };
    ...
    status_t interruptableReadFully(FdTrigger* fdTrigger, iovec* iovs, int niovs,
                                    const std::function<status_t()>& altPoll) override {
        auto readFn = [](int fd, iovec* iovs, size_t niovs, int) -> ssize_t {
            if (niovs == 0) {
                return 0;
            }
            return read(fd, iovs[0].iov_base, iovs[0].iov_len);
        };
