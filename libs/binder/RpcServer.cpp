...
void RpcServer::join() {
    ...
    while ((status = mShutdownTrigger->triggerablePoll(mServer, POLLIN)) == OK) {
        ...
        RpcSocket clientState(TEMP_FAILURE_RETRY(accept4(mServer.fd.get(),
...
void RpcServer::establishConnection(
        sp<RpcServer>&& server, RpcSocket clientFd, std::array<uint8_t, kRpcAddressSize> addr,
        size_t addrLen,
...
status_t RpcServer::setupSocketServer(const RpcSocketAddress& addr) {
    ...
    RpcSocket serverFdState(TEMP_FAILURE_RETRY(
    ...
    if (status_t status = setupExternalServer(std::move(serverFdState.fd)); status != OK) {
