...
void RpcServer::setSupportedFileDescriptorTransportModes(
    ...
    mSupportedFileDescriptorTransportModes = modes;
}
...
void RpcServer::establishConnection(sp<RpcServer>&& server, base::unique_fd clientFd,
    ...
    sp<RpcSession> session;
    {
        ...
        if (requestingNewSession) {
            ...
            if (std::find_if(server->mSupportedFileDescriptorTransportModes.begin(),
                             server->mSupportedFileDescriptorTransportModes.end(), [&](auto mode) {
                                 return static_cast<uint8_t>(mode) ==
                                         header.fileDescriptorTransportMode;
                             }) != server->mSupportedFileDescriptorTransportModes.end()) {
