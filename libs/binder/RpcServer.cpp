...
RpcServer::RpcServer(bool tls) : mTls(tls) {}
RpcServer::~RpcServer() {
    (void)shutdown();
}

sp<RpcServer> RpcServer::make(bool tls) {
    return sp<RpcServer>::make(tls);
}
...
void RpcServer::join() {
    ...
    auto ctx = newServerRpcTransportCtx(mTls);
    LOG_ALWAYS_FATAL_IF(ctx == nullptr, "Unable to create RpcTransportCtx with TLS == %d", mTls);
