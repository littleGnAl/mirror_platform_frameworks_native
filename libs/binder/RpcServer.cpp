...
void RpcServer::setProtocolVersionOverride(uint32_t version) {
    mProtocolVersionOverride = version;
}
...
void RpcServer::establishConnection(sp<RpcServer>&& server, base::unique_fd clientFd) {
    ...
    if (status == OK) {
        ...
        protocolVersion =
                std::min(header.version,
                         server->mProtocolVersionOverride.value_or(RPC_WIRE_PROTOCOL_VERSION));
