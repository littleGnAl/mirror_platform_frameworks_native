...
status_t RpcServer::processCommand(int fd, RpcServer::ConnectionData* connectionData) {
    ...
    if (0 == TEMP_FAILURE_RETRY(recv(fd, &command, sizeof(command), MSG_WAITALL))) {
        ALOGE("Error reading rpc command header: %s", strerror(errno));
        return UNKNOWN_ERROR;
    }

    LOG_ALWAYS_FATAL_IF(command.command != RPC_COMMAND_TRANSACT);
    std::vector<uint8_t> transactionData(command.bodySize); // FIXME: can avoid allocating extra size here?
    if (0 == TEMP_FAILURE_RETRY(recv(fd, transactionData.data(), transactionData.size(), MSG_WAITALL))) {
        ALOGE("Error reading rpc command header: %s", strerror(errno));
        return UNKNOWN_ERROR;
    }
...
status_t RpcServer::processReply(int fd, RpcServer::ConnectionData* data, Parcel* reply) {
    (void) fd;
    (void) data;
    (void) reply;
    return OK;
