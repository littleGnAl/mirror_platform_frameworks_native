...
void RpcServer::setRootObject(const sp<IBinder>& binder) {
    ALOGE("Root object binder: %p", binder.get());
    mServed.push_back(binder);

    RpcState::self().setRootObject(binder);
}

void RpcServer::join() {
    while(true) {
        RpcState::self().dump(); // FIXME
        ...
        // - ???
        status_t error = RpcState::self().getAndExecuteCommand(mConnection->mFd, mConnection);
