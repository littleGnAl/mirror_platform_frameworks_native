...
void RpcServer::setPerSessionRootObject(
        const std::function<sp<IBinder>(sockaddr*, socklen_t)>& makeObject) {
    std::lock_guard<std::mutex> _l(mLock);
    mRootObject.clear();
...
void RpcServer::join() {
    ...
    while ((status = mShutdownTrigger->triggerablePoll(mServer, POLLIN)) == OK) {
        ...
        unique_fd clientFd(TEMP_FAILURE_RETRY(
                accept4(mServer.get(), &addr.addr, &addrLen, SOCK_CLOEXEC | SOCK_NONBLOCK)));
...
void RpcServer::establishConnection(sp<RpcServer>&& server, base::unique_fd clientFd,
                                    SockAddrAny addr, socklen_t addrLen) {
    ...
    sp<RpcSession> session;
    {
        ...
        if (requestingNewSession) {
            ...
            if (server->mRootObjectFactory != nullptr) {
                sessionSpecificRoot = server->mRootObjectFactory(&addr.addr, addrLen);
