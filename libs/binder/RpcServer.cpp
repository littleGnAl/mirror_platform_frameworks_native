...
void RpcServer::join() {
    ...
    auto ctx = RpcTransportServerCtx::create();
    LOG_ALWAYS_FATAL_IF(ctx == nullptr, "Unable to set up RpcTransportServerCtx");
    ...
    while ((status = mShutdownTrigger->triggerablePollRead(mServer)) == OK) {
        ...
        auto client = ctx->sslAccept(std::move(clientFd));
        if (client == nullptr) {
