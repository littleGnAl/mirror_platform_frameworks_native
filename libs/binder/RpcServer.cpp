...
void RpcServer::join() {
    ...
    while ((status = mShutdownTrigger->triggerablePoll(mServer, POLLIN)) == OK) {
        ...
        RpcSocket clientSocket(unique_fd(TEMP_FAILURE_RETRY(
...
status_t RpcServer::setupSocketServer(const RpcSocketAddress& addr) {
    ...
    RpcSocket serverSocket(unique_fd(TEMP_FAILURE_RETRY(
    ...
    if (0 != TEMP_FAILURE_RETRY(bind(serverSocket.fd.get(), addr.addr(), addr.addrSize()))) {
        int savedErrno = errno;
    ...
    //  to 1.
    if (0 != TEMP_FAILURE_RETRY(listen(serverSocket.fd.get(), 50 /*backlog*/))) {
        int savedErrno = errno;
    ...
    if (status_t status = setupExternalServer(std::move(serverSocket.fd)); status != OK) {
