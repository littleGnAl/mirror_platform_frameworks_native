...
void RpcServer::join() {
    ...
    while ((status = mShutdownTrigger->triggerablePoll(mServer, POLLIN)) == OK) {
        ...
        FdState clientState(TEMP_FAILURE_RETRY(accept4(mServer.fd.get(),
        ...
        LOG_RPC_DETAIL("accept4 on fd %d yields fd %d", mServer.fd.get(), clientState.fd.get());

        {
            RpcMutexLockGuard _l(mLock);
            RpcMaybeThread thread =
                    RpcMaybeThread(&RpcServer::establishConnection,
                                   sp<RpcServer>::fromExisting(this), std::move(clientState), addr,
...
void RpcServer::establishConnection(
        sp<RpcServer>&& server, FdState clientFd, std::array<uint8_t, kRpcAddressSize> addr,
        size_t addrLen,
    ...
    if (client == nullptr) {
        ...
        LOG_RPC_DETAIL("Created RpcTransport %d for client fd %d", clientFd.fd.get(),
                       clientFdForLog);
    }
...
status_t RpcServer::setupSocketServer(const RpcSocketAddress& addr) {
    ...
    FdState serverFdState(TEMP_FAILURE_RETRY(
            socket(addr.addr()->sa_family, SOCK_STREAM | SOCK_CLOEXEC | SOCK_NONBLOCK, 0)));
    if (serverFdState.fd == -1) {
        int savedErrno = errno;
        ALOGE("Could not create socket: %s", strerror(savedErrno));
        return -savedErrno;
    }

    if (0 != TEMP_FAILURE_RETRY(bind(serverFdState.fd.get(), addr.addr(), addr.addrSize()))) {
        int savedErrno = errno;
    ...
    //  to 1.
    if (0 != TEMP_FAILURE_RETRY(listen(serverFdState.fd.get(), 50 /*backlog*/))) {
        int savedErrno = errno;
    ...
    if (status_t status = setupExternalServer(std::move(serverFdState)); status != OK) {
...
status_t RpcServer::setupExternalServer(FdState serverFd) {
    RpcMutexLockGuard _l(mLock);
    if (mServer.fd.ok()) {
