...
status_t RpcServer::setupUnixDomainPairServer(unique_fd connectFd) {
    return setupExternalServer(std::move(connectFd), &RpcServer::acceptSocketpairConnection);
}
...
status_t RpcServer::acceptSocketConnection(sp<RpcServer>&& server, RpcTransportFd* out) {
...
status_t RpcServer::acceptSocketpairConnection(sp<RpcServer>&& server, RpcTransportFd* out) {
    base::unique_fd fd;
    if (status_t status = ReceiveFdOverUnixSocket(server->mServer.fd, &fd); status != OK) {
        ALOGE("Could not receive socket: %s", strerror(-status));
        return status;
    }

    if (auto res = setNonBlocking(fd); !res.ok()) {
        ALOGE("acceptSocketpairConnection: %s", res.error().message().c_str());
...
void RpcServer::join() {
    ...
    while ((status = mShutdownTrigger->triggerablePoll(mServer, POLLIN)) == OK) {
        ...
        if (mAcceptFn(sp<RpcServer>::fromExisting(this), &clientSocket) != OK) {
            continue;
        }
