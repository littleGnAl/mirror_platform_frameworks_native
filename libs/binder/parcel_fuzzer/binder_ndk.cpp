...
std::vector<ParcelRead<NdkParcelAdapter>> BINDER_NDK_PARCEL_READ_FUNCTIONS{
        ...
        [](const NdkParcelAdapter& /*p*/, uint8_t /*data*/) {
            FUZZ_LOG() << "about to reset the parcel";
            AParcel* parcel = AParcel_create();
            binder_status_t status = AParcel_reset(parcel);
            AParcel_delete(parcel);
            FUZZ_LOG() << "reset the parcel: " << status;
        },
        [](const NdkParcelAdapter& p, uint8_t /*data*/) {
        ...
        [](const NdkParcelAdapter& /*p*/, uint8_t /*data*/) {
            FUZZ_LOG() << "about to create a parcel";
            AParcel* parcel = AParcel_create();
            AParcel_delete(parcel);
            FUZZ_LOG() << "AParcel_create done";
        },
        [](const NdkParcelAdapter& p, uint8_t data) {
            FUZZ_LOG() << "about to read a ParcelableHolder";
            ndk::AParcelableHolder ph {(data % 1 == 1) ? ndk::STABILITY_LOCAL : ndk::STABILITY_VINTF};
