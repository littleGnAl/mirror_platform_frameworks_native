...
std::optional<RecordedTransaction> RecordedTransaction::fromFile(const unique_fd& fd) {
    ...
    do {
        ...
        switch (chunk.chunkType) {
            case HEADER_CHUNK: {
                size_t sizeToRead = std::min(chunk.dataSize, (uint64_t)sizeof(TransactionHeader));
                if (!android::base::ReadFully(fd, &t.mHeader, sizeToRead)) {
                    LOG(INFO) << "Failed to read transactionHeader from fd " << fd.get();
                    return std::nullopt;
                }
                if (chunk.dataSize > (uint64_t)sizeToRead) {
                    LOG(INFO) << "Header larger than expected. Some fields will be dropped.";
                    lseek(fd.get(), (chunk.dataSize - sizeToRead) + chunk.padding, SEEK_CUR);
                } else if (sizeof(TransactionHeader) > chunk.dataSize) {
                    LOG(INFO) << "Header smaller than expected. Some fields will be empty.";
                }
...
android::status_t RecordedTransaction::writeChunk(borrowed_fd fd, uint32_t chunkType,
    ...
    uint32_t additionalPaddingCount = (uint32_t)PADDING8(byteCount);
