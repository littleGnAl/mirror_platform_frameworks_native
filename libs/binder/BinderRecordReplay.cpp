...
#define PADDING(s) ((8 - s % 8) % 8)

static_assert(PADDING(0) == 0);
static_assert(PADDING(1) == 7);
static_assert(PADDING(7) == 1);
static_assert(PADDING(8) == 0);
...
RecordedTransaction::RecordedTransaction(RecordedTransaction&& t) {
...
std::optional<RecordedTransaction> RecordedTransaction::fromFile(const unique_fd& fd) {
    ...
    uint8_t padding[7];
    if (!android::base::ReadFully(fd, padding, PADDING(t.getDataSize()))) {
    ...
    if (!android::base::ReadFully(fd, padding, PADDING(t.getReplySize()))) {
...
android::status_t RecordedTransaction::dumpToFile(const unique_fd& fd) const {
    ...
    if (!android::base::WriteFully(fd, zeros, PADDING(getDataSize()))) {
    ...
    if (!android::base::WriteFully(fd, zeros, PADDING(getReplySize()))) {
