...
// encountered. This has four notable implications:
//
...
std::optional<RecordedTransaction> RecordedTransaction::fromFile(const unique_fd &fd) {
    ...
    do {
        ...
        payloadMap += mmapPayloadStartOffset /
                8; // Skip chunk descriptor and required mmap page-alignment
        if (payloadMap == MAP_FAILED) {
            LOG(INFO) << "Memory mapping failed for fd " << fd.get();
            LOG(INFO) << "Error: " << errno << " " << strerror(errno);
            return std::nullopt;
        }
