...
Result<size_t> RpcTransportTls::send(const void *buf, size_t size) {
    ...
    if (ret <= 0) {
        ...
        if (err == SSL_ERROR_WANT_WRITE) {
            ...
            return Error(EAGAIN) << "SSL_write(): " << SSL_error_description(err);
        }
...
Result<size_t> RpcTransportTls::recv(void *buf, size_t size) {
    ...
    if (ret < 0) {
        ...
        if (err == SSL_ERROR_WANT_READ) {
            ...
            return Error(EAGAIN) << "SSL_read(): " << SSL_error_description(err);
        }
...
Result<size_t> RpcTransportTls::peek(void *buf, size_t size) {
    ...
    if (ret < 0) {
        ...
        if (err == SSL_ERROR_WANT_READ) {
            ...
            return Error(EAGAIN) << "SSL_peek(): " << SSL_error_description(err);
        }
...
bssl::UniquePtr<SSL> connectOrAccept(SSL_CTX *ctx, android::base::borrowed_fd fd,
    ...
    for (int i = 0; i < 5; i++) {
        ...
        // Hit EAGAIN on non-blocking sockets. Retry a little bit.
    }
    ALOGE("%s(): hit EAGAIN for 5 times, rejecting", fnString);
    return nullptr;
}
