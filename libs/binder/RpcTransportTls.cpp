...
class RpcTransportTlsServerCtx;
class RpcTransportTlsClientCtx;
...
public:
    RpcTransportTls(android::base::unique_fd socket) : mSocket(std::move(socket)) {}
...
private:
    friend RpcTransportTlsServerCtx;
    friend RpcTransportTlsClientCtx;
...
class RpcTransportTlsServerCtx {
public:
    static std::unique_ptr<RpcTransportTlsServerCtx> create();
    std::unique_ptr<RpcTransport> newAcceptedTransport(android::base::unique_fd acceptedFd) {
        auto ssl = connectAccept(mCtx.get(), acceptedFd, &SSL_accept, "SSL_accept");
        auto rpcTransportTls =
                std::unique_ptr<RpcTransportTls>(new RpcTransportTls(std::move(acceptedFd)));
        rpcTransportTls->mSsl = std::move(ssl);
        return rpcTransportTls;
    }
...
class RpcTransportTlsClientCtx {
public:
    static std::unique_ptr<RpcTransportTlsClientCtx> create();
    std::unique_ptr<RpcTransport> newConnectedTransport(android::base::unique_fd connectedFd) {
        ...
        auto rpcTransportTls =
                std::unique_ptr<RpcTransportTls>(new RpcTransportTls(std::move(connectedFd)));
        rpcTransportTls->mSsl = std::move(ssl);
        return rpcTransportTls;
    }
...
std::unique_ptr<RpcTransportTlsServerCtx> RpcTransportTlsServerCtx::create() {
    ...
    auto rpcTransportTlsServerCtx =
            std::unique_ptr<RpcTransportTlsServerCtx>(new RpcTransportTlsServerCtx());
    rpcTransportTlsServerCtx->mCtx = std::move(ctx);

    return rpcTransportTlsServerCtx;
}

std::unique_ptr<RpcTransportTlsClientCtx> RpcTransportTlsClientCtx::create() {
    bssl::UniquePtr<SSL_CTX> ctx(SSL_CTX_new(TLS_method()));
    ...
    auto rpcTransportTlsClientCtx =
            std::unique_ptr<RpcTransportTlsClientCtx>(new RpcTransportTlsClientCtx());
...
extern "C" {
__attribute__((visibility("default"))) android::RpcTransport *newTlsServerRpcTransport(
        android::base::unique_fd acceptedFd) {
    auto ctx = android::RpcTransportTlsServerCtx::create();
    if (ctx == nullptr) return nullptr;
    return ctx->newAcceptedTransport(std::move(acceptedFd)).release();
}

__attribute__((visibility("default"))) android::RpcTransport *newTlsClientRpcTransport(
        android::base::unique_fd connectedFd) {
    auto ctx = android::RpcTransportTlsClientCtx::create();
    if (ctx == nullptr) return nullptr;
    return ctx->newConnectedTransport(std::move(connectedFd)).release();
}
} // extern "C"
