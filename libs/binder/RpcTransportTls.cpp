...
public:
    ...
    status_t pollForSslError(
            const android::RpcSocket& fd, int sslError, FdTrigger* fdTrigger, const char* fnString,
            int additionalEvent,
...
private:
    ...
    status_t handlePoll(int event, const android::RpcSocket& fd, FdTrigger* fdTrigger,
                        const char* fnString,
                        const std::optional<android::base::function_ref<status_t()>>& altPoll) {
        status_t ret;
        if (altPoll) {
...
bool setFdAndDoHandshake(Ssl* ssl, const android::RpcSocket& socket, FdTrigger* fdTrigger) {
...
std::unique_ptr<RpcTransport> RpcTransportCtxTls::newTransport(android::RpcSocket socket,
