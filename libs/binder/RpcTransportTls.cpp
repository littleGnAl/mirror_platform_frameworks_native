...
public:
    ...
    std::string getX509Pem() override;
    status_t addTrustedX509Pem(std::string_view cert) override;
...
std::string RpcTransportCtxTls::getX509Pem() {
    ...
    LOG_ALWAYS_FATAL_IF(ret.empty(), "%s: No pre-configured certificate!", toCString());
    return ret;
}
...
ssl_verify_result_t RpcTransportCtxTls::sslCustomVerify(SSL* rawSsl, uint8_t* outAlert) {
    ...
    bssl::UniquePtr<X509> peerCert;
    {
        ...
        LOG_TLS_DETAIL("%s: got peer certificate: %s", logPrefix, toString(peerCert.get()).c_str());
    }
...
std::unique_ptr<RpcTransportCtxTls> RpcTransportCtxTls::create() {
    ...
    LOG_TLS_DETAIL("%s: set up ctx with certificate: %s", ret->toCString(),
                   toString(cert.get()).c_str());
    return ret;
}

std::unique_ptr<RpcTransport> RpcTransportCtxTls::newTransport(android::base::unique_fd acceptedFd,
