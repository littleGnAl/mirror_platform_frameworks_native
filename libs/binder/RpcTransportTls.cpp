...
public:
    ...
    status_t pollForSslError(
            android::FdState& fd, int sslError, FdTrigger* fdTrigger, const char* fnString,
            int additionalEvent,
...
private:
    ...
    status_t handlePoll(int event, android::FdState& fd, FdTrigger* fdTrigger, const char* fnString,
                        const std::optional<android::base::function_ref<status_t()>>& altPoll) {
        status_t ret;
        if (altPoll) {
            ret = (*altPoll)();
...
public:
    RpcTransportTls(FdState socket, Ssl ssl) : mSocket(std::move(socket)), mSsl(std::move(ssl)) {}
    ...
    // TODO:Pawan
    bool isWaiting() { return mSocket.getPollingState(); };

private:
    android::FdState mSocket;
    Ssl mSsl;
};
...
bool setFdAndDoHandshake(Ssl* ssl, android::FdState& fdState, FdTrigger* fdTrigger) {
    bssl::UniquePtr<BIO> bio = newSocketBio(fdState.fd);
    ...
    while (true) {
        ...
        status_t pollStatus = errorQueue.pollForSslError(fdState, sslError, fdTrigger,
...
public:
    ...
    std::unique_ptr<RpcTransport> newTransport(FdState fd, FdTrigger* fdTrigger) const override;
...
std::unique_ptr<RpcTransport> RpcTransportCtxTls::newTransport(android::FdState fdState,
    ...
    TEST_AND_RETURN(nullptr, setFdAndDoHandshake(&wrapped, fdState, fdTrigger));
    return std::make_unique<RpcTransportTls>(std::move(fdState.fd), std::move(wrapped));
}
