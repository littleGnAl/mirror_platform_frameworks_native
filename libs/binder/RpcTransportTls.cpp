...
public:
    ...
    status_t pollForSslError(
            android::RpcSocket& fd, int sslError, FdTrigger* fdTrigger, const char* fnString,
            int additionalEvent,
...
private:
    ...
    status_t handlePoll(int event, android::RpcSocket& fd, FdTrigger* fdTrigger,
                        const char* fnString,
                        const std::optional<android::base::function_ref<status_t()>>& altPoll) {
        status_t ret;
        if (altPoll) {
...
public:
    RpcTransportTls(RpcSocket socket, Ssl ssl) : mSocket(std::move(socket)), mSsl(std::move(ssl)) {}
...
bool setFdAndDoHandshake(Ssl* ssl, android::RpcSocket& fdState, FdTrigger* fdTrigger) {
...
public:
    ...
    std::unique_ptr<RpcTransport> newTransport(RpcSocket fd, FdTrigger* fdTrigger) const override;
...
std::unique_ptr<RpcTransport> RpcTransportCtxTls::newTransport(android::RpcSocket fdState,
