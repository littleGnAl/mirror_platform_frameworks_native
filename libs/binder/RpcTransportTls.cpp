...
std::unique_ptr<RpcTransport> RpcTransportCtxTlsClient::newTransport(
    ...
    auto [verifyResult, verifyErrorQueue] = wrapped.call(SSL_get_verify_result);
    verifyErrorQueue.clear();
    if (verifyResult == X509_V_OK) {
        LOG_TLS_DETAIL("Client: verify OK!");
    } else {
        ALOGE("Client: verify error: %s", X509_verify_cert_error_string(verifyResult));
        return nullptr;
    }
