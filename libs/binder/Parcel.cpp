    ...
    if (auto* kernelFields = maybeKernelFields()) {
        ...
        for (size_t i = 0; i < otherRpcFields->mObjectPositions.size(); i++) {
            ...
            if (offset <= objPos && objPos < offset + len) {
                ...
                // We always dup, even if the FD was unowned in the source
                // parcel, to match kernel binder behavior.
                rpcFields->mFds->push_back(base::unique_fd(
                        fcntl(std::visit([&](const auto& fd) { return fd.get(); }, oldFd),
                              F_DUPFD_CLOEXEC, 0)));
