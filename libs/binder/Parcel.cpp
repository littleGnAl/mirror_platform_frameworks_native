...
status_t Parcel::appendFrom(const Parcel* parcel, size_t offset, size_t len) {
    ...
    if (kernelFields == nullptr || otherKernelFields == nullptr) {
        // Appending parcels is not support for RPC Binder because we don't know
        // where the FDs are in the data.
        //
        // Maybe need to send something akin to mObjects over the wire to make it
        // work.
        //
        // If we bring it back, make sure to re-add the check that the sessions
        // are equal.
        return BAD_TYPE;
    }
...
std::vector<base::borrowed_fd> Parcel::readRpcFileDescriptions() const {
    const auto* rpcFields = maybeRpcFields();
    LOG_ALWAYS_FATAL_IF(rpcFields == nullptr);
    std::vector<base::borrowed_fd> fds;
    fds.reserve(rpcFields->mFds.size());
    for (const auto& fd : rpcFields->mFds) {
        fds.emplace_back(std::visit([](const auto& x) { return x.get(); }, fd));
    }
    return fds;
}

// DO NOT SUBMIT: document caveat: doesn't work for rpc fds
...
void Parcel::markForRpc(const sp<RpcSession>& session) {
    ...
    LOG_ALWAYS_FATAL_IF(maybeRpcFields() != nullptr, "markForRpc called multiple times");

    // ALOGE("FMAYLEFMAYLE: markForRpc");
...
bool Parcel::enforceInterface(const char16_t* interface, size_t len,
...
status_t Parcel::writeFileDescriptor(int fd, bool takeOwnership) {
    if (auto* rpcFields = maybeRpcFields()) {
        if (!mAllowFds || !rpcFields->mSession->getEnableAncilleryFileDescriptors())
            return FDS_NOT_ALLOWED;
        // TODO: Share constant with transport code.
        constexpr size_t kMaxFdsPerMsg = 253;
        if (rpcFields->mFds.size() >= kMaxFdsPerMsg) {
            ALOGE("Too many file descriptors in Parcel: %zu (max is %zu)", rpcFields->mFds.size(),
                  kMaxFdsPerMsg);
            return BAD_VALUE;
        }
        status_t err = writeInt32(rpcFields->mFds.size());
        if (err != NO_ERROR) return err;
        if (takeOwnership) {
            rpcFields->mFds.emplace_back(base::unique_fd(fd));
        } else {
            rpcFields->mFds.emplace_back(base::borrowed_fd(fd));
        }
        return OK;
    }
...
status_t Parcel::write(const FlattenableHelperInterface& val) {
    ...
    if (buf == nullptr) return BAD_VALUE;
...
int Parcel::readFileDescriptor() const {
    ALOGE("FMAYLE: reading file descriptor");
    if (const auto* rpcFields = maybeRpcFields()) {
        ALOGE("FMAYLE: reading file descriptor FOR RPC with %zu mFds", rpcFields->mFds.size());
        int offset = readInt32();
        if (offset < 0 || static_cast<size_t>(offset) >= rpcFields->mFds.size()) {
            return BAD_INDEX;
        }
        return std::visit([&](const auto& fd) { return fd.get(); }, rpcFields->mFds[offset]);
    }
...
// DO NOT SUBMIT: Probably need to update for RPC-enabled.
int Parcel::readParcelFileDescriptor() const {
    ...
    if (hasComm != 0) {
        ...
        ssize_t written = TEMP_FAILURE_RETRY(::write(comm, &message, sizeof(message)));

        if (written != sizeof(message)) {
            ALOGW("Failed to detach ParcelFileDescriptor written: %zd err: %s", written,
                  strerror(errno));
            return BAD_TYPE;
        }
    }
...
status_t Parcel::readBlob(size_t len, ReadableBlob* outBlob) const {
    ...
    void* ptr =
            ::mmap(nullptr, len, isMutable ? PROT_READ | PROT_WRITE : PROT_READ, MAP_SHARED, fd, 0);
...
status_t Parcel::read(FlattenableHelperInterface& val) const {
    ...
    for (size_t i = 0; i < fd_count && err == NO_ERROR; i++) {
        ...
        if (fd < 0 || ((fds[i] = fcntl(fd, F_DUPFD_CLOEXEC, 0)) < 0)) {
            err = BAD_VALUE;
            ALOGE("fcntl(F_DUPFD_CLOEXEC) failed in Parcel::read, i is %zu, fds[i] is %d, fd_count "
                  "is %zu, error: %s",
...
const flat_binder_object* Parcel::readObject(bool nullMetaData) const {
    ...
    if ((DPOS + sizeof(flat_binder_object)) <= mDataSize) {
        ...
        if (N > 0) {
            ...
            if (OBJS[opos] == DPOS) {
                // Found it!
                ALOGV("Parcel %p found obj %zu at index %zu with forward search", this, DPOS, opos);
            ...
            if (OBJS[opos] == DPOS) {
                // Found it!
                ALOGV("Parcel %p found obj %zu at index %zu with backward search", this, DPOS,
                      opos);
                kernelFields->mNextObjectHint = opos + 1;
                ALOGV("readObject Setting data pos of %p to %zu", this, mDataPos);
                return obj;
...
void Parcel::ipcSetDataReference(const uint8_t* data, size_t dataSize, const binder_size_t* objects,
    ...
    auto* kernelFields = maybeKernelFields();
    LOG_ALWAYS_FATAL_IF(kernelFields == nullptr);
    // TODO: Simplify below, because always non-null.
    ...
    for (size_t i = 0; i < kernelFields->mObjectsSize; i++) {
        ...
        if (!(type == BINDER_TYPE_BINDER || type == BINDER_TYPE_HANDLE || type == BINDER_TYPE_FD)) {
            // We should never receive other types (eg BINDER_TYPE_FDA) as long as we don't
            // support them in libbinder. If we do receive them, it probably means a kernel bug;
...
void Parcel::rpcSetDataReference(const sp<RpcSession>& session, const uint8_t* data,
                                 size_t dataSize, std::vector<base::unique_fd> ancillaryFds,
    ...
    for (auto& fd : ancillaryFds) {
        rpcFields->mFds.emplace_back(std::move(fd));
    }
}
...
status_t Parcel::growData(size_t len) {
    ...
    return (newSize <= mDataSize) ? (status_t)NO_MEMORY
...
status_t Parcel::continueWrite(size_t desired) {
    ...
    // TODO: Might want to truncate mFds if data containing FDs is truncated.
    auto* kernelFields = maybeKernelFields();
