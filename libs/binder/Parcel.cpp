...
status_t Parcel::writeNullableParcelable(const std::unique_ptr<Parcelable>& parcelable) {
    if (!parcelable) {
        return writeInt32(0);
    }
...
status_t Parcel::peekInt32(int32_t *pArg) const {
    const int32_t start = dataPosition();
    status_t status = readInt32(pArg);
