...
status_t Parcel::flattenBinder(const sp<IBinder>& binder)
{
    if (mIsForRpc) {
        RpcAddress address;
        status_t status = mConnection->state()->onBinderLeaving(mConnection, binder, &address);
        if (status != OK) return status;
        // FIXME: delegate write to RpcWireAddress
        status = write(address.get(), sizeof(RpcWireAddress));
        if (status != OK) return status;
...
status_t Parcel::unflattenBinder(sp<IBinder>* out) const
{
    if (mIsForRpc) {
        ...
        sp<IBinder> binder = mConnection->state()->onBinderEntering(mConnection, addr);
