...
{
    if (mIsForRpc) {
        ALOGE("Flattening binder, to send %p", binder.get());
        const RpcAddress& address = RpcState::self().attachBinder(binder);
        ALOGE("Done flattening binder, to send %p", binder.get());
        // FIXME: delegate write to RpcWireAddress
        status_t status = write(address.get(), sizeof(RpcWireAddress));
        if (status != OK) return status;
        ALOGE("Wrote address for %p", binder.get());
...
status_t Parcel::unflattenBinder(sp<IBinder>* out) const
{
    if (mIsForRpc) {
        ...
        status_t status = read(addr.get(), sizeof(RpcWireAddress));
        if (status != OK) return status;
        // FIXME: remove lint disable when address is bigger
        // NOLINTNEXTLINE(performance-move-const-arg)
        sp<IBinder> binder = RpcState::self().lookupOrCreateProxy(mConnection, addr);
