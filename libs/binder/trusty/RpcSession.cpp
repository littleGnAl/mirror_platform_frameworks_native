...
status_t RpcSession::addConnection(std::unique_ptr<RpcTransport> rpcTransport, bool outgoing) {
    mConnection = sp<RpcConnection>::make();
    mConnection->rpcTransport = std::move(rpcTransport);

    status_t status = OK;
    if (outgoing) {
        status = mRpcBinderState->sendConnectionInit(mConnection,
                                                     sp<RpcSession>::fromExisting(this));
    } else {
        status = mRpcBinderState->readConnectionInit(mConnection,
                                                     sp<RpcSession>::fromExisting(this));
    }
