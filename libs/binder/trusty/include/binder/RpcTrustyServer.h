 ...
 */
class RpcTrustyServer : public virtual RefBase, public RpcServer {
public:
    static sp<RpcTrustyServer> make(
...
private:
    friend sp<RpcTrustyServer>;
    explicit RpcTrustyServer(std::unique_ptr<RpcTransportCtx> ctx) : RpcServer(std::move(ctx)) {}
