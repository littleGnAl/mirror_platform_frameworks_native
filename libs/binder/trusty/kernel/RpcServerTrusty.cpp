...
android::base::expected<sp<RpcServerTrusty>, int> RpcServerTrusty::make(
        const std::string& portName, std::shared_ptr<const PortAcl>&& portAcl, size_t msgMaxSize,
    ...
    rc = ktipc_server_add_port(&srv->mKtipcServer, &srv->mKtipcPort, &srv->kKtipcOps);
    if (rc < 0) {
        return unexpected(rc);
    }
...
RpcServerTrusty::RpcServerTrusty(std::unique_ptr<RpcTransportCtx> ctx, const std::string& portName,
    ...
    mKtipcPort.name = mPortName.c_str();
    mKtipcPort.msg_max_size = msgMaxSize;
    mKtipcPort.msg_queue_len = 6; // Three each way
    mKtipcPort.priv = this;
    mKtipcPort.uuid = &kernel_uuid;
    ...
    if (mPortAcl) {
        ...
        mKtipcPortAcl.flags = mPortAcl->flags;
        mKtipcPortAcl.uuid_num = numUuids;
        mKtipcPortAcl.uuids = mUuidPtrs.data();
        mKtipcPortAcl.extra_data = mPortAcl->extraData;

        mKtipcPort.acl = &mKtipcPortAcl;
    } else {
        mKtipcPort.acl = nullptr;
    }
}

int RpcServerTrusty::handleConnect(const ktipc_port* port, handle* chan, const uuid* peer,
                                   void** ctx_p) {
...
int RpcServerTrusty::handleMessage(const ktipc_port* port, handle* chan, void* ctx) {
...
void RpcServerTrusty::handleDisconnect(const ktipc_port* port, handle* chan, void* ctx) {}
