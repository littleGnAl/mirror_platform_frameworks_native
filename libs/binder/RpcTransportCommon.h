...
status_t interruptableReadOrWrite(int fd, FdTrigger* fdTrigger, iovec* iovs, int niovs,
    ...
    while (true) {
        ...
        // To avoid this problem, we manually skip over all zero-length
        // vectors.
        ...
        ssize_t processSize = sendOrReceiveFun(fd, iovs, niovs);
        if (processSize < 0) {
