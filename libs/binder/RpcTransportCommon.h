...
status_t interruptableReadOrWrite(const base::unique_fd& socket, FdTrigger* fdTrigger, iovec* iovs,
                                  int niovs, SendOrReceive sendOrReceiveFun, const char* funName,
    ...
    while (true) {
        ...
        ssize_t processSize = TEMP_FAILURE_RETRY(sendOrReceiveFun(socket.get(), iovs, niovs));
        if (processSize < 0) {
        ...
        if (altPoll) {
            ...
            if (status_t status = fdTrigger->triggerablePoll(socket.get(), event); status != OK)
                return status;
