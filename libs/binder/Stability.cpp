...
Stability::Summary Stability::Summary::currentFromLevel(Level level) {
    return {
...
status_t Stability::setRepr(IBinder* binder, int32_t representation, bool log) {
    auto current = getSummary(binder);
    auto setting = Summary::fromRepr(representation);
...
Stability::Summary Stability::getSummary(IBinder* binder) {
    if (binder == nullptr) {
        return Summary::currentFromLevel(Level::UNDECLARED);
    }

    BBinder* local = binder->localBinder();
    if (local != nullptr) {
        return Summary::fromRepr(local->mStability);
    }

    return Summary::fromRepr(binder->remoteBinder()->mStability);
}

bool Stability::check(Summary provided, Level required) {
