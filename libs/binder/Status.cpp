...
Status Status::fromExceptionCode(int32_t exceptionCode) {
    return Status(exceptionCode, 0);
}

Status Status::fromExceptionCode(int32_t exceptionCode,
                                 const String8& message) {
    return Status(exceptionCode, 0, message);
}

Status Status::fromApplicationError(int32_t applicationErrorCode) {
    return Status(EX_APPLICATION_SPECIFIC, applicationErrorCode);
}

Status Status::fromApplicationError(int32_t applicationErrorCode,
                                    const String8& message) {
    return Status(EX_APPLICATION_SPECIFIC, applicationErrorCode, message);
}
...
void Status::setApplicationError(const String8& message, int32_t errorCode) {
    setException(EX_APPLICATION_SPECIFIC, message);
    mErrorCode = errorCode;
}
