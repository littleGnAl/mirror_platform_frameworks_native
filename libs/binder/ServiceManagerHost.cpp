...
void DeviceServiceExtra::cleanup(const void* id, void* obj, void* /* cookie */) {
    ...
    auto ptr = static_cast<DeviceServiceExtra*>(obj);
    delete ptr;
}
...
sp<IBinder> getDeviceService(std::vector<std::string> serviceDispatcherArgs) {
    ...
    extra->commandResult = std::move(*result);
    // `servicedispatcher` process must be alive to keep the port open.
    if (extra->commandResult.exitCode.has_value()) {
        ALOGE("Command exits with: %s", extra->commandResult.toString().c_str());
        return nullptr;
    }
    if (!extra->commandResult.stderr.empty()) {
        LOG_HOST("servicedispatcher writes to stderr: %s", extra->commandResult.stderr.c_str());
    }
    ...
    unsigned int devicePort = parsePortNumber(extra->commandResult.stdout, "device port");
    ...
    binder->attachObject(kDeviceServiceExtraId, static_cast<void*>(extra.release()), nullptr,
