...
void RpcConnection::join() {
    ...
    unique_fd clientFd(TEMP_FAILURE_RETRY(accept4(hole.fd().get(), (struct sockaddr *)&clientSa, &clientSaLen, SOCK_CLOEXEC)));
    if (clientFd < 0) {
...
RpcConnection::ExclusiveHole::ExclusiveHole(const sp<RpcConnection>& connection) : mConnection(connection) {
    ...
    // FIXME: needs to round-robin these (otherwise, everything gets queued in
    //        one thread)
    pid_t tid = gettid();
