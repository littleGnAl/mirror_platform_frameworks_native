...
sp<IBinder> RpcConnection::getRootObject() {
    ...
    sp<IBinder> binder = RpcState::self().lookupOrCreateProxy(this, std::make_shared<RpcWireAddress>());
...
status_t RpcConnection::transact(const RpcAddress& address, uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags) {
    return RpcState::self().transact(mFd, address, code, data, this, reply, flags);
}

status_t RpcConnection::sendDecStrong(const RpcAddress& address) {
    return RpcState::self().sendDecStrong(mFd, address);
}

void RpcConnection::debugDump() {
    RpcState::self().dump();
}
