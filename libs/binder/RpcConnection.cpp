...
RpcConnection::ExclusiveSocket::ExclusiveSocket(const sp<RpcConnection>& connection)
  : mConnection(connection) {
    ...
    while (true) {
        ...
        // In the server case, currently this will always find and use the
        // nested TID being used by the server. Though - this could cause
        // deadlocks when calls are made from other threads on the server, in
        // response to an IPC. Discussing on email thread:
        // "How do nested transactions work when a server makes them from
        // another thread?"
        // FIXME: probably rework all of this
        ...
        // FIXME: abort hard if
        // - this is not a nested call, and no client is available (remind dev
        //   to start threadpool)
        // - this is a oneway call, and it could only be a nested call

        LOG_RPC_DETAIL("No available connection (have %zu clients and %zu servers). Waiting...",
                       mConnection->mClients.size(),
                       mConnection->mServers.size());
...
void RpcConnection::ExclusiveSocket::findSocket(
    ...
    for (sp<ConnectionSocket>& socket : sockets) {
        // take first available connection (intuition = caching)
        if (*available == nullptr && socket->exclusiveTid == std::nullopt) {
            *available = socket;
            continue;
        }
