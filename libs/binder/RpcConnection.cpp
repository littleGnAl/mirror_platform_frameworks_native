...
status_t RpcConnection::transact(int address, uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags) {
    ...
    if (sizeof(RpcCommand) != TEMP_FAILURE_RETRY(send(mFd.get(), &command, sizeof(command), 0))) {
        ALOGE("Failed to send command header: %s", strerror(errno));
        return UNKNOWN_ERROR;
    }

    // FIXME: bad cast
    if ((int)transactionData.size() != TEMP_FAILURE_RETRY(send(mFd.get(), transactionData.data(), transactionData.size(), 0))) {
