...
void RpcConnection::join() {
    ...
    LOG_RPC_DETAIL("accept4 on fd %d yields fd %d", socket.fd().get(), clientFd.get());

    {
        std::lock_guard<std::mutex> _l(mSocketMutex);
        // FIXME: we should accept4 multiple clients and reuse single listen,
        // rather than only supporting one connection per socket.
...
RpcConnection::ExclusiveSocket::ExclusiveSocket(const sp<RpcConnection>& connection)
  : mConnection(connection) {
    ...
    while (true) {
        ...
        LOG_RPC_DETAIL("No available connection. Waiting.");
