...
void RpcConnection::join() {
    ...
    // FIXME: cleanup state (there is a strong dep cycle binder -> connection ->
    // state -> strong counts)
}
...
RpcConnection::ExclusiveSocket::ExclusiveSocket(const sp<RpcConnection>& connection,
  ...
  : mConnection(connection) {
    ...
    // FIXME: factor server lookup here, since we are only interested in a very
    // specific case, and we don't need to retry?
