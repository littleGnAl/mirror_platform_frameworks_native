...
sp<IBinder> RpcConnection::getRootObject() {
    ExclusiveSocket socket(this, nullptr, SocketUse::CLIENT);
...
status_t RpcConnection::sendDecStrong(const RpcAddress& address) {
    ExclusiveSocket socket(this, &address, SocketUse::CLIENT_REFCOUNT);
...
void RpcConnection::join() {
    ...
    ExclusiveSocket socket(this, nullptr, SocketUse::SERVER);
...
RpcConnection::ExclusiveSocket::ExclusiveSocket(const sp<RpcConnection>& connection,
  ...
  : mConnection(connection) {
    // FIXME: needs right oneway queueing behavior, and comparison test.
    //
    ...
    // might drown out calls from another thread. This is particularly dangerous
    // in the case of oneway calls, since many oneway calls will be sent to the
    // first FD without looking at others.
    (void) address;
    ...
    while (true) {
        ...
        if (use != SocketUse::CLIENT_ASYNC) {
            ...
            findSocket(tid,
                       &exclusive,
                       (useAvailable ? &available : nullptr),
                       mConnection->mServers);
        }
