    ...
    status_t interruptableReadFully(
        ...
        auto readFn = [&](iovec* iovs, size_t niovs) -> ssize_t {
            ...
            for (size_t i = 0; i < niovs; i++) {
                auto& iov = iovs[i];
                for (size_t iovPos = 0; iovPos < iov.iov_len;) {
                    status_t ret = fillReadBuffer();
                    if (ret != OK) {
                        return ret;
                    }

                    size_t iovLeft = iov.iov_len - iovPos;
                    size_t bufferLeft = mReadBufferSize - mReadBufferPos;
                    // Iovecs should not cross message boundaries for any
                    // current implementation, so if it happens then it's
                    // probably a bug. We should log this so it can be
                    // investigated later.
                    ALOGE_IF(iovLeft > bufferLeft, "Iov %zu crosses message boundary (%zu>%zu)", i,
                             iovLeft, bufferLeft);

                    size_t numBytes = std::min(iovLeft, bufferLeft);
                    memcpy(reinterpret_cast<uint8_t*>(iov.iov_base) + iovPos,
                           mReadBuffer.get() + mReadBufferPos, numBytes);
                    iovPos += numBytes;
