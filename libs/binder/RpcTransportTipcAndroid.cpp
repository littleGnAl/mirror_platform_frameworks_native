    ...
    status_t interruptableReadFully(
        ...
        auto readFn = [&](iovec* iovs, size_t niovs) -> ssize_t {
            ...
            for (size_t i = 0; i < niovs; i++) {
                ...
                auto& iov = iovs[i];
                size_t bufferLeft = mReadBuffer.size() - mReadBufferPos;
                LOG_ALWAYS_FATAL_IF(iov.iov_len > bufferLeft,
                                    "Iov %zu crosses message boundary (%zu>%zu)", i, iov.iov_len,
                                    bufferLeft);
...
private:
    status_t fillReadBuffer() {
        ...
        mReadBuffer.reserve(kDefaultBufferSize);

        while (true) {
            // Resize to full capacity so all bytes from data() to data()+size()
            // are valid
            mReadBuffer.resize(mReadBuffer.capacity());
            ...
            if (processSize == 0) {
                ...
                if (savedErrno == EMSGSIZE) {
                    ...
                    mReadBuffer.reserve(mReadBuffer.capacity() * 2);
                    continue;
                } else {
                    // Clear the internal buffer. We might return to this
                    // function later if e.g. read() returned EAGAIN
                    mReadBuffer.clear();
