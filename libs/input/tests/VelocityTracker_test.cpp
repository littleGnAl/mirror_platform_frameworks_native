...
MotionEvent* createMotionEventStream(const Position* positions, size_t numSamples) {
    MotionEvent* event = new MotionEvent();
    PointerCoords coords;
    PointerProperties properties[1];
...
static void computeAndCheckVelocityLsq2(const Position* positions, size_t numSamples,
    ...
    MotionEvent* event = createMotionEventStream(positions, numSamples);
...
TEST_F(VelocityTrackerTest, solveUnweightedLeastSquaresDeg2Test) {
    ...
    size_t count = sizeof(values) / sizeof(Position);
    computeAndCheckVelocityLsq2(values, count, AMOTION_EVENT_AXIS_X, 0);
    computeAndCheckVelocityLsq2(values, count, AMOTION_EVENT_AXIS_Y, 0);
}
