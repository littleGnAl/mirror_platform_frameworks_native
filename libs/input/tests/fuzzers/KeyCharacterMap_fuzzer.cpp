...
#define TEMP_FILE_PATH "/data/local/tmp/keycharactermapXXXXXXXXX"
size_t TEMP_FILE_NAME_LEN = sizeof(TEMP_FILE_PATH);

android::KeyCharacterMap::Format getFormat(uint16_t option) {
    if (option == 0)
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    android::KeyCharacterMap::Format format = getFormat(tester.ConsumeIntegralInRange(0, 3));

    char filePath[TEMP_FILE_NAME_LEN];
    memcpy(filePath, TEMP_FILE_PATH, TEMP_FILE_NAME_LEN);
