...
#define LSQ2_STRATEGY "lsq2"
#define IMPULSE_STRATEGY "impulse"
#define LSQ3_STRATEGY "lsq3"
#define WLSQ2_DELTA_STRATEGY "wlsq2-delta"
#define WLSQ2_CENTRAL_STRATEGY "wlsq2-central"
#define WLSQ2_RECENT_STRATEGY "wlsq2-recent"
#define INT1_STRATEGY "int1"
#define INT2_STRATEGY "int2"
#define LEGACY_STRATEGY "legacy"
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    for (size_t i = 1; i < vectorSize; i++) {
        ...
        for (size_t j = 0; j < positionsCount; j++) {
            float x = fdp.ConsumeFloatingPointInRange<float>(0, 5000);
            float y = fdp.ConsumeFloatingPointInRange<float>(0, 5000);
            Position pos;
            pos.x = x;
            pos.y = y;
            if (!pos.isValid()) {
    ...
    // Fuzzing is done using each strategies
    std::vector<const char*> strategies;

    strategies.push_back(LSQ2_STRATEGY);
    strategies.push_back(IMPULSE_STRATEGY);
    strategies.push_back(LSQ3_STRATEGY);
    strategies.push_back(WLSQ2_DELTA_STRATEGY);
    strategies.push_back(WLSQ2_CENTRAL_STRATEGY);
    strategies.push_back(WLSQ2_RECENT_STRATEGY);
    strategies.push_back(INT1_STRATEGY);
    strategies.push_back(INT2_STRATEGY);
    strategies.push_back(LEGACY_STRATEGY);

    uint32_t randomStrategy = fdp.ConsumeIntegralInRange<uint32_t>(0, strategies.size() - 1);

    computeVelocity(strategies.at(randomStrategy), motions);
    computeQuadraticEstimate(strategies.at(randomStrategy), motions);
