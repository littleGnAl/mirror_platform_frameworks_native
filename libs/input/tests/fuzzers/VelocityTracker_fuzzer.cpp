...
// Function to create MotionEvent from FuzzedDataProvider
std::optional<std::vector<MotionEvent>> getMotionEventFromFDP(FuzzedDataProvider* fdp) {
    ...
    return createMotionEventStream(motions);
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    std::optional<std::vector<MotionEvent>> events = getMotionEventFromFDP(&fdp);
