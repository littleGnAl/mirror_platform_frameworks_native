...
// There can be 48 different axis types that pointerCoords can have.
static constexpr uint32_t AXIS_TYPE_MAX_LENGTH = 48;

enum class EventType {
    KEY_EVENT = 0,
    MOTION_EVENT = 1,
};
static constexpr size_t EVENT_TYPE_MAX_LENGTH = 2;
...
extern "C" int LLVMFuzzerTestOneInput(const char* data, size_t size) {
    ...
    EventType eventType =
            static_cast<EventType>(fdp.ConsumeIntegralInRange<uint8_t>(0, EVENT_TYPE_MAX_LENGTH));
        ...
        case EventType::MOTION_EVENT: {
            ...
            for (size_t i = 0; i < pointerCount; i++) {
                ...
                pointerProperties[i].id =
                        (i + 2) % pointerCount; // (i+2) just to change the order of id
                pointerProperties[i].toolType = fdp.ConsumeIntegral<int32_t>();

                pointerCoords[i].clear();
                for (size_t j = 0;
                     j < fdp.ConsumeIntegralInRange<uint32_t>(0, PointerCoords::MAX_AXES); j++) {
                    pointerCoords[i]
