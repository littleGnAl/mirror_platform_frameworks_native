...
struct __attribute__ ((__packed__)) HDRStaticInfo {
    ...
    struct __attribute__ ((__packed__)) Type2 {
        ...
        uint32_t mAmbientIlluminance; //in 0.0001 cd/m^2
        Primaries1 mAmbientLight; //chromaticity


        // content color volume
        // Valid only if bit 3 of mValidFields is 1
        uint8_t mCCVCancelFlag;
        uint8_t mCCVPersistenceFlag;
        uint8_t mCCVPrimariesPresentFlag;
        uint8_t mCCVMaxContentLuminancePresentFlag;
        uint8_t mCCVMinContentLuminancePresentFlag;
        uint8_t mCCVAvgContentLuminancePresentFlag;
