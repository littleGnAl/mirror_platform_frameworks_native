...
class LayerLibrary {
   public:
    ...
    void* GetGPA(const Layer& layer, const std::string& gpa_name) const;
...
bool LayerLibrary::EnumerateLayers(size_t library_idx,
    ...
    if (properties == nullptr) {
        ALOGE(
            "vkEnumerateInstanceLayerProperties failed allocation for library '%s': %d",
            path_.c_str(), result);
        return false;
    }
    result = enumerate_instance_layers(&num_instance_layers, properties.get());
    if (result != VK_SUCCESS) {
...
void* LayerLibrary::GetGPA(const Layer& layer,
                           const std::string& gpa_name) const {
    std::string layer_name { layer.properties.layerName };
    if (void* gpa = GetTrampoline((layer_name + gpa_name).c_str()))
      return gpa;
    return GetTrampoline(("vk" + gpa_name).c_str());
}
