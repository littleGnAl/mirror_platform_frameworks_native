 ...
 * Subsequent calls always replace any existing parameters. The socket tag is
 * kept when the socket is sent to another process using binder IPCs or other
 * mechanisms such as UNIX socket fd passing. Any app can accept blame for
 * future traffic performed on a socket originally created by another app by
 * calling this method with its own UID. However, only apps holding the
