...
public:
    ...
    status_t            writeNullableParcelable(const std::unique_ptr<Parcelable>& parcelable);
    ...
    status_t            peekInt32(int32_t *pArg) const;
    ...
    template<typename T>
    status_t            readNullableParcelable(std::unique_ptr<T>* parcelable) const;
...
status_t Parcel::readTypedVector(std::unique_ptr<std::vector<T>>* val,
                                 status_t(Parcel::*read_func)(T*) const) const {
    int32_t size;
    status_t status = peekInt32(&size);
    val->reset();
...
status_t Parcel::readParcelableVector(std::unique_ptr<std::vector<std::unique_ptr<T>>>* val) const {
    ...
    status = unsafeReadTypedVector(val->get(), &Parcel::readNullableParcelable);
...
template<typename T>
status_t Parcel::readNullableParcelable(std::unique_ptr<T>* parcelable) const {
    int32_t present;
    status_t status = peekInt32(&present);
    parcelable->reset();
