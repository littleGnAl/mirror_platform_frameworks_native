...
#include "external/perfetto/protos/perfetto/android_vendor/atrace_categories.pb.h"
...
const char* kVendorCategoriesPath = "/vendor/etc/atrace/atrace_categories.pb";
...
void initVendorCategoriesFromFile() {
    std::string buf;
    if (!android::base::ReadFileToString(kVendorCategoriesPath, &buf)) {
        fprintf(stderr, "error opening vendor categories file %s: %s (%d)\n", kVendorCategoriesPath,
                strerror(errno), errno);
    }
    perfetto::protos::atrace::Categories categories;
    if (!categories.ParseFromString(buf)) {
        fprintf(stderr, "cannot parse vendor categories file %s\n", kVendorCategoriesPath);
    }

    g_vendorFileCategories.reserve(categories.categories().size());
    for (const auto& file_cat : categories.categories()) {
        TracingVendorFileCategory cat;
        cat.name = file_cat.name();
        cat.description = file_cat.description();
        for (const auto& group : file_cat.groups()) {
            for (const std::string& event : group.events()) {
                cat.ftrace_enable_paths.push_back("events/" + group.name() + "/" + event +
                                                  "/enable");
            }
        }
        g_vendorFileCategories.push_back(cat);
    }
}
