...
static void pokeHalServices()
{
    ...
    Return<bool> ret = sm->broadcastNotifySyspropsChanged();

    if (!ret.isOk()) {
        fprintf(stderr, "transaction failed when poking hal services %s\n", ret.description().c_str());
    } else if (!ret) {
        fprintf(stderr, "failed to poke hal services\n");
    }
}
