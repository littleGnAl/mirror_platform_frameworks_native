...
using AidlIAtraceDevice = aidl::android::hardware::atrace::IAtraceDevice;
using AidlTracingCategory = aidl::android::hardware::atrace::TracingCategory;
...
class VendorAtrace {
  public:
    ...
    virtual bool listCategories(std::vector<TracingVendorCategory>* out) = 0;
    virtual bool enableCategories(const std::vector<std::string>& names) = 0;
    virtual bool disableAllCategories() = 0;
...
class NullVendorAtrace : public VendorAtrace {
  public:
    ...
    bool listCategories(std::vector<TracingVendorCategory>*) override {
        return true;
    };
    bool enableCategories(const std::vector<std::string>&) override {
        return true;
    }
    bool disableAllCategories() override {
        return true;
    }
};

class HidlV0VendorAtrace : public VendorAtrace {
 public:
  HidlV0VendorAtrace(sp<IAtraceDevice> hal) : hal_(hal) {
  }
  ~HidlV0VendorAtrace() = default;

  bool listCategories(std::vector<TracingVendorCategory>* categories) override {
    ...
    if (!ret.isOk()) {
        PrintError(ret.description());
        return false;
    }
    return true;
  }

  bool enableCategories(const std::vector<std::string>& categories) override {
...
class AidlVendorAtrace : public VendorAtrace {
 public:
  AidlVendorAtrace(std::shared_ptr<AidlIAtraceDevice> hal) : hal_(hal) {
  }
  ~AidlVendorAtrace() = default;

  bool listCategories(std::vector<TracingVendorCategory>* categories) override {
      std::vector<AidlTracingCategory> out;
      ...
      if (!ret.isOk()) {
          PrintError(ret.getMessage());
      }
      return true;
  }

  bool enableCategories(const std::vector<std::string>& categories) override {
      ndk::ScopedAStatus ret = hal_->enableCategories(categories);
      if (!ret.isOk()) {
          PrintError(ret.getMessage());
      }
      return true;
  ...
  bool disableAllCategories() override {
      ndk::ScopedAStatus ret = hal_->disableAllCategories();
      if (!ret.isOk()) {
          PrintError(ret.getMessage());
      }
      return true;
  ...
  private:
    std::shared_ptr<AidlIAtraceDevice> hal_;
};
...
VendorAtrace* GetVendorAtrace() {
    ...
    if (!impl) {
        const auto instance = std::string() + AidlIAtraceDevice::descriptor + "/default";
        auto atraceBinder = ndk::SpAIBinder(AServiceManager_getService(instance.c_str()));
        std::shared_ptr<AidlIAtraceDevice> device(AidlIAtraceDevice::fromBinder(atraceBinder));
        if (device) {
    ...
    if (!impl) {
        ...
        if (svc) {
            impl = std::unique_ptr<VendorAtrace>(new HidlV0VendorAtrace(std::move(svc)));
        }
    }

    if (!impl) {
        fprintf(stderr, "Could not connect to Atrace HAL\n");
...
{
    GetVendorAtrace()->listCategories(&g_vendorCategories);
}
...
static bool setUpVendorTracing()
{
    ...
    return GetVendorAtrace()->enableCategories(categories);
}
...
static bool cleanUpVendorTracing()
{
    ...
    return GetVendorAtrace()->disableAllCategories();
}
