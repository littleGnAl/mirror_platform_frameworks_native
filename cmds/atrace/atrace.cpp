...
bool findTraceFiles()
{
    FILE *fp = popen("/system/bin/mount", "r");
    if (fp == NULL) {
        fprintf(stderr, "Failed to run mount\n");
        return false;
    }

    std::string line, debugfs, tracefs;
    while(!feof(fp)) {
        char buffer[256];
        ...
        auto first = line.substr(0, first_space);

        auto second_space = line.find_first_of(' ', first_space + 1);
        if (second_space == std::string::npos) {
            continue;
        }
        auto second = line.substr(first_space + 1, second_space - first_space - 1);

        auto third_space = line.find_first_of(' ', second_space + 1);
        if (third_space == std::string::npos) {
            continue;
        }
        auto third = line.substr(second_space + 1, third_space - second_space - 1);

        if (first == "debugfs" && second == "on") {
            if (!debugfs.empty()) {
                fprintf(stderr, "Error: Two debugfs entries\n");
                debugfs.clear();
                tracefs.clear();
                break;
            }
            debugfs = third;
        } else if (first == "tracefs" && second == "on") {
            if (!tracefs.empty()) {
                fprintf(stderr, "Error: Two tracefs entries\n");
                debugfs.clear();
                tracefs.clear();
                break;
            }
    ...
    if (tracefs.empty() && debugfs.empty()) {
        fprintf(stderr, "Error: Did not find trace folder\n");
        return false;
    }

    if (!tracefs.empty() && !debugfs.empty()) {
        fprintf(stderr, "Error: Both tracefs and debugfs mounted\n");
        return false;
    }

    if (tracefs.empty()) {
        g_traceFolder = debugfs + "/tracing/";
    } else {
        g_traceFolder = tracefs + "/";
    }
