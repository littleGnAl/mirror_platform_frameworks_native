...
// Clear the reference profile for the given location.
// The location is the package name for primary apks or the dex path for secondary dex files.
static bool clear_current_profiles(const std::string& location, bool is_secondary_dex) {
    if (is_secondary_dex) {
...
bool clear_current_profiles(const std::string& pkgname) {
    return clear_current_profiles(pkgname, /*is_secondary_dex*/false);
}
...
static unique_fd open_secondary_dex_reference_profile(uid_t uid, const std::string& dex_path,
        bool read_write) {
...
// a re-compilation of the location.
...
Dex2oatFileWrapper maybe_open_reference_profile(const std::string& pkgname,
    ...
    unique_fd ufd = is_secondary_dex
        ? open_secondary_dex_reference_profile(uid, dex_path, /*read_write*/false)
...
static bool process_secondary_dex_dexopt(const char* dex_path, const char* pkgname,
    ...
    bool profile_was_updated = analyse_profiles(uid, profiles_fd, reference_profile_fd, dex_path,
