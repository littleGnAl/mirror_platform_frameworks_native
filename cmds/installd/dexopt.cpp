...
static unique_fd open_profile(int uid, const std::string& profile, bool read_write) {
    ...
    if (fd.get() < 0) {
        if (errno != ENOENT) {
            ...
            // after we kick in a merge attempt. In this case the current profile
            // will not be found.
            // Also, the profiles for secondary dex files might be deleted by the
            // app at any time, so we can't expect the same strong guarantees that
            // we will find them.
...
static bool process_secondary_dex_dexopt(const char* original_dex_path, const char* pkgname,
    ...
    {
        // Secondary dex files might contain symlinks.
        // Resolve them before validating to make sure we they refer a path inside the application
        // directory.
