...
// SELinux labels of files in app data directory.
static constexpr const char* kAppDataFileSELinuxContext = ":app_data_file:";
static constexpr const char* kAppExecDataFileSELinuxContext = ":app_exec_data_file:";
...
static bool restorecon_pkgdir_file(const std::string& path,
                                   ...
                                   bool is_secondary_dex) {
  if (is_secondary_dex) {
    return selinux_android_restorecon_pkgdir(path.c_str(), se_info, uid, /* flags= */ 0u) == 0;
  }
  return true;
}

static bool set_app_exec_selinux_label(const std::string& path, bool is_secondary_dex) {
  if (is_secondary_dex) {
    std::string app_data_con;

    {
      ...
      app_data_con.assign(con);
      freecon(con);
    }

    size_t index = app_data_con.find(kAppDataFileSELinuxContext);
    if (index == std::string::npos) {
      return false;
    }

    std::string app_exec_con = app_data_con.replace(
        index, strlen(kAppDataFileSELinuxContext), kAppExecDataFileSELinuxContext);
    if (setfilecon(path.c_str(), app_exec_con.c_str()) < 0) {
      return false;
    }
  }
  return true;
...
Dex2oatFileWrapper open_oat_out_file(const char* apk_path, const char* oat_dir,
    ...
    if (wrapper_fd.get() < 0) {
        ...
        ALOGE("installd cannot restorecon '%s' for output during dexopt\n", out_oat_path);
        wrapper_fd.reset(-1);
    } else if (!set_app_exec_selinux_label(out_oat_path, is_secondary_dex)) {
        ALOGE("installd cannot setfilecon '%s' for output during dexopt\n", out_oat_path);
        wrapper_fd.reset(-1);
    }
    return wrapper_fd;
}
