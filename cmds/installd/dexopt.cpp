...
class DexOptStatus {
public:
    // add PID of forked dexopt pids to the record
    void add_dexopt_pid(pid_t pid) {
    ...
    void control_dexopt_blocking(bool block) {
        ...
        if (!block) {
            dexopt_pids_.clear();
            dexopt_killed_pids_.clear();
            return;
        }
        // Blocked, also kill currently running tasks
        for (auto pid : dexopt_pids_) {
            LOG(INFO) << "cancel pending dexopt, killing child pid:" << pid;
            kill(pid, SIGTERM);
        }
        // Give small time for cleanup but cannot wait for long time as this directly adds
        // delay to the cancellation and user can be waiting.
        if (!dexopt_pids_.empty()) {
...
static SecondaryDexOptProcessResult process_secondary_dex_dexopt(const std::string& dex_path,
    ...
    /* parent */
    dexopt_status_->add_dexopt_pid(pid);
