...
UniqueFile maybe_open_app_image(const std::string& out_oat_path,
    ...
    if (!generate_app_image) {
        UniqueFile::RemoveFileAndBackup(image_path);
        return UniqueFile();
    }
    ...
    UniqueFile image_file = UniqueFile::CreateWritableFileWithBackup(image_path,
        /*permissions*/ 0600, UnlinkIgnoreResult);
    if (image_file.fd() < 0) {
...
bool open_vdex_files_for_dex2oat(const char* apk_path, const char* out_oat_path, int dexopt_needed,
    ...
    if (update_vdex_in_place) {
        ...
        *out_vdex_wrapper = UniqueFile::CreateWritableFileWithBackup(out_vdex_path_str,
            /*permissions*/0644, UnlinkIgnoreResult);
...
UniqueFile open_oat_out_file(const char* apk_path, const char* oat_dir,
    ...
    UniqueFile oat = UniqueFile::CreateWritableFileWithBackup(out_oat_path, /*permissions*/0644,
        UnlinkIgnoreResult);
    if (oat.fd() < 0) {
