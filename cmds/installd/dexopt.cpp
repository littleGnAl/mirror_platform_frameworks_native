...
std::vector<std::string> SplitBySpaces(char *buf) {
...
static void run_dex2oat(int zip_fd, int oat_fd, int input_vdex_fd, int output_vdex_fd, int image_fd,
    ...
    std::string dex2oat_Xms_arg;
    if (get_property("dalvik.vm.dex2oat-Xms", dex2oat_Xms_flag, nullptr) > 0) {
        dex2oat_Xms_arg = StringPrintf("-Xms%s", dex2oat_Xms_flag);
    }

    char dex2oat_Xmx_flag[kPropertyValueMax];
    std::string dex2oat_Xmx_arg;
    if (get_property("dalvik.vm.dex2oat-Xmx", dex2oat_Xmx_flag, nullptr) > 0) {
        dex2oat_Xmx_arg = StringPrintf("-Xmx%s", dex2oat_Xmx_flag);
    }

    char dex2oat_threads_buf[kPropertyValueMax];
    std::string dex2oat_threads_arg;
    const char* threads_property = post_bootcomplete
            ? "dalvik.vm.dex2oat-threads"
            : "dalvik.vm.boot-dex2oat-threads";
    if (get_property(threads_property, dex2oat_threads_buf, nullptr) > 0) {
        dex2oat_threads_arg = StringPrintf("-j%s", dex2oat_threads_buf);
    }
    ...
    if (get_property(dex2oat_isa_features_key.c_str(), dex2oat_isa_features, nullptr) > 0) {
        instruction_set_features_arg = StringPrintf("--instruction-set-features=%s",
                                                    dex2oat_isa_features);
    }
    ...
    if (get_property(dex2oat_isa_variant_key.c_str(), dex2oat_isa_variant, nullptr) > 0) {
        instruction_set_variant_arg = StringPrintf("--instruction-set-variant=%s",
                                                   dex2oat_isa_variant);
    }
    ...
    std::vector<std::string> dex2oat_flags_args;
    if (get_property("dalvik.vm.dex2oat-flags", dex2oat_flags, nullptr) > 0) {
        dex2oat_flags_args = SplitBySpaces(dex2oat_flags);
    }
    ...
    if (image_fd >= 0 && get_property("dalvik.vm.appimageformat", app_image_format, nullptr) > 0) {
        image_format_arg = StringPrintf("--image-format=%s", app_image_format);
    }

    char dex2oat_large_app_threshold[kPropertyValueMax];
    std::string dex2oat_large_app_threshold_arg;
    if (get_property("dalvik.vm.dex2oat-very-large", dex2oat_large_app_threshold, nullptr) > 0) {
        dex2oat_large_app_threshold_arg = StringPrintf("--very-large-app-threshold=%s",
    ...
    if (dex2oat_compiler_filter_arg.empty()) {
        ...
        if (get_property("dalvik.vm.dex2oat-filter", dex2oat_compiler_filter_flag, nullptr) > 0) {
