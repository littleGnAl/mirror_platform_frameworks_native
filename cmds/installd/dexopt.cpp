...
int dexopt(const char* apk_path, uid_t uid, const char* pkgname, const char* instruction_set,
    ...
    char* real_compiler_filter = const_cast<char*>(compiler_filter);
    ...
    if (skip_compilation) {
        real_compiler_filter = const_cast<char*>("verify-none");
        dexopt_flags &= ~DEXOPT_PROFILE_GUIDED;
    } else if (vm_safe_mode) {
        real_compiler_filter = const_cast<char*>("interpret-only");
        dexopt_flags &= ~DEXOPT_PROFILE_GUIDED;
    }

    // profile guide compilation was changed to be same as install action.
    // so there is no need to compile the app again.
    char install_compiler_filter[kPropertyValueMax];
    if (profile_guided
         && get_property("pm.dexopt.install", install_compiler_filter, "") > 0
         && !strstr(install_compiler_filter, "profile")
         && strcmp(real_compiler_filter, install_compiler_filter) == 0) {
        return -1;
    }

    profile_guided = (dexopt_flags & DEXOPT_PROFILE_GUIDED) != 0;
