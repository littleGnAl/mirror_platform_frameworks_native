...
static int analyze_profiles(uid_t uid, const std::string& package_name,
    ...
    if (profiles_fd.empty() || (reference_profile_fd.get() < 0)) {
        ...
        return PROFILES_ANALYSIS_NO_OPTIMIZE_EMPTY_PROFILES;
    }
    ...
    if (need_to_compile) {
        ...
        result = PROFILES_ANALYSIS_NO_OPTIMIZE_EMPTY_PROFILES;
    } else {
        result = PROFILES_ANALYSIS_NO_OPTIMIZE;
    }
    return result;
}
...
//
// Return PROFILES_ANALYSIS_NO_OPTIMIZE if the package should not optimize, and, as a special
// case returns PROFILES_ANALYSIS_NO_OPTIMIZE_EMPTY_PROFILES is all profiles are empty.
