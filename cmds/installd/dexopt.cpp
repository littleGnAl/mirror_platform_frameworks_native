...
bool open_vdex_files(const char* apk_path, const char* out_oat_path, int dexopt_needed,
    ...
    if (dexopt_action != DEX2OAT_FROM_SCRATCH) {
        ...
        // 2) We dex2oat due to boot image change, because we then know the existing vdex file cannot
        //    be currently used by a running process.
        // 3) We are not doing a profile guided compilation, because dexlayout requires two different
        //    vdex files to operate.
