...
int dexopt(const char* dex_path, uid_t uid, const char* pkgname, const char* instruction_set,
    ...
    *completed = true;
    ...
    // We've been successful, commit work files and keep profile.
    // If committing (=renaming tmp to regular) fails, try to make it consistent. If consistency
    // cannot be guaranteed, remove files.
    reference_profile.DisableCleanup();
    if (!out_oat.CommitWorkFile()) {
        LOG(ERROR) << "Cannot commit out_oat:" << out_oat.path();
        // out_oat fully removed. out_vdex, out_image are still old files
        return -1;
    }
    if (!out_vdex.CommitWorkFile()) {
        LOG(ERROR) << "Cannot commit out_vdex:" << out_vdex.path();
        // out_oat and out_vdex fully removed. out_image keep the old file
        RestorableFile::RemoveAllFiles(out_oat.path());
        return -1;
    }
    if (!out_image.CommitWorkFile()) {
        LOG(ERROR) << "Cannot commit out_image:" << out_image.path();
        // All three files are completely removed.
        RestorableFile::RemoveAllFiles(out_oat.path());
        RestorableFile::RemoveAllFiles(out_vdex.path());
        return -1;
    }
