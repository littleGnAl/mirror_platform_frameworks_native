...
class RunDex2OatTest : public testing::Test {
  public:
    ...
    // results. Alternatively, mock out the class to avoid the side effect.
    ...
    struct RunDex2OatArgs {
        static std::unique_ptr<RunDex2OatArgs> MakeDefaultTestArgs() {
            ...
            args->input_dex = std::make_unique<UniqueFile>(ZIP_FD, INPUT_PATH);
            args->output_oat = std::make_unique<UniqueFile>(OAT_FD, OUTPUT_PATH);
            args->input_vdex = std::make_unique<UniqueFile>(INPUT_VDEX_FD, "UNUSED_PATH");
            args->output_vdex = std::make_unique<UniqueFile>(OUTPUT_VDEX_FD, "UNUSED_PATH");
            args->instruction_set = "arm64";
            args->compilation_reason = "rundex2oattest";
            return args;
        }

        std::unique_ptr<UniqueFile> output_oat;
        std::unique_ptr<UniqueFile> output_vdex;
        std::unique_ptr<UniqueFile> output_image;
        std::unique_ptr<UniqueFile> input_dex;
        std::unique_ptr<UniqueFile> input_vdex;
        std::unique_ptr<UniqueFile> dex_metadata;
        std::unique_ptr<UniqueFile> profile;
        int swap_fd = -1;
    ...
    void CallRunDex2Oat(std::unique_ptr<RunDex2OatArgs> args) {
        ...
        runner.Initialize(args->output_oat.get(),
                          args->output_vdex.get(),
                          args->output_image.get(),
                          args->input_dex.get(),
                          args->input_vdex.get(),
                          args->dex_metadata.get(),
                          args->profile.get(),
...
TEST_F(RunDex2OatTest, WithAllOtherInputFds) {
    ...
    args->output_image = std::make_unique<UniqueFile>(IMAGE_FD, "UNUSED");
    args->profile = std::make_unique<UniqueFile>(PROFILE_FD, "UNUSED");
...
TEST_F(RunDex2OatTest, DoNotGenerateCompactDexWithVdexInPlaceUpdate) {
    ...
    args->input_vdex =
        std::make_unique<UniqueFile>(INPUT_VDEX_FD, "UNUSED_PATH");
    args->output_vdex =
        std::make_unique<UniqueFile>(INPUT_VDEX_FD, "UNUSED_PATH");
    CallRunDex2Oat(std::move(args));
