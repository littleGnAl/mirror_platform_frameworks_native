...
class RunDex2OatTest : public testing::Test {
  public:
    ...
    struct RunDex2OatArgs {
        ...
        bool generate_compact_dex = false;
    ...
    void initializeDefaultExpectedFlags() {
        ...
        default_expected_flags_["--compact-dex-level"] = "=none";
...
TEST_F(RunDex2OatTest, GenerateCompactDex) {
    auto args = RunDex2OatArgs::MakeDefaultTestArgs();
    args->generate_compact_dex = true;
    CallRunDex2Oat(std::move(args));

    SetExpectedFlagUsed("--compact-dex-level", FLAG_UNUSED);
    VerifyExpectedFlags();
}
