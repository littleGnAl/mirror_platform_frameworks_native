...
public:
    // Created invalid instance with no fd (=-1) and empty path.
    RestorableFile();
    ...
    // Close work file, delete it and reset all internal states into default states.
    void reset();

    // CLose work file and close all files including work file, backup file and regular file.
    void ResetAndRemoveAllFiles();

    // Create a backup file by renaming existing regular file. This will return false if renaming
    ...
    // Undo the commit and restore the backup file into the regular one. If renaming fails,
