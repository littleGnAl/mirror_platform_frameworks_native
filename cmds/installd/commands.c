    ...
    const char* output_file_name, const char *pkgname, const char *instruction_set)
{
    ...
    char boot_image_arg[strlen("--boot-image=") + PKG_PATH_MAX];
    ...
    // TODO: This is a hack. installd is is not the right place for this
    // information. It should be in the framework, because it's the package
    // manager that coordinates most of this.
    if (strcmp(instruction_set, "arm64") || strcmp(instruction_set, "x86_64")) {
        snprintf(boot_image_arg, sizeof(boot_image_arg),
                 "--boot-image=/data/dalvik-cache/system@framework@boot64.art");
    } else {
        snprintf(boot_image_arg, sizeof(boot_image_arg),
