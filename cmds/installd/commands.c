    ...
    const char* output_file_name, const char *pkgname, const char *instruction_set)
{
    ...
    bool have_dex2oat_arg = property_get("dalvik.vm.dex2oat-flags", dex2oat_flags, NULL) > 0;
    ...
    bool have_profile_top_k_arg = false;
    ...
    char* argv[7  // program name, mandatory arguments and the final NULL
               ...
               + (have_profile_top_k_arg ? 1 : 0)
               + (have_dex2oat_arg ? 1 : 0)];
    int i = 0;
    ...
    if (have_profile_top_k_arg) {
        argv[i++] = profile_top_k_arg;
    }
    if (have_dex2oat_arg) {
