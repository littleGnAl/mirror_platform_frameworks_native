...
int create_image_location(char path[PKG_PATH_MAX]) {
    //sprintf(path, "%s/%s", );
    return 0;
}
...
#if 0
int patchoat(const char *initial_path, uid_t uid, int is_public,
    ...
    if (zip_fd < 0) {
        ALOGE("installd cannot open '%s' for input during patchoat\n", apk_path);
        return -1;
    }
    ...
    if (out_fd < 0) {
        ALOGE("installd cannot open '%s' for output during patchoat\n", out_path);
        goto fail;
    }
    if (fchmod(out_fd,
               S_IRUSR|S_IWUSR|S_IRGRP |
               (is_public ? S_IROTH : 0)) < 0) {
        ALOGE("installd cannot chmod '%s' during patchoat\n", out_path);
        goto fail;
    }
    if (fchown(out_fd, AID_SYSTEM, uid) < 0) {
        ALOGE("installd cannot chown '%s' during patchoat\n", out_path);
        goto fail;
    }
...
{
    if (is_patchoat) {
      ALOGE("Got a patchoat request for %s pkg: %s\n", (apk_path) ? apk_path : "<unknown>",
