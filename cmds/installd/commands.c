...
int dexopt(const char *apk_path, uid_t uid, bool is_public,
           ...
           int dexopt_status, bool vm_safe_mode, bool debuggable)
{
    struct utimbuf ut;
    ...
    if (dexopt_status == DEXOPT_UP_TO_DATE) {
      return 0;
    }
    ...
    if (dexopt_status == DEXOPT_PATCHOAT_NEEDED) {
        if (!calculate_odex_file_path(in_odex_path, apk_path, instruction_set)) {
            return -1;
        }
        input_file = in_odex_path;
    } else if (dexopt_status == DEXOPT_SELF_PATCHOAT_NEEDED) {
        input_file = out_path;
    } else {
        input_file = apk_path;
    }
