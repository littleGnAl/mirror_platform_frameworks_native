...
{
    if (strlen(apk_path) + strlen("oat/")
        + strlen(instruction_set) + strlen("odex") >= (PKG_PATH_MAX - 1)) {
    ...
    strcpy(end + 1, "oat/");       // path now is /system/framework/oat/\0
    strcat(path, instruction_set); // path now is /system/framework/oat/<isa>\0
    strcat(path, apk_end);
    end = strrchr(path, '.');
    if (end == NULL) {
