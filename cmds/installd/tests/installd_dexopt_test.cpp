...
protected:
    ...
    ::testing::AssertionResult create_mock_app() {
        ...
        {
            int fd = open(dm_file_.c_str(), O_WRONLY | O_CREAT, S_IRUSR | S_IWUSR);
            if (fd < 0) {
            ...
            std::string profile_file_ = app_private_dir_ce_ + "/primary.prof";
            run_cmd("profman --generate-test-profile=" + profile_file_);
            ::chmod(profile_file_.c_str(), 0755);
            ::chown(profile_file_.c_str(), kTestAppUid, kTestAppGid);

            // Add profile to zip.
            ZipWriter writer(file);
            writer.StartEntry("primary.prof", ZipWriter::kCompress);

            android::base::unique_fd profileFd(open(profile_file_.c_str(), O_RDONLY));
            if (profileFd == -1) {
               return ::testing::AssertionFailure() << "Failed to open profile '" << profile_file_ << "'";
            }
            std::vector<char> buffer(64 * 1024);
            while (true) {
               ssize_t nread = TEMP_FAILURE_RETRY(read(profileFd, buffer.data(), buffer.size()));
               if (nread < 0) {
                   return ::testing::AssertionFailure() << "Failed to read profile '" << profile_file_ << "'";
               }
               if (nread == 0) {
                   break;
               }
               int result = writer.WriteBytes(buffer.data(), nread);
               if (result != 0) {
                   return ::testing::AssertionFailure() << "Failed to write zip entry '" << dm_file_ << "'";
               }
            }
            writer.FinishEntry();
            writer.Finish();
