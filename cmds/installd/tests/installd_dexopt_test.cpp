...
protected:
    ...
    void CompileSecondaryDex(const std::string& path, int32_t dex_storage_flag,
        ...
        if (should_dex_be_compiled) {
          ...
          // OAT file should have a different SELinux label.
          // TODO(dbrazdil): Check that the app process has "execute" permission.
          // TODO(dbrazdil): Set `se_info` to current target SDK version.
          ASSERT_NE(con_path, GetSelinuxContext(odex));
        }
