...
static constexpr const char* kTryMountFsTypes[] = {"ext4", "erofs"};
...
static void TryExtraMount(const char* name, const char* slot, const char* target) {
    ...
    [[maybe_unused]] bool ok = TryMountWithFstypes(block_device.c_str(), target);
}
