...
static int otapreopt_chroot(const int argc, char **arg) {
    ...
    if (mount("tmpfs", kPostinstallApexDir, "tmpfs", MS_NODEV | MS_NOEXEC | MS_NOSUID, nullptr)
        != 0) {
      // FIXME: dexoptota tests are failing with this error:
      //
      //   Failed to mount tmpfs in /postinstall/apex: Permission denied
      //
      // Add some logging to understand the origin of this denial.
