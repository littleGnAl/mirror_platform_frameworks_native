...
static void ActivateApexPackages(int status_fd) {
    std::vector<std::string> apexd_cmd{"/system/bin/apexd", "--otachroot-bootstrap"};
    std::string apexd_error_msg;

    bool exec_result = Exec(apexd_cmd, &apexd_error_msg, status_fd);
    if (!exec_result) {
...
static bool RunDexoptCommand(int argc, char **arg, const std::string& dexopt_cmd, int status_fd) {
    ...
    bool exec_result = Exec(cmd, &error_msg, status_fd);
    if (!exec_result) {
...
static int otapreopt_chroot(const int argc, char **arg) {
    ...
    bool linkerconfig_exec_result = Exec(linkerconfig_cmd, &linkerconfig_error_msg, fd);
    ...
    for (int iter = 0; iter < kMaximumPackages; iter++) {
        ...
        if (!RunDexoptCommand(argc, arg, android::String8(cmd).string(), fd)) {
            exit(213);
        }
