...
static binder::Status createAppDataDirs(const std::string& path,
    ...
    struct stat st_code_cache{};
    bool parent_dir_exists = (stat(path.c_str(), &st) == 0);
    bool cache_exists = (stat(cache_path.c_str(), &st_cache) == 0);
    bool code_cache_exists = (stat(code_cache_path.c_str(), &st_code_cache) == 0);
    ...
    if (parent_dir_exists && !cache_exists &&
        restorecon_app_data_lazy(path, "cache", seInfo, uid, false)) {
        return error("Failed to restorecon " + path + "/cache dir");
    }

    // If the parent dir exists, the restorecon would already have been done
    // as a part of the recursive restorecon above
    if (parent_dir_exists && !code_cache_exists &&
        restorecon_app_data_lazy(path, "code_cache", seInfo, uid, false)) {
        return error("Failed to restorecon " + path + "/code_cache dir");
    }
    return ok();
}
