...
static bool internal_storage_has_project_id() {
    ...
    if (!supports_sdcardfs()) {
        ...
        LOG(INFO) << "supports project id:" << access(first_boot_path.c_str(), F_OK);
...
static void deductDoubleSpaceIfNeeded(stats* stats, int64_t doubleSpaceToBeDeleted, uid_t uid,
    ...
    if (!supports_sdcardfs()) {
        LOG(INFO) << "data size before" << stats->dataSize;
        stats->dataSize -= doubleSpaceToBeDeleted;
        LOG(INFO) << "subtracted double" << doubleSpaceToBeDeleted;
        ...
        LOG(INFO) << "subtracted obb" << appObbSize;
        LOG(INFO) << "data size after" << stats->dataSize;
    }
}
...
static void collectQuotaStats(const std::string& uuid, int32_t userId,
    ...
    if (extStats != nullptr) {
        ...
        LOG(INFO) << "double space in ext stats " << doubleSpaceToBeDeleted;
    }

    if (stats != nullptr) {
        LOG(INFO) << "Inside stats calculation project supported:" << supportsProjectId;
        if (!supportsProjectId) {
            if ((space = GetOccupiedSpaceForUid(uuid, uid)) != -1) {
                stats->dataSize += space;
            }
            LOG(INFO) << "stats.datasize " << stats->dataSize;
            ...
            if (cacheGid != -1) {
                if ((space = GetOccupiedSpaceForGid(uuid, cacheGid)) != -1) {
                    ...
                    LOG(INFO) << "stats.cacheSize " << stats->cacheSize;
                }
            }
        } else {
            ...
            LOG(INFO) << "project id calculation data " << stats->dataSize;
            projectId = get_project_id(uid, PROJECT_ID_APP_CACHE_START);
            if ((space = GetOccupiedSpaceForProjectId(uuid, projectId)) != -1) {
                stats->cacheSize += space;
                stats->dataSize += space;
            }
            LOG(INFO) << "project id calculation cache " << stats->cacheSize;
            LOG(INFO) << "project id calculation cache: data " << stats->dataSize;
        }
        ...
        if (sharedGid != -1) {
            if ((space = GetOccupiedSpaceForGid(uuid, sharedGid)) != -1) {
                ...
                LOG(INFO) << "code calc " << stats->codeSize;
            }
        }
    }
}
...
binder::Status InstalldNativeService::getAppSize(const std::optional<std::string>& uuid,
    ...
    for (const auto& packageName : packageNames) {
        LOG(INFO) << "getAppSize: packageName" << packageName;
        CHECK_ARGUMENT_PACKAGE_NAME(packageName);
    }
    ...
    if (flags & FLAG_USE_QUOTA && appId >= AID_APP_START && !ownsExternalStorage(appId)) {
        LOG(INFO) << "Inside quota calculation";
        ATRACE_BEGIN("code");
        for (const auto& codePath : codePaths) {
            calculate_tree_size(codePath, &stats.codeSize, -1,
                    multiuser_get_shared_gid(0, appId));
            LOG(INFO) << "Inside code path calculation:" << codePath << &stats.codeSize;
        }
        ATRACE_END();
