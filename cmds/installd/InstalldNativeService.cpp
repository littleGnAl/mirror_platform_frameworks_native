...
binder::Status InstalldNativeService::getAppSize(const std::optional<std::string>& uuid,
    ...
    ATRACE_END();
    // Calculating the app size of the media app in a manual way, since calculating
    // it through quota apis also includes external media storage in the app storage numbers
