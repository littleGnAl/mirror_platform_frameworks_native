...
public:
    FsveritySetupAuthToken(int unowned_fd) { this->authFd.reset(unique_fd(unowned_fd)); }

    int getRawFd() const { return authFd.get(); }

    void revoke() { authFd.reset(); }
...
binder::Status InstalldNativeService::createFsveritySetupAuthToken(
    ...
    int dupFd = dup(authFd.get());
    if (dupFd < 0) {
        return exception(binder::Status::EX_SERVICE_SPECIFIC, "dup failed");
    }
    *_aidl_return = sp<FsveritySetupAuthToken>::make(dupFd); // take ownership
    return ok();
}
...
binder::Status InstalldNativeService::enableFsverity(const sp<IFsveritySetupAuthToken>& authToken,
    ...
    // Authenticate
    {
        ...
        int retval = fstat(authTokenInstance->getRawFd(), &stFromAuthFd);
        if (retval < 0) {
            *_aidl_return = retval;
            return ok();
        }
