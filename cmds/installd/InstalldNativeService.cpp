...
template <class Key>
struct shared_lock {
    using ChildLock = std::recursive_mutex;
    using Map = std::unordered_map<Key, std::shared_ptr<ChildLock>>;
    using MapLock = std::recursive_mutex;

    shared_lock(Key key, Map& map, MapLock& mapLock)
          : mKey(std::move(key)), mMap(map), mMapLock(mapLock) {}

    void lock() { getChildLock()->lock(); }
    void unlock() { getChildLock()->unlock(); }

private:
    std::shared_ptr<std::recursive_mutex> getChildLock() {
        std::lock_guard<MapLock> mapLock(mMapLock);
        auto& childLock = mMap[mKey];
        if (!childLock) {
            childLock = std::make_shared<ChildLock>();
        }
        return childLock;
    }
...
#define LOCK_USER()                                                  \
    shared_lock<userid_t> localUserLock(userId, mUserIdLock, mLock); \
    std::lock_guard userLock(localUserLock)

#define LOCK_PACKAGE()                                                               \
    shared_lock<std::string> localPackageLock(packageName, mPackageNameLock, mLock); \
...
#define LOCK_USER() std::lock_guard<std::recursive_mutex> lock(mLock)

#define LOCK_PACKAGE() std::lock_guard<std::recursive_mutex> lock(mLock)
