...
binder::Status InstalldNativeService::getUserSize(const std::unique_ptr<std::string>& uuid,
    ...
    if (flags & FLAG_USE_QUOTA) {
        ...
        ATRACE_BEGIN("obb");
        gid_t obbGid = multiuser_get_uid(userId, AID_MEDIA_OBB);
        if (quotactl(QCMD(Q_GETQUOTA, GRPQUOTA), device.c_str(), obbGid,
                reinterpret_cast<char*>(&dq)) != 0) {
            if (errno != ESRCH) {
                PLOG(ERROR) << "Failed to quotactl " << device << " for GID " << obbGid;
            }
        } else {
#if MEASURE_DEBUG
            LOG(DEBUG) << "quotactl() for GID " << obbGid << " " << dq.dqb_curspace;
#endif
