...
binder::Status InstalldNativeService::compileLayouts(const std::string& apkPath,
                                                     ...
                                                     const std ::string& outDexFile,
                                                     bool* _aidl_return) {
    const char* apk_path = apkPath.c_str();
    const char* package_name = packageName.c_str();
    const char* out_dex_file = outDexFile.c_str();
    *_aidl_return = android::installd::view_compiler(apk_path, package_name, out_dex_file);
    return *_aidl_return ? ok() : error("view_compiler failed");
}
