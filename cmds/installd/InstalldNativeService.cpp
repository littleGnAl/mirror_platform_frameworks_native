...
struct LocalLockHolder {
    using RefLock = InstalldNativeService::RefLock;
    using Map = std::unordered_map<Key, android::wp<RefLock>>;
    ...
    LocalLockHolder(Key key, Map& map, MapLock& mapLock)
        ...
        mRefLock = weakPtr.promote();
        if (!mRefLock) {
            // Create a new lock.
            mRefLock = new RefLock();
            weakPtr = mRefLock;
        }
    }
    ~LocalLockHolder() {
        ...
        const wp<RefLock>& weakPtr = found->second;
        // If this was the last pointer then it's ok to remove.
        if (!weakPtr.promote()) {
            mMap.erase(found);
        }
    }
...
private:
    ...
    Map& mMap;
    MapLock& mMapLock;
    sp<InstalldNativeService::RefLock> mRefLock;
};
