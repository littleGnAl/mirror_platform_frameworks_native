...
binder::Status InstalldNativeService::destroyAppData(const std::optional<std::string>& uuid,
    ...
    if (flags & FLAG_STORAGE_EXTERNAL) {
        ...
        for (const auto& n : mStorageMounts) {
            ...
            // We can't do rename-delete as we don't fully control external storage.
            auto path = StringPrintf("%s/Android/data/%s", extPath.c_str(), pkgname);
...
binder::Status InstalldNativeService::cleanupDeletedDirs(const std::optional<std::string>& uuid) {
    ...
    for (auto userId : users) {
        ...
        find_and_delete_renamed_deleted_dir(ce_path.c_str());
        find_and_delete_renamed_deleted_dir(de_path.c_str());
    }
    return ok();
}
