    ...
    if (flags & FLAG_STORAGE_DE) {
        ...
        if (property_get_bool("dalvik.vm.usejitprofiles", false)) {
            const std::string profile_dir =
                    create_primary_cur_profile_package_dir_path(userId, pkgname);
            ...
            const std::string ref_profile_path =
                    create_primary_ref_profile_package_dir_path(pkgname);
...
binder::Status InstalldNativeService::getAppSize(const std::unique_ptr<std::string>& uuid,
    ...
    if (flags & FLAG_USE_QUOTA && appId >= AID_APP_START) {
        ...
        for (size_t i = 0; i < packageNames.size(); i++) {
            ...
            auto userProfilePath = create_primary_cur_profile_package_dir_path(userId, pkgname);
            calculate_tree_size(userProfilePath, &stats.dataSize);
            auto refProfilePath = create_primary_ref_profile_package_dir_path(pkgname);
