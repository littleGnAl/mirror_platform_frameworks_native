...
binder::Status InstalldNativeService::enableFsverity(int32_t callingUid,
    ...
    // Only allows using it on /data for consistency.
    struct stat userdata_stat;
    if (stat("/data", &userdata_stat) != 0) {
        return error("Failed to stat /data");
    }
    if (st.st_dev != userdata_stat.st_dev) {
        return error("Only allowed in /data");
    }
