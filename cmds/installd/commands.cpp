...
static bool is_system_location(int dexopt_needed) {
  return dexopt_needed < 0;
}

static int get_dexopt_action(int dexopt_needed) {
  return abs(dexopt_needed);
}
           ...
           const char* volume_uuid ATTRIBUTE_UNUSED, const char* shared_libraries)
{
    ...
    int dexopt_action = get_dexopt_action(dexopt_needed);
    switch (dexopt_action) {
        ...
        case PATCHOAT_FOR_RELOCATION:
            if (is_system_location(dexopt_needed)) {
