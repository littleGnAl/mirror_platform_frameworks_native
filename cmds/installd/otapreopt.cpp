...
private:
    ...
    bool ReadEnvironment() {
        ...
        //
        // init.environ.rc, etc/classpath and, derive_classpath all have the
        // same format for environment variable exports (since they are all
        // meant to be read by init) and can be matched by the same regex.
        ...
        {
          // NB We intentionally do not set MFD_CLOEXEC.  We want to make sure
          // that derive_classpath can write to the fd.
          android::base::unique_fd fd(memfd_create("derive_classpath_temp", 0));
          if (!fd.ok()) {
