...
static int do_ping(char **arg UNUSED, char reply[REPLY_MAX] UNUSED)
{
    return 0;
}

static int do_install(char **arg, char reply[REPLY_MAX] UNUSED)
{
    return install(arg[0], atoi(arg[1]), atoi(arg[2]), arg[3]); /* pkgname, uid, gid, seinfo */
}

static int do_dexopt(char **arg, char reply[REPLY_MAX] UNUSED)
{
...
static int do_move_dex(char **arg, char reply[REPLY_MAX] UNUSED)
{
    return move_dex(arg[0], arg[1], arg[2]); /* src, dst, instruction_set */
}

static int do_rm_dex(char **arg, char reply[REPLY_MAX] UNUSED)
{
    return rm_dex(arg[0], arg[1]); /* pkgname, instruction_set */
}

static int do_remove(char **arg, char reply[REPLY_MAX] UNUSED)
{
    return uninstall(arg[0], atoi(arg[1])); /* pkgname, userid */
}

static int do_rename(char **arg, char reply[REPLY_MAX] UNUSED)
{
    return renamepkg(arg[0], arg[1]); /* oldpkgname, newpkgname */
}

static int do_fixuid(char **arg, char reply[REPLY_MAX] UNUSED)
{
    return fix_uid(arg[0], atoi(arg[1]), atoi(arg[2])); /* pkgname, uid, gid */
}

static int do_free_cache(char **arg, char reply[REPLY_MAX] UNUSED) /* TODO int:free_size */
{
    return free_cache((int64_t)atoll(arg[0])); /* free_size */
}

static int do_rm_cache(char **arg, char reply[REPLY_MAX] UNUSED)
{
    return delete_cache(arg[0], atoi(arg[1])); /* pkgname, userid */
}

static int do_get_size(char **arg, char reply[REPLY_MAX] UNUSED)
{
    int64_t codesize = 0;
    int64_t datasize = 0;
    int64_t cachesize = 0;
...
static int do_rm_user_data(char **arg, char reply[REPLY_MAX] UNUSED)
{
    return delete_user_data(arg[0], atoi(arg[1])); /* pkgname, userid */
}

static int do_mk_user_data(char **arg, char reply[REPLY_MAX] UNUSED)
{
    return make_user_data(arg[0], atoi(arg[1]), atoi(arg[2]), arg[3]);
                             /* pkgname, uid, userid, seinfo */
}

static int do_mk_user_config(char **arg, char reply[REPLY_MAX] UNUSED)
{
    return make_user_config(atoi(arg[0])); /* userid */
}

static int do_rm_user(char **arg, char reply[REPLY_MAX] UNUSED)
{
    return delete_user(atoi(arg[0])); /* userid */
}

static int do_movefiles(char **arg UNUSED, char reply[REPLY_MAX] UNUSED)
{
    return movefiles();
}

static int do_linklib(char **arg, char reply[REPLY_MAX] UNUSED)
{
    return linklib(arg[0], arg[1], atoi(arg[2]));
}

static int do_idmap(char **arg, char reply[REPLY_MAX] UNUSED)
{
...
static int do_patchoat(char **arg, char reply[REPLY_MAX] UNUSED) {
    /* apk_path, uid, is_public, pkgname, instruction_set, vm_safe_mode, should_relocate */
...
int main(const int argc UNUSED, const char *argv[] UNUSED) {
    char buf[BUFFER_MAX];
