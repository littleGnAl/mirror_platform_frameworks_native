...
class UniqueFile {
 ...
 public:
    ...
    void reset();
    void reset(int new_value, std::string path);
    void reset(int new_value, std::string path, CleanUpFunction new_cleanup);
 ...
 private:
    ...
    std::function<void (const std::string&)> cleanup_;
    bool do_cleanup_;
    bool auto_close_;
};
