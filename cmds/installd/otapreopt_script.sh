...
# This script runs as a postinstall step to drive otapreopt.
# See system/extras/postinst/postinst.sh for some docs.
...
if [ "$(/postinstall/system/bin/otapreopt_chroot --version)" != 2 ]; then
  # The chroot wrapper in the OTA package will have a correct sepolicy label but
  # it still depends on the (normally older) sepolicy rules in the running
  # system. Check that it works by reading its stdout. If that fails then the
  # sepolicy is too old and there's no use spending time trying to dexopt
  # (b/291974157).
...
print_otadexopt_cmds | \
  /postinstall/system/bin/otapreopt_chroot $STATUS_FD $TARGET_SLOT_SUFFIX | \
  report_progress
