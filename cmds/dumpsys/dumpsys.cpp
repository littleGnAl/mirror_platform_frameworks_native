...
static unique_fd OpenFile(const std::string& path) {
    int fd =
...
int Dumpsys::main(int argc, char* const argv[]) {
    ...
    for (size_t i = 0; i < N; i++) {
        ...
        if (service != nullptr) {
            ...
            if (N > 1) {
                if (dump_on_dir) {
                    std::string name = String16::std_string(service_name);
                    std::string path = dump_dir + "/" + name + ".txt";
                    fd = OpenFile(path);
                    fprintf(stderr, "DUMP OF SERVICE %s ON %s\n", name.c_str(), path.c_str());
                } else {
                    aout << "------------------------------------------------------------"
                            "-------------------"
                         << endl;
            ...
            if (N > 1) {
              ...
              if (dump_on_dir) {
                  dprintf(fd.get(), "--------- %.3fs was the duration\n", elapsed_seconds.count());
              } else {
