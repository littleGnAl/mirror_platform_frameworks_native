...
TEST_F(DumpsysTest, ListAllServicesWithThread) {
    ...
    auto status =
        android::defaultServiceManager()->addService(String16("Valet"), new android::BBinder);
    ASSERT_EQ(status, OK);
...
TEST_F(DumpsysTest, ListServiceWithThread) {
    ...
    android::ProcessState::self()->setThreadPoolMaxThreadCount(2);
    auto status =
        android::defaultServiceManager()->addService(String16("Locksmith"), new android::BBinder);
    ASSERT_EQ(status, OK);
    ProcessState::self()->startThreadPool();
