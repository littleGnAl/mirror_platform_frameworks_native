...
class DumpstateClientTest : public Test {
  public:
    void SetUp() {
        binder_ = sp<NiceMock<MockBinder>>::make();
        ds_ = sp<NiceMock<MockDumpstate>>::make();
...
#define EXPECT_CALL_START_BUGREPOR_ONCE_THEN_DO(obj, ret_status, reaction)  \
...
TEST_F(DumpstateClientTest, NoArgument) {
    EXPECT_CALL_START_BUGREPOR_ONCE_THEN_DO(*ds_, Status::ok(), [&]() {
...
// Tests 'bugreportz -p' - it will show dumpstate's progress to stdout
...
TEST_F(DumpstateClientTest, AnotherReportInProgress) {
    ...
    EXPECT_CALL_START_BUGREPOR_ONCE_THEN_DO(*ds_, st, [&]() {
...
TEST_F(DumpstateClientTest, DropRootUserFail) {
    EXPECT_CALL_START_BUGREPOR_ONCE_THEN_DO(*ds_, Status::ok(), [&]() {
