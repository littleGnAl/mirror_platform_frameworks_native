...
class DumpstateHelper {
  public:
    ...
    virtual ~DumpstateHelper() {}

    android::binder::Status StartBugreport(bool show_progress);

    void Wait();

  protected:
    /* Prepare dumpstate binder service. Calling in ctor. */
    virtual android::sp<android::os::IDumpstate> GetService();
    /* Initialize dumpstate listener. */
    virtual android::sp<android::os::IDumpstateListener> CreateListener();
