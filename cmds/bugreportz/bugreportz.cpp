...
int bugreportz(bool show_progress,
               const android::sp<android::os::bugreportz::DumpstateClient>& client) {
    android::binder::Status status = client->StartBugreport(show_progress);
    if (!status.isOk()) {
        printf("FAIL:Could not take the bugreport. (%s)\n", status.toString8().c_str());
        return EXIT_FAILURE;
    }
    android::status_t run_status = client->WaitForBugreport();
    if (run_status != android::OK) {
        printf("FAIL:Could not take the bugreport. (Error code %d)\n", run_status);
        return EXIT_FAILURE;
    }
    return EXIT_SUCCESS;
}
