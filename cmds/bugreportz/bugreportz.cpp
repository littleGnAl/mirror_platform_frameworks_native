...
std::string DumpstateHelper::GetPath(const std::string& suffix) const {
    return StringPrintf("/bugreports/%s-%s%s", GetBasename().c_str(), date_.c_str(),
                                       suffix.c_str());
}
...
void DumpstateHelper::Wait() {
    promise_finish_.get_future().wait();
}

Status DumpstateHelper::StartBugreport(bool show_progress) {
    if (!ds_.get() || !listener_.get()) {
    ...
    return ds_->startBugreport(
        ...
        android::os::IDumpstate::BUGREPORT_MODE_DEFAULT,
        listener_,
...
int bugreportz(bool show_progress, DumpstateHelper helper) {
    Status status = helper.StartBugreport(show_progress);
    if (!status.isOk()) {
