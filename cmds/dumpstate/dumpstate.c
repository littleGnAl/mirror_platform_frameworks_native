...
static void dump_emmc() {
    ...
    fd = TEMP_FAILURE_RETRY(open("/d/mmc0/mmc0:0001/ext_csd", O_RDONLY | O_NONBLOCK | O_CLOEXEC));
    if (fd < 0) {
        return;
    }
    ...
    printf("rev 1.%d (MMC %s)\n",
           ...
           (ext_csd_rev < (int)(sizeof(ver_str) / sizeof(ver_str[0]))) ?
               ...
               "Unknown");
    if ((ext_csd_rev < 7) ||
            (bytes_read < (ssize_t)(EXT_PRE_EOL_INFO * sizeof(hex)))) {
        printf("\n");
        return;
    }
