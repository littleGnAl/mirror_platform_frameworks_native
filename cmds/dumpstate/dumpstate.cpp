...
static void FinalizeFile() {
    ...
    } else if (ds.options_->use_control_socket) {
        if (do_text_file) {
...
Dumpstate::RunStatus Dumpstate::RunInternal(int32_t calling_uid,
    ...
    if (options_->do_stream_zip_file || options_->use_control_socket) {
    ...
    if (!PrepareToWriteToFile() && options_->do_stream_zip_file){
        // Only when the file could not be open while streaming.
        return ERROR;
    }
