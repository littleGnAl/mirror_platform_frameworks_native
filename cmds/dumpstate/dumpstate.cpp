...
static Dumpstate::RunStatus dumpstate() {
    ...
    if (ds.dump_pool_) {
        ...
        dump_incident_report = ds.dump_pool_->enqueueTask(DUMP_INCIDENT_REPORT_TASK, &DumpIncidentReport);
        dump_board = ds.dump_pool_->enqueueTaskWithFd(DUMP_BOARD_TASK, &Dumpstate::DumpstateBoard, &ds, _1);
...
Dumpstate::RunStatus Dumpstate::DumpstateDefaultAfterCritical() {
    ...
    if (dump_pool_) {
        ...
        dump_traces = dump_pool_->enqueueTask(DUMP_TRACES_TASK, &Dumpstate::DumpTraces, &ds, &dump_traces_path);
    } else {
