...
/*
 * Copies input_file to out_fd, if out_fd is not -1; otherwise to output_file path.
 * Returns true if successful.
 */
static bool CopyFileToOutputDirectoryOrFd(const std::string& input_file, int out_fd,
                                          const std::string& output_file) {
    // Obtain a handle to the source file.
    android::base::unique_fd in_fd(OpenForRead(input_file));

    // Obtain a handle to the destination file.
    android::base::unique_fd output_file_fd;
    if (out_fd == -1) {
        // out_fd is invalid; so fall back on input_file.
        ...
        output_file_fd.reset(OpenForWrite(output_file));
        out_fd = output_file_fd.get();
    } else {
...
static void DumpHals() {
    ...
    auto ret = sm->list([&](const auto& interfaces) {
        for (const std::string& interface : interfaces) {
            ...
            const std::string path = DUMPSTATE_DIRECTORY + "/lshal_debug_" + cleanName;
...
void Dumpstate::DumpstateBoard() {
    ...
    for (int i = 0; i < NUM_OF_DUMPS; i++) {
        paths.emplace_back(
            StringPrintf("%s/%s", DUMPSTATE_DIRECTORY.c_str(), kDumpstateBoardFiles[i].c_str()));
...
static void PrepareToWriteToFile() {
    create_parent_dirs(DUMPSTATE_DIRECTORY.c_str());
...
static void FinalizeFile() {
    ...
    if (ds.options_->do_zip_file) {
        if (!ds.FinishZipFile()) {
            ...
            if (ds.options_->fd == -1 && !ds.options_->use_outfile.empty()) {
                ds.final_path_ = ds.GetPath(ds.bugreport_dir_, ".zip");
            }
            if (android::os::CopyFileToOutputDirectoryOrFd(ds.path_, ds.options_->fd,
                                                           ds.final_path_)) {
...
/*
 * Dumps relevant informtation to a bugreport based on the given options.
 *
...
Dumpstate::RunStatus Dumpstate::Run() {
    ...
    std::string stats_path =
        is_redirecting
            ? android::base::StringPrintf("%s/dumpstate-stats.txt", DUMPSTATE_DIRECTORY.c_str())
            : "";
