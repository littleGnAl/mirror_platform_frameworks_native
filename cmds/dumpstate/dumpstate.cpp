 ...
 */
static bool CopyFileToOutputFileOrFd(const std::string& input_file, int out_fd,
    ...
    if (out_fd == -1) {
        // out_fd is invalid; so fall back on output_file.
...
static void FinalizeFile() {
    ...
    if (ds.options_->do_zip_file) {
        if (!ds.FinishZipFile()) {
            ...
            if (android::os::CopyFileToOutputFileOrFd(ds.path_, ds.options_->fd,
                                                      ds.final_path_)) {
