...
void Dumpstate::SetOptionsFromProperties() {
    ...
    if (!notification_title.empty()) {
        ...
        MYLOGD("notification (title:  %s, description: %s)\n",
               notification_title.c_str(), notification_description.c_str());
    }
}


bool Dumpstate::ValidateOptions() {
    if ((options_.do_zip_file ||
         options_.do_add_date || ds.update_progress_ || options_.do_broadcast) &&
        !options_.use_outfile) {
        return false;
    }
...
int run_main(int argc, char* argv[]) {
    ...
    if (is_redirecting) {
        ds.bugreport_dir_ = dirname(options.use_outfile);
        std::string build_id = android::base::GetProperty("ro.build.id", "UNKNOWN_BUILD");
        std::string device_name = android::base::GetProperty("ro.product.name", "UNKNOWN_DEVICE");
        ds.base_name_ = android::base::StringPrintf("%s-%s-%s", basename(options.use_outfile),
