...
static void PrepareToWriteToFile() {
    ...
    std::string destination = ds.options_->bugreport_fd != -1
                                  ? StringPrintf("[fd:%d]", ds.options_->bugreport_fd)
...
static void FinalizeFile() {
    ...
    if (ds.options_->do_zip_file) {
        if (!ds.FinishZipFile()) {
            ...
            if (ds.options_->bugreport_fd != -1) {
                copy_succeeded = android::os::CopyFileToFd(ds.path_, ds.options_->bugreport_fd);
            } else {
...
void Dumpstate::DumpOptions::Initialize(BugreportMode bugreport_mode,
    ...
    bugreport_fd = bugreport_fd_in.get();
    screenshot_fd = screenshot_fd_in.get();
...
bool Dumpstate::DumpOptions::ValidateOptions() const {
    if (bugreport_fd != -1 && !do_zip_file) {
        return false;
    }

    bool has_out_file_options = !use_outfile.empty() || bugreport_fd != -1;
