...
void Dumpstate::PrintHeader() const {
    ...
    printf(
        "Dumpstate info: id=%d pid=%d dry_run=%d parallel_run=%d lab_run=%d args=%s "
        "bugreport_mode=%s\n",
        id_, pid_, PropertiesHelper::IsDryRun(), PropertiesHelper::IsParallelRun(),
        PropertiesHelper::IsLabRun(), options_->args.c_str(),
...
static void DumpIncidentReport() {
    ...
    RunCommandToFd(fd, "", {"incident", "-u"},
                   CommandOptions::WithTimeout(PropertiesHelper::IsLabRun() ? 20 : 120).Build());
...
Dumpstate::RunStatus Dumpstate::RunInternal(int32_t calling_uid,
    ...
    if (PropertiesHelper::IsLabRun()) {
        MYLOGI(
            "Running on lab-run mode, which has shorter timeouts "
            "(to disable, call 'setprop dumpstate.lab_run false')\n");
    }
