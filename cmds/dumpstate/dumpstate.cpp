...
static void DumpExtFragUnusableIndexInfo() {
    struct stat st;
    ...
    auto unusable_index_regex = std::regex{"Node\\s+([0-9]+),\\s+zone\\s+([a-zA-z]+)\\s+(.*)"};
    for (std::string line; std::getline(ifs, line);) {
        ...
        if (std::regex_match(line, match_results, unusable_index_regex)) {
            ...
            for (int i = 0; i < free_pages_per_order.size(); i++) {
            ...
            for(unsigned int i = 0; i < free_pages_per_order.size(); i++) {
