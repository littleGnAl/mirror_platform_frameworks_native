...
Dumpstate::RunStatus Dumpstate::DumpOptions::Initialize(int argc, char* argv[]) {
    ...
    while ((c = getopt(argc, argv, "dho:svqzpLPBRSV:w")) != -1) {
        switch (c) {
            ...
            case 'z':                                break;  // compatibility no-op
            case 'w':
...
Dumpstate::RunStatus Dumpstate::RunInternal(int32_t calling_uid,
    ...
    // Interactive, wear & telephony modes are default to do progress update
    // and this var may enable from cli option or when using control socket
