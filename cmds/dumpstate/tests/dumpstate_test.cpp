...
class ZippedBugReportStreamTest : public DumpstateBaseTest {
  public:
    ...
    void GenerateBugreport() {
        ...
        ds.Initialize();
        EXPECT_EQ(Dumpstate::RunStatus::OK, ds.Run(/*calling_uid=*/-1, /*calling_package=*/""));
    }
    ...
    void GetEntry(const ZipArchiveHandle archive, const std::string_view entry_name,
                  ZipEntry* data) {
...
TEST_F(ZippedBugReportStreamTest, StreamWifiReport) {
    ...
    ZipEntry entry;
    GetEntry(handle, "main_entry.txt", &entry);
