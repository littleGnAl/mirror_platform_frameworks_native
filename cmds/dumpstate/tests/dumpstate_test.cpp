...
class ZippedBugReportStreamTest : public DumpstateBaseTest {
  public:
    ...
    void GenerateBugreport() {
        // Running the task in another thread and DropRootUser there if called.
        auto future_ = std::async(std::launch::async, [this]() -> Dumpstate::RunStatus {
            ds.Initialize();
            return ds.Run(/*calling_uid=*/-1, /*calling_package=*/"");
        });
        // Now, block until bugreport is complete.
        EXPECT_EQ(Dumpstate::RunStatus::OK, future_.get());
    }

    void OpenArchiveFromString(std::string& content, ZipArchiveHandle* handle) {
        TemporaryFile debugFile(kTestDataPath);
        auto err = OpenArchiveFromMemory(reinterpret_cast<uint8_t*>(content.data()),
                                         content.length(), debugFile.path, handle);
        EXPECT_EQ(0, err) << "Can't open " << debugFile.path << " : " << ErrorCodeString(err);
        ASSERT_TRUE(*handle);
    }
    ...
    void SetOpenSocketFn(std::function<int(const char*)> fn) {
        ds.open_socket_fn_ = std::move(fn);
    };
...
// Generate a quick wifi report to stdout, open it and verify entry exist.
TEST_F(ZippedBugReportStreamTest, StreamWifiReport) {
    ds.options_->wifi_only = true;
    ds.options_->stream_to_socket = true;
    SetOpenSocketFn([](const char*) -> int { return STDOUT_FILENO; });

    CaptureStdout();
    GenerateBugreport();
    std::string output = GetCapturedStdout();
    OpenArchiveFromString(output, &handle);

    ZipEntry entry;
    VerifyEntry(handle, "main_entry.txt", &entry);
    std::string bugreport_txt_name;
    bugreport_txt_name.resize(entry.uncompressed_length);
    ExtractToMemory(handle, &entry, reinterpret_cast<uint8_t*>(bugreport_txt_name.data()),
                    entry.uncompressed_length);
    EXPECT_THAT(bugreport_txt_name,
                testing::ContainsRegex("bugreport-.*-wifi(-[[:digit:]]+){6}.txt"));
    VerifyEntry(handle, bugreport_txt_name, &entry);
}
