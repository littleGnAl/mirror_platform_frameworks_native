...
void GetEntry(const ZipArchiveHandle archive, const std::string_view entryName, ZipEntry* data) {
    int32_t e = FindEntry(archive, entryName, data);
    EXPECT_EQ(e, 0) << ErrorCodeString(e) << " entry name: " << entryName;
}

// Extracts the main bugreport txt from the given archive and writes into out_fd.
void ExtractBugreport(ZipArchiveHandle* handle, int out_fd) {
    ...
    // Read the main bugreport txt and extract to out_fd.
    ZipEntry entry;
    GetEntry(*handle, bugreport_txt_name, &entry);
    ExtractEntryToFile(*handle, &entry, out_fd);
}
