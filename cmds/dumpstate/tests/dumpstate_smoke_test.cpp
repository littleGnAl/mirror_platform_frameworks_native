...
int OpenForWrite(const std::string& filename) {
    return unique_fd(TEMP_FAILURE_RETRY(open(filename.c_str(),
...
class DumpstateBinderTest : public Test {
  protected:
    ...
    // Waits until listener gets the callbacks and verifies the execution is complete.
    void WaitAndExpectExecutionComplete(DumpstateListener* listener) {
        // Wait till one of finished, error or timeout.
        static const int kBugreportTimeoutSeconds = 120;
        int i = 0;
        while (!listener->is_finished_ && listener->error_code_ == -1 &&
        ...
        EXPECT_TRUE(
            listener->error_code_ == IDumpstateListener::BUGREPORT_ERROR_USER_DENIED_CONSENT ||
            listener->error_code_ == IDumpstateListener::BUGREPORT_ERROR_USER_CONSENT_TIMED_OUT);
    }
};
...
TEST_F(DumpstateBinderTest, Baseline) {
    ...
    WaitAndExpectExecutionComplete(listener.get());
...
TEST_F(DumpstateBinderTest, ServiceDies_OnInvalidInput) {
    ...
    android::binder::Status status =
        ds_binder->startBugreport(123, "com.dummy.package", bugreport_fd, screenshot_fd,
                                  ...
                                  listener);
    EXPECT_EQ(listener->error_code_, IDumpstateListener::BUGREPORT_ERROR_INVALID_INPUT);
...
TEST_F(DumpstateBinderTest, SimultaneousBugreportsNotAllowed) {
    ...
    EXPECT_EQ(listener2->error_code_, IDumpstateListener::BUGREPORT_ERROR_RUNTIME_ERROR);
