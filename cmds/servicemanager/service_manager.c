...
static bool check_mac_perms(const char *name, pid_t spid, const char *perm)
{
    ...
    /* If name is NULL, we can't retrieve the tctx and will use servicemanager.*/
    if (name == NULL) {
        if (getcon(&tctx) != 0) {
    ...
    } else if (selabel_lookup(sehandle, &tctx, name, 1) != 0) {
...
static int svc_can_list(uid_t uid, pid_t spid)
{
    ...
    return check_mac_perms(NULL, spid, perm) ? 1 : 0;
}
                   ...
                   struct binder_io *reply)
{
    ...
    case SVC_MGR_LIST_SERVICES: {
        ...
        if (svc_can_list(txn->sender_euid, txn->sender_pid)) {
            si = svclist;
            while ((n-- > 0) && si)
                si = si->next;
            if (si) {
