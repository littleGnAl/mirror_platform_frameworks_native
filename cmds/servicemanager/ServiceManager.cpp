...
Status ServiceManager::addService(const std::string& name, const sp<IBinder>& binder, bool allowIsolated, int32_t dumpPriority) {
    ...
    if (binder->remoteBinder() != nullptr && binder->linkToDeath(sp<ServiceManager>::assertStrongRefExists(this)) != OK) {
...
Status ServiceManager::registerForNotifications(
    ...
    if (OK != IInterface::asBinder(callback)->linkToDeath(sp<ServiceManager>::assertStrongRefExists(this))) {
        LOG(ERROR) << "Could not linkToDeath when adding " << name;
        return Status::fromExceptionCode(Status::EX_ILLEGAL_STATE);
    }
...
Status ServiceManager::registerClientCallback(const std::string& name, const sp<IBinder>& service,
    ...
    if (OK != IInterface::asBinder(cb)->linkToDeath(sp<ServiceManager>::assertStrongRefExists(this))) {
...
ssize_t ServiceManager::Service::getNodeStrongRefCount() {
    sp<BpBinder> bpBinder = sp<BpBinder>::assertStrongRefExists(binder->remoteBinder());
