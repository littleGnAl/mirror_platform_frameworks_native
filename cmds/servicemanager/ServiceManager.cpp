...
Status ServiceManager::addService(const std::string& name, const sp<IBinder>& binder, bool allowIsolated, int32_t dumpPriority) {
    ...
    if (it != mNameToService.end()) {
        ...
        if (existing.ctx.debugPid != ctx.debugPid) {
            LOG(INFO) << "Service '" << name << "' originally registered from PID "
                      << existing.ctx.debugPid << " but it is now being registered from PID "
                      << ctx.debugPid
                      << ". New process restarted before we got death notification? Otherwise, "
