...
TEST(Vintf, UpdatableViaApex) {
    ...
    EXPECT_TRUE(sm->updatableViaApex("android.hardware.drm.IDrmFactory/clearkey", &updatableViaApex)
                        .isOk());
    EXPECT_EQ(std::make_optional<std::string>("com.android.hardware.clearkey"), updatableViaApex);
}
...
TEST(Vintf, UpdatableViaApex_InvalidNameReturnsNullOpt) {
    ...
    auto sm = getPermissiveServiceManager();
    std::optional<std::string> updatableViaApex;
    EXPECT_TRUE(sm->updatableViaApex("android.hardware.drm.IDrmFactory", &updatableViaApex)
...
TEST(Vintf, GetUpdatableNames) {
    ...
    EXPECT_TRUE(sm->getUpdatableNames("com.android.hardware.clearkey", &names).isOk());
    EXPECT_EQ(std::vector<std::string>{"android.hardware.drm.IDrmFactory/clearkey"}, names);
}
