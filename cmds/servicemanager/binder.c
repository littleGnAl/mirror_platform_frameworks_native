...
void binder_dump_txn(struct binder_transaction_data* txn)
{
    ...
    fprintf(stderr, "  target %016" PRIx64 "  cookie %016" PRIx64
                    "  code %08x  flags %08x\n",
            (uint64_t)txn->target.ptr, (uint64_t)txn->cookie,
            txn->code, txn->flags);
    ...
    while (count--) {
        obj = (struct flat_binder_object*) (
                ((char*)(uintptr_t)txn->data.ptr.buffer) + *offs++);
        fprintf(stderr, "  - type %08x  flags %08x  ptr %016" PRIx64
                        "  cookie %016" PRIx64 "\n",
                obj->hdr.type, obj->flags, (uint64_t)obj->binder,
                (uint64_t)obj->cookie);
    }
}
...
struct binder_state* binder_open(size_t mapsize)
{
    ...
    if (bs->fd < 0) {
        ALOGE("cannot open device (%s)", strerror(errno));
        goto fail_open;
    }

    if ((ioctl(bs->fd, BINDER_VERSION, &vers) == -1) ||
        (vers.protocol_version != BINDER_CURRENT_PROTOCOL_VERSION)) {
        ALOGE("kernel driver version (%d) differs from user space version (%d)",
              vers.protocol_version, BINDER_CURRENT_PROTOCOL_VERSION);
        goto fail_open;
    }
    ...
    if (bs->mapped == MAP_FAILED) {
        ALOGE("cannot map device (%s)", strerror(errno));
        goto fail_map;
    }
...
int binder_write(struct binder_state* bs, void* data, size_t len)
{
    ...
    if (res < 0) {
        ALOGE("binder_write: ioctl failed (%s)", strerror(errno));
    }
    return res;
}
...
int binder_call(struct binder_state* bs, struct binder_io* msg,
                struct binder_io* reply, uint32_t target, uint32_t code)
{
    int res;
    struct binder_write_read bwr;
    struct {
...
void binder_loop(struct binder_state* bs, binder_handler func)
{
    ...
    for (;;) {
        ...
        if (res < 0) {
            ALOGE("binder_loop: ioctl failed (%s)", strerror(errno));
            break;
        }
        ...
        if (res < 0) {
            ALOGE("binder_loop: io error %d %s", res, strerror(errno));
            break;
        }
    }
}

static void bio_init_from_txn(struct binder_io* bio,
                              struct binder_transaction_data* txn)
{
