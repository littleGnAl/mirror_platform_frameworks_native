...
std::string ListCommand::parseCmdline(pid_t pid) {
...
void ListCommand::dump() {
    if (mVintf) {
        if (!!mFileOutput) {
            dumpVintf(NullableOStream<std::ostream>(mFileOutput.get()));

            // prevent writing more content to the XML file
            mFileOutput->close();
            mFileOutput = nullptr;
            mLshal.popOutputStream();
        } else {
            dumpVintf(out());
        }
    } else {
...
Status ListCommand::parseArgs(const std::string &command, const Arg &arg) {
    ...
    for (;;) {
        ...
        case 'v': {
            if (optarg) {
                mFileOutput = std::make_unique<std::ofstream>(optarg);
                mLshal.pushOutputStream(*mFileOutput);
                if (!mFileOutput->is_open()) {
                    err() << "Could not open file '" << optarg << "'." << std::endl;
                    return IO_ERROR;
                }
            }
            mVintf = true;
