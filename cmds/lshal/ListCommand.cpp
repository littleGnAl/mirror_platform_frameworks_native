...
bool scanBinderContext(pid_t pid,
    ...
    static const std::regex kContext("^\\context \\w+");

    bool isDesiredContext = false;
    std::string line;
    std::smatch match;
    while(getline(ifs, line)) {
        if (std::regex_search(line, match, kContext)) {
...
bool ListCommand::getPidInfo(
    ...
    static const std::regex kThreadLine("^\\s*thread \\d+: l (\\d)\\d need_return 0\\s*$");
    ...
    return scanBinderContext(serverPid, "hwbinder", [&](const std::string& line) {
        ...
        if (std::regex_search(line, match, kThreadLine)) {
            bool waitingForWork = match.str(1) == "1";
    ...
    forEachTable([this] (const Table &table) {
        ...
        if (!mNeat) {
            printLine("Interface", "Transport", "Arch", "Server", "Server CMD",
                      "PTR", "Thread Use", "Clients", "Clients CMD");
        }
...
Status ListCommand::fetchBinderized(const sp<IServiceManager> &manager) {
    ...
    for (auto &pair : allPids) {
        ...
        if (!getPidInfo(serverPid, &pair.second)) {
    ...
    for (const auto &fqInstanceName : fqInstanceNames) {
        ...
        putEntry(HWSERVICEMANAGER_LIST, {
            ...
            .clientPids = writePidInfo ? Pids{} : allPids[info.pid].refPids[info.ptr],
            .threadUsage = writePidInfo ? 0 : allPids[info.pid].threadUsage,
            .threadCount = writePidInfo ? 0 : allPids[info.pid].threadCount,
            .arch = fromBaseArchitecture(info.arch),
        });
    }
    return status;
