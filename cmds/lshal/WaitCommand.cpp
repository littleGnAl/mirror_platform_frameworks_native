...
Status WaitCommand::main(const Arg &arg) {
    ...
    IServiceManager::Transport declared = mLshal.serviceManager()->getTransport(interface, instance);
    if (declared == IServiceManager::Transport::EMPTY) {
        mLshal.err() << "Service not found in VINTF manifest (missing permissions or not in manifest?).\n";
        return NO_INTERFACE;
    }

    using android::hardware::details::getRawServiceInternal;
    (void)getRawServiceInternal(interface, instance, true /*retry*/, false /*getStub*/);
    return OK;
}
