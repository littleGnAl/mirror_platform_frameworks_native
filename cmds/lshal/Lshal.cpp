...
std::vector<std::string> Lshal::getCmdlines(Pids *pids) {
    static const pid_t myPid = getpid();
    std::vector<std::string> ret;
    for (auto it = pids->begin(); it != pids->end(); /* empty */ ) {
        if (*it == myPid) {
            it = pids->erase(it);
            continue;
        }
        std::string s = getCmdline(*it);
        if (s.empty()) {
            it = pids->erase(it);
            continue;
        }
...
void Lshal::postprocess() {
    ...
    for (TableEntry &entry : mTable) {
        ...
        entry.clientCmdlines = getCmdlines(&entry.clientPids);
    }
}
...
void Lshal::dump() const {
    ...
    mOut << std::left;
    printLine("Interface", "Instance", "Transport", "Server", "Server CMD", "PTR", "Clients", "Clients CMD");
...
Status Lshal::parseArgs(int argc, char **argv) {
    ...
    for (;;) {
        ...
        c = getopt_long(argc, argv, "hintpacm", longOptions, &optionIndex);
        if (c == -1) {
            break;
        }
        switch (c) {
