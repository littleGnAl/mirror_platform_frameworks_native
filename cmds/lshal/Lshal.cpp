...
Status Lshal::emitDebugInfo(
    ...
    if (!retBase.isOk()) {
        err << "Cannot get " << interfaceName << "/" << instanceName << ": "
            << retBase.description();
    ...
    if (base == nullptr) {
        err << interfaceName << "/" << instanceName << " does not exist, or "
            << "no permission to connect." << std::endl;
        return NO_INTERFACE;
    }
    ...
    if (relay.initCheck() != OK) {
        err << "PipeRelay::initCheck() FAILED w/ " << relay.initCheck() << std::endl;
        return IO_ERROR;
    }
    ...
    if (!ret.isOk()) {
        err << "debug() FAILED on " << interfaceName << "/" << instanceName << ": "
            << ret.description();
        return TRANSACTION_ERROR;
    }
    return OK;
